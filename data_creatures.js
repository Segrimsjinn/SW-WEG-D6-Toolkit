const CREATURES_DATA = [
 {
  "name": "Acklay",
  "type": "Predator",
  "planet": "Geonosis",
  "habitat": "Desert/Cave",
  "dex": "1D+1",
  "per": "2D+1",
  "str": "7D",
  "move": "12",
  "size": "6 meters long",
  "orneriness": "",
  "special": "Armor: +2D STR vs physical. Bite: STR+2. Claws: STR+1D. Exceptional Reflexes: +1D initiative. Resistant to Radiation: +1D STR vs radiation.",
  "source": "d20 Core Rulebook (page 335)"
 },
 {
  "name": "Adar",
  "type": "Adaptive carnivore",
  "planet": "Omiddelon III",
  "habitat": "Forest, jungle, swamp, urban",
  "dex": "2D",
  "per": "2D",
  "str": "4D",
  "move": "13 (running), 15 (leaping), 22 (flying, 5 min/hr)",
  "size": "Up to 2.5 meters long",
  "orneriness": "",
  "special": "Search: tracking 3D+2. Brawling 6D, climbing/jumping 6D, flight 6D+2. Claws: STR+2D damage. Leaping Attack: STR+3D damage. Wings: flight. Adaptive: can metabolize virtually any food source. Hunts in packs of 4-36. Creature scale.",
  "source": "WEG40080 Creatures of the Galaxy p.4"
 },
 {
  "name": "Aduban Behemoth",
  "type": "Dark side experiment",
  "planet": "Aduba-3",
  "habitat": "Various",
  "dex": "1D",
  "per": "1D+1",
  "str": "11D",
  "move": "6",
  "size": "Gargantuan",
  "orneriness": "",
  "special": "Bite: STR+2D+2. Punch: STR+1D. Energy Beam: 4D damage, range 20/40/80m. Darkvision: 20m. Lightsaber Vulnerability: lightsaber damage doubled, wild die 6 may cause explosion.",
  "source": "Geonosis and the Outer Rim Worlds"
 },
 {
  "name": "Aga",
  "type": "Immense predator",
  "planet": "Zelos II",
  "habitat": "Forest/Mountain",
  "dex": "4D",
  "per": "3D",
  "str": "5D",
  "move": "9",
  "size": "4 meters tall",
  "orneriness": "5D",
  "special": "Bite: STR+1D. Claws: STR+1D+2. Clubbing Attack: STR+2D. Screech: Easy PER or stunned 1 round. Armor: +2D physical, +1D energy.",
  "source": "Planets Collection (page 252)"
 },
 {
  "name": "Agr",
  "type": "Flying predator",
  "planet": "Kashyyyk",
  "habitat": "Forest",
  "dex": "2D",
  "per": "2D+2",
  "str": "2D+1",
  "move": "15 (flying)",
  "size": "1.1 meters tall, 2m wingspan",
  "orneriness": "",
  "special": "Beak: STR+1D. Claws: STR+2.",
  "source": ""
 },
 {
  "name": "Airsquid",
  "type": "Small airborne vermin",
  "planet": "Bespin",
  "habitat": "Atmosphere",
  "dex": "3D+2",
  "per": "2D+1",
  "str": "1D",
  "move": "40 (flying)",
  "size": "Small",
  "orneriness": "",
  "special": "Exceptional Reflexes: +1D initiative. Bite: STR+2. Strong-Minded: +3D PER vs mind-influencing.",
  "source": "Geonosis and the Outer Rim Worlds (page 25)"
 },
 {
  "name": "Aiwha",
  "type": "Colossal aquatic herd animal",
  "planet": "Kamino",
  "habitat": "Aquatic",
  "dex": "1D+1",
  "per": "2D+1",
  "str": "6D",
  "move": "20 (swimming/flying)",
  "size": "20-30 meters wingspan",
  "orneriness": "1D",
  "special": "Slamming: STR+2D.",
  "source": "Geonosis and the Outer Rim Worlds (page 78)"
 },
 {
  "name": "Akorec",
  "type": "Reptilian predator",
  "planet": "Trammis",
  "habitat": "Various",
  "dex": "4D+2",
  "per": "1D",
  "str": "5D+1",
  "move": "20",
  "size": "10 meters tall",
  "orneriness": "",
  "special": "Bite: STR+1D+2. Blaster-Resistant Scales: +4D STR vs blaster damage. Claws: STR+2. Low Light Vision.",
  "source": ""
 },
 {
  "name": "Albino Farlek",
  "type": "Dangerous predator",
  "planet": "",
  "habitat": "Various",
  "dex": "3D",
  "per": "3D+2",
  "str": "6D",
  "move": "12",
  "size": "2m tall, 4m long",
  "orneriness": "",
  "special": "Fangs: STR+1D. Snare-tongue: sticky adhesive (Difficult STR to break), 2m range. Tail: STR+2D. Odor: offensive scent.",
  "source": "Operation: Elrood (page 63)"
 },
 {
  "name": "Altagak",
  "type": "Solitary carnivore",
  "planet": "Altora",
  "habitat": "Savannah, plains",
  "dex": "4D+2",
  "per": "3D",
  "str": "4D",
  "move": "8 (walking), 28 (attack sprint)",
  "size": "1.8-2.2 meters long",
  "orneriness": "",
  "special": "Running 6D. Sneak 5D (+1D when stalking prey). Claws: STR+2 damage. Tusks: STR+2D damage (head butt charge). Teeth: STR+1D damage. Tail: STR+1 damage. Speed: attack sprint for up to 8 rounds (80+ km/hr). Threatening Display: expands spinal mane to appear larger.",
  "source": "WEG40080 Creatures of the Galaxy p.5"
 },
 {
  "name": "Andoan Mineral-Fish",
  "type": "Mineral-consuming shellfish",
  "planet": "Ando",
  "habitat": "Aquatic",
  "dex": "1D",
  "per": "1D",
  "str": "1D",
  "move": "8",
  "size": "1 meter long",
  "orneriness": "",
  "special": "Tail: STR+2D damage. Fins: STR+1 damage. Shell: +1D STR to resist damage. Mineral Sense: can sense large concentrations of minerals and metals. Creature scale.",
  "source": "WEG40080 Creatures of the Galaxy p.6"
 },
 {
  "name": "Apilid Worm",
  "type": "Subterranean predator",
  "planet": "Vasha",
  "habitat": "Subterranean",
  "dex": "3D",
  "per": "1D",
  "str": "2D+1",
  "move": "8",
  "size": "Small",
  "orneriness": "",
  "special": "Bite: STR+2. Blindsight: uses vibrations.",
  "source": ""
 },
 {
  "name": "Armadid",
  "type": "Mountain predator",
  "planet": "Nubia",
  "habitat": "Mountain",
  "dex": "3D+1",
  "per": "1D+2",
  "str": "4D+2",
  "move": "10",
  "size": "3 meters long",
  "orneriness": "",
  "special": "Camouflage: +2D hide in rocky terrain. Exceptional Reflexes: +1D initiative. Force-Sensitive. Horns: STR+2. Low-light Vision. Natural Armor: +2D vs physical, +1D vs energy. Tail pincer: STR damage.",
  "source": "Coruscant and the Core Worlds (page 123)"
 },
 {
  "name": "Arquet",
  "type": "Mountain carnivore",
  "planet": "Pellastrallas",
  "habitat": "Mountain",
  "dex": "3D+2",
  "per": "3D",
  "str": "3D",
  "move": "9/12 (charging)",
  "size": "1.8-2.4 meters long",
  "orneriness": "",
  "special": "Armor: +2D vs physical and energy. Camouflage. Claws: STR+2. Feigned Immobility. Horns: STR+2D. Teeth: STR+1D.",
  "source": "Creatures of the Galaxy (pages 7-8)"
 },
 {
  "name": "Ash Angel",
  "type": "Small airborne scavenger",
  "planet": "Sullust",
  "habitat": "Volcanic/Cave",
  "dex": "4D+1",
  "per": "2D+1",
  "str": "1D+1",
  "move": "4/10 (flying)",
  "size": "1m tall, 2.5m wingspan",
  "orneriness": "",
  "special": "Poison: Difficult Stamina or -1D STR, then paralyzed 1D half-hours. Camouflage: +2 hide in ashy terrain. Fire/Blaster Resistance: +2D resist.",
  "source": "Ultimate Adversaries (pages 67-68)"
 },
 {
  "name": "Asyyyriak",
  "type": "Medium-size forest predator",
  "planet": "Kashyyyk",
  "habitat": "Forest",
  "dex": "4D+2",
  "per": "5D",
  "str": "4D",
  "move": "8",
  "size": "1-2 meters long",
  "orneriness": "",
  "special": "Camouflage: +2D hide in forest. Exceptional Reflexes: +1D initiative. Bite: STR+1D. Claws: STR+2. Multiattack: 2 brawling attacks no penalty. Fast Healing: 1 injury level per 2 min.",
  "source": "Ultimate Adversaries (pages 68-69)"
 },
 {
  "name": "Attack Stohl",
  "type": "Domestic defense pet",
  "planet": "Ichtor 8, Endor",
  "habitat": "Various",
  "dex": "3D",
  "per": "2D",
  "str": "2D",
  "move": "5",
  "size": "1-1.5 meters long",
  "orneriness": "",
  "special": "Bite: STR+1D. Poison: +3D stun damage (12hr recovery). Coil: Moderate STR to wrap, Difficult STR to disengage.",
  "source": "WEG40080 Creatures of the Galaxy p.8"
 },
 {
  "name": "Avril",
  "type": "Avian cliff dweller",
  "planet": "Yavin 8",
  "habitat": "Jungle/Mountain",
  "dex": "2D",
  "per": "2D",
  "str": "2D",
  "move": "6/15 (flight)",
  "size": "1.8-2.5m tall, 8m wingspan",
  "orneriness": "",
  "special": "Acute Vision: +2D visual search. Bite: STR+2. Talons: STR+2.",
  "source": "Galaxy Guide 2 (pages 23-24)"
 },
 {
  "name": "B'karr",
  "type": "Scavenger",
  "planet": "Goroth",
  "habitat": "Mountain/Desert",
  "dex": "2D",
  "per": "2D+1",
  "str": "1D",
  "move": "13",
  "size": "0.3 meters long",
  "orneriness": "",
  "special": "Bite: STR+1. Poisonous Saliva: 3D stun on damage. Poison Resistance: +3D STR vs poison. Camouflage: +1D sneak in broken rock.",
  "source": "Goroth, Slave of the Empire (pages 91-92)"
 },
 {
  "name": "Bandara",
  "type": "Semi-aquatic pest",
  "planet": "Devaron",
  "habitat": "Swamp/Aquatic",
  "dex": "2D",
  "per": "1D",
  "str": "+1",
  "move": "1 (swarm)/15 (individual)",
  "size": "2 centimeters",
  "orneriness": "",
  "special": "Mating Song: Easy PER or -1D all actions (Difficult willpower negates). Hunger: eat anything, 1D damage/round contact.",
  "source": "WEG40080 Creatures of the Galaxy p.11"
 },
 {
  "name": "Bandigo",
  "type": "Aggressive omnivore",
  "planet": "Sriluur",
  "habitat": "Desert/Mountain",
  "dex": "3D",
  "per": "3D",
  "str": "3D",
  "move": "12/9 (climbing)",
  "size": "1.4m at shoulder",
  "orneriness": "",
  "special": "Claws: STR+2D. Stone throwing: STR+1D (grenade ranges).",
  "source": "Geonosis and the Outer Rim Worlds (pages 130-131)"
 },
 {
  "name": "Bantha",
  "type": "Herd animal",
  "planet": "Unknown",
  "habitat": "Desert/Plains",
  "dex": "2D",
  "per": "2D",
  "str": "8D",
  "move": "5",
  "size": "2-3m at shoulder",
  "orneriness": "2D",
  "special": "Horns: STR-1D. Trample: STR.",
  "source": "Core Rulebook (page 222)"
 },
 {
  "name": "Bantha (Tatooine)",
  "type": "Beast of burden / Tusken mount",
  "planet": "Tatooine",
  "habitat": "Desert",
  "dex": "2D",
  "per": "1D",
  "str": "5D, stamina 8D",
  "move": "15",
  "size": "2-3m at shoulder",
  "orneriness": "3",
  "special": "Hardy constitution: Can go weeks without food or water and quickly adapts to new surroundings and climates.",
  "source": "WEG40124 Galaxy Guide 1: A New Hope p.21"
 },
 {
  "name": "Barri",
  "type": "Mysterious spacefaring creature",
  "planet": "",
  "habitat": "Space",
  "dex": "2D",
  "per": "1D",
  "str": "4D",
  "move": "10",
  "size": "4 meters long",
  "orneriness": "",
  "special": "Corrosive Spittle: 3D damage/turn until washed away, eats through armor. Innate Navigational Sense: astrogation 6D. Creature scale. Move is meters/round, not Space units.",
  "source": "WEG40080 Creatures of the Galaxy p.14"
 },
 {
  "name": "Battle Hydra",
  "type": "Alchemical Sith creation",
  "planet": "Yavin 4",
  "habitat": "Mountain caverns",
  "dex": "4D",
  "per": "2D",
  "str": "3D+2",
  "move": "8 (walking), 14 (flying)",
  "size": "1.0–1.5 meters tall, 3.1–3.6 meters long",
  "orneriness": "1D",
  "special": "Brawling parry 5D, dodge 6D. Search 3D+2, sneak 4D. Brawling 6D. Double Attacks: two heads grant two attacks per round at no penalty; each additional attack beyond two adds -1D cumulative to all actions. Jaws: each head does STR+1D damage. Claws: STR+2D damage. Tail Stinger: hooked tail delivers lethal crystallized acid poison — victim takes 3D damage per round until dead; no known natural antidote, though Jedi Force powers (detoxify poison, detoxify another's poison) can neutralize it. Senses: +2D to Perception when looking in two directions simultaneously. Won't attack unless cornered or commanded by a Sith Lord.",
  "source": "WEG40114 The Jedi Academy Sourcebook p.119"
 },
 {
  "name": "Bear Spider",
  "type": "Nocturnal predator",
  "planet": "",
  "habitat": "Forest/Cave",
  "dex": "3D",
  "per": "2D",
  "str": "4D",
  "move": "14",
  "size": "1.5 meters",
  "orneriness": "",
  "special": "Fangs: STR+1D. Claws: STR+1D. Webs: 3D STR.",
  "source": "Twin Star of Kira (page 72)"
 },
 {
  "name": "Beck-tori (Colossal)",
  "type": "Aquatic parasite",
  "planet": "Nam Priax",
  "habitat": "Aquatic",
  "dex": "1D",
  "per": "1D+2",
  "str": "9D+2",
  "move": "10 (swim)/2 (crawl)",
  "size": "Colossal",
  "orneriness": "",
  "special": "Bite: STR+1D+2. Breathe Underwater. Force-Sensitive (Control 3D, Sense 3D+1). Swallow Whole: STR+1D+2 then 3D acid/round. Spikes: STR+1D+2.",
  "source": "Power of the Jedi Sourcebook (pages 77-79)"
 },
 {
  "name": "Beck-tori (Huge)",
  "type": "Aquatic parasite",
  "planet": "Nam Priax",
  "habitat": "Aquatic",
  "dex": "2D",
  "per": "1D+2",
  "str": "5D+2",
  "move": "10 (swim)/2 (crawl)",
  "size": "Huge",
  "orneriness": "",
  "special": "Bite: STR+1D. Breathe Underwater. Force-Sensitive (Control 1D+2, Sense 1D+2). Swallow Whole: STR+1D then 3D acid/round. Spikes: STR+1D.",
  "source": "Power of the Jedi Sourcebook (pages 77-79)"
 },
 {
  "name": "Beck-tori (Medium)",
  "type": "Aquatic parasite",
  "planet": "Nam Priax",
  "habitat": "Aquatic",
  "dex": "3D+1",
  "per": "1D+2",
  "str": "2D",
  "move": "16 (swim)/4 (crawl)",
  "size": "Medium",
  "orneriness": "",
  "special": "Bite: STR+1. Breathe Underwater: +1D+1 swimming. Force-Sensitive. Low-light Vision. Swallow Whole: STR+1 then 3D acid/round. Spikes: STR+1.",
  "source": "Power of the Jedi Sourcebook (pages 77-79)"
 },
 {
  "name": "Beldon",
  "type": "Atmospheric floater",
  "planet": "Bespin",
  "habitat": "Atmosphere",
  "dex": "1D",
  "per": "4D",
  "str": "9D",
  "move": "70 (200 km/h)",
  "size": "800m to 10km",
  "orneriness": "",
  "special": "Electrolocomotion. Electrolocation: 2km scan. Walker Scale.",
  "source": "WEG40119 Galaxy Guide 2: Yavin and Bespin p.53–54; WEG40089 The Movie Trilogy Sourcebook SE p.174"
 },
 {
  "name": "Bergruutfa",
  "type": "Domesticated riding beast",
  "planet": "",
  "habitat": "Plains",
  "dex": "1D",
  "per": "1D+2",
  "str": "6D",
  "move": "15",
  "size": "Up to 7 meters tall",
  "orneriness": "1D",
  "special": "Search 3D+1. Brawling: head butt 7D, lifting 9D. Armored Head: +2D energy/physical to resist damage. Armored Body: +1D energy/physical to resist damage. Head Butt: STR+1D damage, target thrown 1 meter per 2 result points. Drool: Easy DEX or fall in puddle. Creature scale.",
  "source": "WEG40080 Creatures of the Galaxy p.15"
 },
 {
  "name": "Binarian Carnivorous Plant",
  "type": "Carnivorous plant",
  "planet": "Binaros",
  "habitat": "Jungle",
  "dex": "1D",
  "per": "1D",
  "str": "4D",
  "move": "0",
  "size": "3 meters tall",
  "orneriness": "",
  "special": "Tendrils: 6 tendrils, entangle -1D DEX. Bite: 4D+1 acid/2 rounds. Swallow on incapacitated.",
  "source": "DarkStryder Campaign (pages 27-28)"
 },
 {
  "name": "Binarian Sabercat",
  "type": "Jungle predator",
  "planet": "Binaros",
  "habitat": "Jungle",
  "dex": "3D",
  "per": "3D+1",
  "str": "3D+2",
  "move": "12",
  "size": "1m at shoulder, 2-3m long",
  "orneriness": "",
  "special": "Claws: STR+2D, two attacks no penalty. Bite: STR+2D.",
  "source": "DarkStryder Campaign (page 26)"
 },
 {
  "name": "Biodroid",
  "type": "Bio-engineered fighting creature",
  "planet": "Kathol",
  "habitat": "Various",
  "dex": "3D",
  "per": "1D",
  "str": "5D",
  "move": "12",
  "size": "3 meters tall",
  "orneriness": "",
  "special": "Horn-Claws: STR+2D. Bite: STR+1D. Carapace: +2D resist.",
  "source": "DarkStryder Campaign \u2013 Endgame (page 106)"
 },
 {
  "name": "Black Behemoth",
  "type": "Predator",
  "planet": "Veron",
  "habitat": "Forest",
  "dex": "1D+2",
  "per": "1D",
  "str": "5D",
  "move": "9",
  "size": "2m tall, 4m long",
  "orneriness": "",
  "special": "Armor: +1D vs physical. Teeth: STR+2.",
  "source": "Planets Collection (pages 88-89)"
 },
 {
  "name": "Black Bha'lir",
  "type": "Jungle predator",
  "planet": "Iyred",
  "habitat": "Jungle",
  "dex": "5D+1",
  "per": "4D+2",
  "str": "4D",
  "move": "18",
  "size": "0.9-1.0m tall, 1.22m long",
  "orneriness": "",
  "special": "Claws: STR+1D. Teeth: STR+1D. Stealth: +1D sneak.",
  "source": "The Black Sands of Socorro (page 68)"
 },
 {
  "name": "Blastail",
  "type": "Small forest predator",
  "planet": "Kashyyyk",
  "habitat": "Forest",
  "dex": "3D+2",
  "per": "3D+2",
  "str": "2D+1",
  "move": "15",
  "size": "1-1.2m long",
  "orneriness": "",
  "special": "Bite: STR+1D. Claws: STR+2. Disease: Difficult STR or infection (-2 pips STR, then -1D next day). Tail Bulb: blinding 1D+2 rounds, range 4/8/16. Darkvision: 20m.",
  "source": "Ultimate Adversaries (page 70)"
 },
 {
  "name": "Bloodsniffer",
  "type": "Predator",
  "planet": "Kamar",
  "habitat": "Various",
  "dex": "3D",
  "per": "2D",
  "str": "3D",
  "move": "16",
  "size": "1-2.1m long",
  "orneriness": "",
  "special": "Blood Drain: must consume own weight every 2 days. Claws: STR+2. Thorny Spur: STR+1D, wound+ hits major vein (Difficult first aid to stop bleeding).",
  "source": "Han Solo and the Corporate Sector Sourcebook (pages 140-141)"
 },
 {
  "name": "Blossoming Gulper",
  "type": "Huge aquatic predator",
  "planet": "Mon Calamari",
  "habitat": "Aquatic",
  "dex": "1D",
  "per": "2D+2",
  "str": "7D",
  "move": "2 (swimming)",
  "size": "Huge",
  "orneriness": "",
  "special": "Bite: STR+1D+2. Camouflage: +2D+2 hide in underwater plants. Swallow Whole: STR+1D + 3D acid/round. Blindsight. Exceptional Reflexes: +1D initiative.",
  "source": "Geonosis and the Outer Rim Worlds (pages 102-103)"
 },
 {
  "name": "Blue Desert People",
  "type": "Plains lizard",
  "planet": "",
  "habitat": "Desert/Plains",
  "dex": "4D",
  "per": "2D+2",
  "str": "4D+1",
  "move": "30 (90 km/h)",
  "size": "1.6-2.4m long",
  "orneriness": "3D",
  "special": "Night Vision: ignore darkness penalties. Claws: STR+1D.",
  "source": "Cracken's Threat Dossier (page 51)"
 },
 {
  "name": "Blurrg",
  "type": "Plains herd animal",
  "planet": "Endor",
  "habitat": "Forest/Plains",
  "dex": "1D+2",
  "per": "1D",
  "str": "4D+2",
  "move": "6",
  "size": "Large",
  "orneriness": "2D",
  "special": "Bite: STR. Run: Move 10 for short bursts.",
  "source": ""
 },
 {
  "name": "Bogan's Brown Nafen",
  "type": "Aerial insectivorous rodent",
  "planet": "Bogan",
  "habitat": "Various",
  "dex": "5D",
  "per": "4D",
  "str": "1D",
  "move": "30 (flying)/4 (walking)",
  "size": "30cm wingspan",
  "orneriness": "",
  "special": "Venom: <10 bites harmless. Every 10th bite requires Very Easy STR or -1D cumulative penalty on all actions; if penalty exceeds STR, character falls asleep. Cured by medpac or 6-10 hours sleep. Disease: Moderate STR or become wounded after 6-15 hours onset; Easy medicine to cure.",
  "source": "WEG40080 Creatures of the Galaxy p.16"
 },
 {
  "name": "Bogey",
  "type": "Kessel energy being",
  "planet": "",
  "habitat": "Subterranean",
  "dex": "5D",
  "per": "1D",
  "str": "1D",
  "move": "14 (flying)",
  "size": "1 meter wide",
  "orneriness": "",
  "special": "(Energy being \u2014 minimal combat stats)",
  "source": "Jedi Academy Sourcebook (page 120)"
 },
 {
  "name": "Bolstyngar",
  "type": "Forest predator",
  "planet": "Kashyyyk",
  "habitat": "Forest",
  "dex": "1D+2",
  "per": "5D+1",
  "str": "4D+2",
  "move": "8",
  "size": "3.5-4.2m tall",
  "orneriness": "",
  "special": "Bite: STR+1D+1. Auditory Clusters: +1D sound-related PER.",
  "source": ""
 },
 {
  "name": "Boma Beast",
  "type": "Monstrous forest creature",
  "planet": "Onderon",
  "habitat": "Forest",
  "dex": "3D+2",
  "per": "4D",
  "str": "7D",
  "move": "18",
  "size": "2-3m tall",
  "orneriness": "3D",
  "special": "Claws: STR+2D. Spikes: 6D damage to melee attackers.",
  "source": "WEG40082 Tales of the Jedi Companion pp.106-107"
 },
 {
  "name": "Bonegnawer",
  "type": "Airborne predator",
  "planet": "Tatooine",
  "habitat": "Desert",
  "dex": "2D+2",
  "per": "3D",
  "str": "4D+2",
  "move": "4/20 (flying)",
  "size": "8-10m wingspan",
  "orneriness": "",
  "special": "Bite: STR+3D+1. Exceptional Reflexes: +1D initiative. Low Light Vision. Slam Attack: STR+1D+2.",
  "source": "Alien Anthology (pages 23-24)"
 },
 {
  "name": "Boneworm",
  "type": "Tunneling predator",
  "planet": "Necropolis",
  "habitat": "Subterranean",
  "dex": "3D+2",
  "per": "1D",
  "str": "1D",
  "move": "6 (burrow)",
  "size": "cm to 1m long",
  "orneriness": "",
  "special": "Bite/Burrow: STR+3D. Blindsight. Feeds on corpses, attacks helpless.",
  "source": "Alien Anthology (page 25)"
 },
 {
  "name": "Borcatu",
  "type": "Testy scavenger",
  "planet": "",
  "habitat": "Various",
  "dex": "3D",
  "per": "2D",
  "str": "1D+2",
  "move": "11",
  "size": "0.2-0.5m long",
  "orneriness": "",
  "special": "Bite: STR+2. Claws: STR+1. Armored Hide: +2 vs physical. Digging.",
  "source": "WEG40080 Creatures of the Galaxy p.17"
 },
 {
  "name": "Bordok (Greater)",
  "type": "Forest mount",
  "planet": "Forest Moon of Endor",
  "habitat": "Forest",
  "dex": "3D+1",
  "per": "3D+1",
  "str": "5D",
  "move": "28",
  "size": "3m at shoulder",
  "orneriness": "1D",
  "special": "Low Light Vision. Surefooted: +3D climbing in rough terrain.",
  "source": "Alien Anthology (pages 25-26)"
 },
 {
  "name": "Bordok (Lesser)",
  "type": "Forest mount",
  "planet": "Forest Moon of Endor",
  "habitat": "Forest",
  "dex": "4D",
  "per": "3D+1",
  "str": "3D",
  "move": "20",
  "size": "1.5m at shoulder",
  "orneriness": "1D",
  "special": "Low Light Vision. Surefooted: +3D climbing in rough terrain.",
  "source": "Alien Anthology (pages 25-26)"
 },
 {
  "name": "Bouncebeast",
  "type": "Timid herbivore",
  "planet": "Duroon",
  "habitat": "Various",
  "dex": "2D+1",
  "per": "2D",
  "str": "1D+2",
  "move": "5 (bounce)/2 (rolling)",
  "size": "0.3-0.5m diameter",
  "orneriness": "",
  "special": "Danger Sense. Pseudopod Skin: form limbs as needed. Accidental hit: STR+1D.",
  "source": "Han Solo and the Corporate Sector Sourcebook (page 140)"
 },
 {
  "name": "Bulfus",
  "type": "",
  "planet": "",
  "habitat": "Aquatic",
  "dex": "3D+1",
  "per": "1D+1",
  "str": "2D+1",
  "move": "20 (flying)/10 (swim)/6",
  "size": "Medium",
  "orneriness": "",
  "special": "Bite: STR+2. Diving Ram: STR+1D. Exceptional Reflexes: +1D initiative.",
  "source": "Coruscant and the Core Worlds (page 79)"
 },
 {
  "name": "Burning Snake",
  "type": "Serpent",
  "planet": "Yavin 13",
  "habitat": "Desert/Volcanic",
  "dex": "2D+1",
  "per": "2D+2",
  "str": "1D",
  "move": "8",
  "size": "3 meters long",
  "orneriness": "",
  "special": "Heat Radiation: Absorbs solar energy and radiates waves of heat at will — 4D damage to anyone approaching. Multiple burning snakes can combine attacks. Habitat: sunny desert flats (does not seek shade).",
  "source": "WEG40119 Galaxy Guide 2: Yavin and Bespin p.28–29"
 },
 {
  "name": "Butcherbug",
  "type": "Web-using hunter",
  "planet": "Dagobah",
  "habitat": "Swamp",
  "dex": "2D+2",
  "per": "3D",
  "str": "2D",
  "move": "6",
  "size": "0.7m long",
  "orneriness": "",
  "special": "Armor: +2D physical, +2 energy. Microfine Web: VD search to spot, 6D damage on contact (9D if moving fast).",
  "source": "Galaxy Guide 3 (page 63)"
 },
 {
  "name": "C'Oron Bel (Silverjaw)",
  "type": "Fast aquatic predator",
  "planet": "Goroth",
  "habitat": "Aquatic",
  "dex": "2D",
  "per": "3D",
  "str": "2D+2",
  "move": "18 (swim)/6",
  "size": "1.5-1.8m long",
  "orneriness": "",
  "special": "Jaws: STR+2D. Charge: Move 18 max 2m from water. Armor: +1D physical (metal scales).",
  "source": "Goroth, Slave of the Empire (pages 93-94)"
 },
 {
  "name": "Cairnmog",
  "type": "",
  "planet": "",
  "habitat": "Plains",
  "dex": "3D+1",
  "per": "2D",
  "str": "4D",
  "move": "18",
  "size": "Large",
  "orneriness": "",
  "special": "Armor: +2D physical, +1D energy. Gore: STR+1. Kick: STR+2. Trample: STR+2.",
  "source": "Coruscant and the Core Worlds (page 68)"
 },
 {
  "name": "Camray Eel",
  "type": "Sea predator",
  "planet": "",
  "habitat": "Aquatic",
  "dex": "4D",
  "per": "2D",
  "str": "3D",
  "move": "16",
  "size": "5 meters long",
  "orneriness": "",
  "special": "Coils: 5D constriction. Teeth: STR+2D. Camouflage: +2D sneak near seabed.",
  "source": "Adventure Journal Issues 1-4 (page 70)"
 },
 {
  "name": "Caniphant",
  "type": "Guard animal",
  "planet": "",
  "habitat": "Various",
  "dex": "3D",
  "per": "3D",
  "str": "2D",
  "move": "14",
  "size": "0.75m high, 1.2m long",
  "orneriness": "",
  "special": "Bite: STR+1.",
  "source": "Game Chambers of Questal (page 11)"
 },
 {
  "name": "Capture Beast",
  "type": "",
  "planet": "",
  "habitat": "Various",
  "dex": "2D",
  "per": "2D",
  "str": "14D+2 (walker)",
  "move": "8",
  "size": "200 meters long",
  "orneriness": "",
  "special": "Enclose: forms living wall. Force Immunity: immune to Sense/Alter. Legs: 100 pairs, 7D+1 STR grapple. Walker Scale.",
  "source": "New Jedi Order Sourcebook (pages 87-88)"
 },
 {
  "name": "Casting Hawk",
  "type": "",
  "planet": "",
  "habitat": "Forest",
  "dex": "2D+1",
  "per": "2D+2",
  "str": "2D+1",
  "move": "16 (fly)/2",
  "size": "1.5-1.8m wingspan",
  "orneriness": "",
  "special": "Beak: STR+1. Excellent Vision: +2 pip search. Exceptional Reflexes: +1D initiative. Sap Attack: Difficult STR to break, -1D attack, -2D DEX, half move. Talons: STR.",
  "source": "Coruscant and the Core Worlds (page 132)"
 },
 {
  "name": "Cavehopper",
  "type": "Subterranean predator",
  "planet": "",
  "habitat": "Subterranean",
  "dex": "6D+2",
  "per": "3D",
  "str": "3D+1",
  "move": "12",
  "size": "Medium",
  "orneriness": "",
  "special": "Bite: STR+1D. Darkvision: 20m. Exceptional Reflexes: +1D initiative. Kick: STR+1D+2 (twice/round). Thick Hide: +2 pip vs physical.",
  "source": ""
 },
 {
  "name": "Cherfer",
  "type": "Vicious herd beast",
  "planet": "Elom",
  "habitat": "Plains",
  "dex": "3D",
  "per": "2D",
  "str": "4D",
  "move": "14",
  "size": "1-1.5m at shoulder",
  "orneriness": "",
  "special": "Snout: STR+1D or dig. Hooves: STR+1D.",
  "source": "WEG40080 Creatures of the Galaxy p.19"
 },
 {
  "name": "Chiilak",
  "type": "Aquatic mammal",
  "planet": "Moon of Misnor",
  "habitat": "Aquatic/Arctic",
  "dex": "1D",
  "per": "2D",
  "str": "4D+2",
  "move": "9/18 (swimming)",
  "size": "2.2m tall",
  "orneriness": "",
  "special": "Dodge 4D, brawling parry 6D. Search 4D, tracking 4D. Brawling 5D, climbing/jumping 5D, swimming 6D. Claws: STR+1D damage. Stamina: hold breath 20 min, swim for hours. Creature scale.",
  "source": "WEG40080 Creatures of the Galaxy p.20"
 },
 {
  "name": "Chiru (Sand Wasp)",
  "type": "Swarming insect",
  "planet": "Socorro",
  "habitat": "Desert",
  "dex": "4D+1",
  "per": "2D+2",
  "str": "1D",
  "move": "20",
  "size": "2.54 centimeters",
  "orneriness": "",
  "special": "Sting: 3D poison, stunned = -1D for 1D rounds (cumulative).",
  "source": "Black Sands of Socorro (pages 23-24)"
 },
 {
  "name": "Choarn",
  "type": "Ocean scavenger",
  "planet": "Mon Calamari",
  "habitat": "Aquatic",
  "dex": "3D",
  "per": "2D+1",
  "str": "2D-6D",
  "move": "2-6",
  "size": "1-8 meters",
  "orneriness": "",
  "special": "(Variable stats by size)",
  "source": "Death in the Undercity (page 55)"
 },
 {
  "name": "Chrysalide Rancor",
  "type": "",
  "planet": "",
  "habitat": "Various",
  "dex": "2D+2",
  "per": "1D+1",
  "str": "10D",
  "move": "30",
  "size": "5m tall",
  "orneriness": "",
  "special": "Claws: STR+1D+2. Low Light Vision. Natural Armor: +2D+1 physical and energy. Teeth: STR+1D+1. Terrifying Presence: Difficult Willpower/Control or flee/attack (fail 10+ = cower).",
  "source": "Dark Side Sourcebook (pages 120-121)"
 },
 {
  "name": "Cidwen",
  "type": "Aggressive avian",
  "planet": "Atraken",
  "habitat": "Various",
  "dex": "5D",
  "per": "4D",
  "str": "1D+1",
  "move": "30 (flying)/22 (gliding)",
  "size": "0.5m long, 1m wingspan",
  "orneriness": "",
  "special": "Beak: STR+2D. Claws: STR+2D+2.",
  "source": "Planets Collection (page 200)"
 },
 {
  "name": "Claw Vine",
  "type": "Medium forest scavenger",
  "planet": "Kashyyyk",
  "habitat": "Forest",
  "dex": "2D",
  "per": "1D+1",
  "str": "4D",
  "move": "0",
  "size": "2 meters",
  "orneriness": "",
  "special": "Blindsight: 4m range. Exceptional Reflexes: +1D initiative.",
  "source": "Ultimate Adversaries (page 74)"
 },
 {
  "name": "Clawbird",
  "type": "Avian predator",
  "planet": "",
  "habitat": "Forest",
  "dex": "1D",
  "per": "1D",
  "str": "1D",
  "move": "15 (flying)",
  "size": "10cm long",
  "orneriness": "",
  "special": "Poison Spines: STR+1 + slow poison (3D paralysis, +1D/hour to max 6D). Medpac counteracts.",
  "source": "Thrawn Trilogy Sourcebook (pages 155-156)"
 },
 {
  "name": "Clawcat",
  "type": "Cunning mountain predator",
  "planet": "Goroth",
  "habitat": "Mountain",
  "dex": "2D",
  "per": "3D",
  "str": "2D",
  "move": "16",
  "size": "0.8-1.0m tall, 2.5m long",
  "orneriness": "6D",
  "special": "Paws: STR+2. Jaws: STR+1D. Hunting Cry: 2D+2 stun in 10m (ignores armor, blast helmet +1, sealed +1D).",
  "source": "Goroth, Slave of the Empire (pages 92-93)"
 },
 {
  "name": "Cliff Wampa",
  "type": "",
  "planet": "Hoth",
  "habitat": "Arctic/Mountain",
  "dex": "3D+2",
  "per": "4D",
  "str": "7D",
  "move": "12/10 (climbing)",
  "size": "3.5m tall",
  "orneriness": "",
  "special": "Claws: STR+2D+1. Bite: STR+1D+2. Climbing: +2D. Camouflage: +2D sneak in snow. Cold Resistant.",
  "source": "Galaxy Guide 3 (page 66)"
 },
 {
  "name": "Cliffborer Worm",
  "type": "",
  "planet": "",
  "habitat": "Mountain/Subterranean",
  "dex": "3D",
  "per": "2D+1",
  "str": "2D+2",
  "move": "10/6 (burrow)",
  "size": "10m long",
  "orneriness": "",
  "special": "Armored Hide: +4D physical, +2D energy. Bite: STR+4D. Bore: through any material. Claw: STR+1D.",
  "source": "Alien Anthology (pages 26-27)"
 },
 {
  "name": "Clodhopper",
  "type": "",
  "planet": "",
  "habitat": "Swamp",
  "dex": "5D",
  "per": "1D",
  "str": "2D",
  "move": "30",
  "size": "1m long",
  "orneriness": "",
  "special": "Pesticide Resistance: +5D after first exposure. Swarm Attack: 1D + 1 pip per 20 bugs.",
  "source": "Secrets of Naboo (page 54)"
 },
 {
  "name": "Colo Claw Fish",
  "type": "",
  "planet": "Naboo",
  "habitat": "Aquatic",
  "dex": "4D+1",
  "per": "1D",
  "str": "5D (walker)",
  "move": "70 (200 km/h)",
  "size": "40m long",
  "orneriness": "",
  "special": "Bite: STR+4D (walker). Claws: STR+2D (walker). Venom: Moderate STR or paralyzed 36hr minus 1hr/die STR.",
  "source": "Secrets of Naboo (page 55)"
 },
 {
  "name": "Colossus Wasp",
  "type": "Giant insect",
  "planet": "Ithull",
  "habitat": "Space",
  "dex": "2D",
  "per": "4D",
  "str": "7D",
  "move": "1 (space)",
  "size": "1-1.5 km long",
  "orneriness": "",
  "special": "Space Survival. Starfighter Scale.",
  "source": "WEG40082 Tales of the Jedi Companion p.107"
 },
 {
  "name": "Condor Dragon",
  "type": "Large flying reptavian predator",
  "planet": "Moon of Endor",
  "habitat": "Forest",
  "dex": "3D+1",
  "per": "2D",
  "str": "4D",
  "move": "2/15 (flying)",
  "size": "7-8m wingspan",
  "orneriness": "",
  "special": "Low Light Vision. Bite: STR+1D+1. Claws: STR+1D.",
  "source": ""
 },
 {
  "name": "Cooha",
  "type": "Avian predator",
  "planet": "Gandeid IV",
  "habitat": "Various",
  "dex": "2D",
  "per": "1D",
  "str": "2D+2",
  "move": "18/9 (standard gravity)",
  "size": "1.5m tall",
  "orneriness": "",
  "special": "Beak: STR+2. Claws: STR+1D.",
  "source": "Truce at Bakura Sourcebook (page 102)"
 },
 {
  "name": "Corellian Sand Panther",
  "type": "",
  "planet": "Corellia",
  "habitat": "Plains/Desert",
  "dex": "3D+2",
  "per": "3D",
  "str": "4D+1",
  "move": "16",
  "size": "~2m long",
  "orneriness": "",
  "special": "Bite: STR+1D. Claws: STR+2 (both no penalty). Poison: Moderate STR or 7D venom. Scent tracking.",
  "source": "Alien Anthology (pages 27-28)"
 },
 {
  "name": "Coromon Headhunter",
  "type": "",
  "planet": "",
  "habitat": "Forest",
  "dex": "5D",
  "per": "2D+2",
  "str": "4D+2",
  "move": "16",
  "size": "Medium",
  "orneriness": "",
  "special": "Dew Claw Attack: called shot to neck, STR+1D, MW/Kill = decapitation. Exceptional Reflexes: +1D initiative. Foreclaw: STR+1.",
  "source": "Coruscant and the Core Worlds (page 99)"
 },
 {
  "name": "Corrupted Snake",
  "type": "Mutated Sith predator",
  "planet": "",
  "habitat": "Various",
  "dex": "2D+2",
  "per": "2D+2",
  "str": "7D+2",
  "move": "8",
  "size": "10m long",
  "orneriness": "",
  "special": "Bite: STR+1D+2. Constrict: STR/round. Exceptional Reflexes: +1D initiative. Poisonous: Sith-type venom.",
  "source": ""
 },
 {
  "name": "Coruscani Ogre",
  "type": "Undercity denizen",
  "planet": "Coruscant",
  "habitat": "Urban/Subterranean",
  "dex": "2D",
  "per": "3D",
  "str": "5D",
  "move": "11",
  "size": "",
  "orneriness": "",
  "special": "Smell: no darkness penalties for PER. Equipment: chains, club (STR+2D). Character Points: 2.",
  "source": "Alien Anthology (pages 28-29)"
 },
 {
  "name": "Cracian Thumper",
  "type": "",
  "planet": "Craci",
  "habitat": "Plains",
  "dex": "3D",
  "per": "3D+2",
  "str": "3D",
  "move": "12",
  "size": "1-1.8m at shoulder",
  "orneriness": "1D",
  "special": "Claws: STR+1. Tail: STR+1D+2. Silent Movement: +1D+2 sneak. Carries 110kg cargo or rider + 50kg.",
  "source": "Core Rulebook (page 222)"
 },
 {
  "name": "Crawlfish",
  "type": "Swamp crustacean",
  "planet": "Yavin IV",
  "habitat": "Swamp",
  "dex": "3D",
  "per": "1D",
  "str": "1D",
  "move": "3",
  "size": "60-140cm long",
  "orneriness": "",
  "special": "Slipperiness: +1D dodge.",
  "source": "Galaxy Guide 12 (page 19)"
 },
 {
  "name": "Crimson Slug",
  "type": "Spacefaring predator",
  "planet": "",
  "habitat": "Space",
  "dex": "2D",
  "per": "1D",
  "str": "5D",
  "move": "6",
  "size": "6-8m long",
  "orneriness": "",
  "special": "Vacuum survival. Teeth: STR+1D.",
  "source": "Tapani Sector Instant Adventures (page 72)"
 },
 {
  "name": "Croator",
  "type": "Swamp avian",
  "planet": "Wyndigal 2",
  "habitat": "Swamp",
  "dex": "2D",
  "per": "2D",
  "str": "1D+2",
  "move": "15 (running), 10 (swimming)",
  "size": "2.5 meters tall at the shoulder",
  "orneriness": "",
  "special": "Reflective Plumage and Hood: shiny membranes reflect UV rays. Off-worlders can add +2D to survival rolls to withstand Wyndigal 2 climate.",
  "source": "WEG40080 Creatures of the Galaxy p.21"
 },
 {
  "name": "Crown of Silk",
  "type": "Tiny Force-using airborne parasite",
  "planet": "Cerea",
  "habitat": "Forest",
  "dex": "5D",
  "per": "1D+2",
  "str": "1D",
  "move": "4 (flying)",
  "size": "80cm diameter",
  "orneriness": "",
  "special": "Blindsight: 20m. Camouflage: +2 hide in low light. Poison: Moderate stamina or -1D STR then stunned 1D hrs. Parasitic Attack: drains PER. Force-Sensitive (Control 2D+1, Sense 3D, Alter 1D+2).",
  "source": "Ultimate Adversaries (pages 77-78)"
 },
 {
  "name": "Crynoid",
  "type": "Poisonous arachnid",
  "planet": "Neftali",
  "habitat": "Arctic/Mountain",
  "dex": "4D",
  "per": "1D",
  "str": "+1",
  "move": "8",
  "size": "3cm long",
  "orneriness": "",
  "special": "Poisonous Bite: 4D damage every 5 min for 1 hour. Difficult stamina or -3D all actions 6 hours.",
  "source": "Wretched Hives (page 17)"
 },
 {
  "name": "Cthon",
  "type": "",
  "planet": "",
  "habitat": "Urban/Subterranean",
  "dex": "1D+2",
  "per": "2D+1",
  "str": "5D+1",
  "move": "10",
  "size": "Large",
  "orneriness": "",
  "special": "Blindsight: 10m hearing. Slam Attack: STR damage.",
  "source": "Coruscant and the Core Worlds (page 35)"
 },
 {
  "name": "D'oemir Bear",
  "type": "Arctic predator",
  "planet": "Neftali",
  "habitat": "Arctic",
  "dex": "2D",
  "per": "2D+1",
  "str": "6D",
  "move": "10",
  "size": "1.3m (fours)/1.98m (standing)",
  "orneriness": "",
  "special": "Hibernation: up to 1 year. Claws: STR+1D. Bite: STR+2.",
  "source": "Black Sands of Socorro (page 99)"
 },
 {
  "name": "Danchaf (Tree Goblin)",
  "type": "Organized predator",
  "planet": "Garban",
  "habitat": "Forest",
  "dex": "4D",
  "per": "4D",
  "str": "2D+1",
  "move": "8",
  "size": "1.5-2m tall",
  "orneriness": "",
  "special": "Brawling 4D, climbing/jumping 4D. Claws: STR+1D damage. Climbing: +1D to climbing rolls. Stealth: PER rolls to spot are one difficulty higher in trees/bushes. Senses: +1D+2 to search, search: tracking, and PER rolls. Larger goblins have STR 3D+1.",
  "source": "WEG40080 Creatures of the Galaxy p.23"
 },
 {
  "name": "Dark Wolf",
  "type": "Medium desert predator",
  "planet": "Sriluur",
  "habitat": "Desert",
  "dex": "4D+2",
  "per": "3D+2",
  "str": "4D+2",
  "move": "16",
  "size": "Medium",
  "orneriness": "",
  "special": "Stinger Tail: STR, Moderate stamina or -1D STR. Claws: STR+2, Moderate stamina or paralyzed 1D hrs. Bite: STR+1. Night Hunter: +1D tracking at night. Darkvision: 20m. Exceptional Reflexes: +1D initiative.",
  "source": "Geonosis and the Outer Rim Worlds (page 131)"
 },
 {
  "name": "Derkolo",
  "type": "Small pack hunter",
  "planet": "Sileron",
  "habitat": "Various",
  "dex": "3D+2",
  "per": "1D",
  "str": "3D",
  "move": "20",
  "size": "1.5m long",
  "orneriness": "",
  "special": "Armor: +1D vs physical. Claws: STR+1D+2.",
  "source": "Planets Collection (page 90)"
 },
 {
  "name": "Dewback",
  "type": "Desert mount",
  "planet": "Tatooine",
  "habitat": "Desert",
  "dex": "3D",
  "per": "2D",
  "str": "4D",
  "move": "35 (day)/7 (night)",
  "size": "1.3-2.1m at shoulder",
  "orneriness": "3D",
  "special": "(Standard mount \u2014 slows dramatically at night)",
  "source": "Star Wars Sourcebook (pages 87-88)"
 },
 {
  "name": "Dhislug",
  "type": "Predatory slug",
  "planet": "Algara II",
  "habitat": "Swamp",
  "dex": "3D",
  "per": "2D",
  "str": "5D",
  "move": "3",
  "size": "3m long",
  "orneriness": "",
  "special": "Armor: +1D STR vs physical. Constriction: STR+2D. Acid: 5D.",
  "source": "Planets Collection (page 192)"
 },
 {
  "name": "Dianoga",
  "type": "Scavenger",
  "planet": "",
  "habitat": "Aquatic/Urban",
  "dex": "2D",
  "per": "3D+1",
  "str": "6D",
  "move": "3",
  "size": "Various",
  "orneriness": "",
  "special": "Colour change: +4D sneak. Tentacles: opposed STR to escape.",
  "source": "Galaxy Guide 1 (page 57)"
 },
 {
  "name": "Diathim",
  "type": "Large vacuum herd animal",
  "planet": "Moons of Iego",
  "habitat": "Space",
  "dex": "5D+1",
  "per": "5D",
  "str": "4D+1",
  "move": "22 (flying)",
  "size": "Large",
  "orneriness": "",
  "special": "Exceptional Reflexes: +1D initiative. Blindsight. Energy Pulse: 7D energy, range 3-75/200/500m (every 3 rounds). Vacuum Creature. Force-Sensitive (Control 2D+2, Alter 2D+2, Sense 2D).",
  "source": "Geonosis and the Outer Rim Worlds (page 72)"
 },
 {
  "name": "Diehard",
  "type": "Hardy insect",
  "planet": "Atraken",
  "habitat": "Various",
  "dex": "5D",
  "per": "1D",
  "str": "4D",
  "move": "24 (flying)",
  "size": "0.5m long",
  "orneriness": "",
  "special": "Mandibles: STR+2D. Armor: +2D vs energy.",
  "source": "Planets Collection (page 201)"
 },
 {
  "name": "Dimlurker",
  "type": "",
  "planet": "",
  "habitat": "Subterranean",
  "dex": "3D",
  "per": "3D",
  "str": "3D+1",
  "move": "12/12 (climb)",
  "size": "Large",
  "orneriness": "",
  "special": "Claws: STR+1D. Crushing Attack: STR+2D (grapple hug). Darkvision: 20m (eyeless). Stench. Terrifying Presence: opposed Control/Willpower vs PER or flee/attack.",
  "source": "Coruscant and the Core Worlds (page 155)"
 },
 {
  "name": "Dinko",
  "type": "Scavenger",
  "planet": "Proxima Dibal",
  "habitat": "Various",
  "dex": "3D+1",
  "per": "2D+1",
  "str": "1D+2",
  "move": "",
  "size": "",
  "orneriness": "",
  "special": "Stink: offensive scent, weeks to remove. Bite: STR + 3D toxin (opposed STR or nausea).",
  "source": "Han Solo and the Corporate Sector Sourcebook (page 141)"
 },
 {
  "name": "Divto",
  "type": "Predatory serpent",
  "planet": "",
  "habitat": "Forest",
  "dex": "3D+2",
  "per": "3D+2",
  "str": "4D",
  "move": "7",
  "size": "3m long",
  "orneriness": "",
  "special": "Teeth: 5D. Multiple Attacks: 3 per round (one per head) no penalty. Poison: 6D+2 stun (3 consecutive rounds).",
  "source": "Alien Anthology (pages 28-29)"
 },
 {
  "name": "Doashim",
  "type": "Medium predator",
  "planet": "Ryloth",
  "habitat": "Mountain/Cave",
  "dex": "3D+2",
  "per": "4D",
  "str": "4D+1",
  "move": "10",
  "size": "Medium",
  "orneriness": "",
  "special": "Camouflage: +1D hide in mountains. Thick Hide: +1 vs physical. Claws and Bite: STR+1D+1 each. Darkvision: 20m. Multiattack: 2 brawling no penalty. Terrifying Presence: intimidation 5D.",
  "source": "Ultimate Adversaries (pages 80-81)"
 },
 {
  "name": "Dochinie",
  "type": "Airborne predator",
  "planet": "Mina",
  "habitat": "Various",
  "dex": "3D+2",
  "per": "1D+1",
  "str": "2D",
  "move": "15/25 (flight)",
  "size": "Medium",
  "orneriness": "",
  "special": "Bite: STR+1.",
  "source": ""
 },
 {
  "name": "Draagax",
  "type": "Crazed nocturnal omnivore",
  "planet": "Relkass",
  "habitat": "Forest",
  "dex": "4D",
  "per": "3D",
  "str": "4D",
  "move": "12",
  "size": "1.6-2.0m at shoulder",
  "orneriness": "",
  "special": "Dodge 5D, running 6D. Sneak 5D. Brawling 5D, climbing/jumping 5D. Enhanced Speed: after ingesting Relkass sentinel plant, berserker state for several hours, Move increases to 28. Infrared Vision: no penalties in darkness. Poisoned Fangs: paralytic poison, Moderate stamina or immediate incapacitation, +1 difficulty level per successive bite. Creature scale.",
  "source": "WEG40080 Creatures of the Galaxy p.24"
 },
 {
  "name": "Dragonsnake",
  "type": "Underwater predator",
  "planet": "Dagobah",
  "habitat": "Swamp",
  "dex": "2D",
  "per": "2D",
  "str": "5D",
  "move": "5 (swim)",
  "size": "4m long",
  "orneriness": "",
  "special": "Bite: STR+2D (double attack roll = swallowed). Razor Fins: STR+3D. Constrictor Coils: STR/round until incapacitated.",
  "source": "Galaxy Guide 3 (page 63)"
 },
 {
  "name": "Draz",
  "type": "Canine hunter",
  "planet": "Korbin",
  "habitat": "Various",
  "dex": "4D",
  "per": "3D",
  "str": "2D",
  "move": "10",
  "size": "1.5m long",
  "orneriness": "",
  "special": "Teeth: STR+2D. Claws: STR+1D.",
  "source": "Planets Collection (page 244)"
 },
 {
  "name": "Dread Weapon",
  "type": "",
  "planet": "",
  "habitat": "Space",
  "dex": "0D+2",
  "per": "1D",
  "str": "10D+2 (SF scale)",
  "move": "20",
  "size": "2,000m long",
  "orneriness": "",
  "special": "Vacuum survival. Inhalation: Difficult DEX or drawn 20m closer (50m radius). Starfighter Scale.",
  "source": "New Jedi Order Sourcebook (pages 85-86)"
 },
 {
  "name": "Droch",
  "type": "",
  "planet": "",
  "habitat": "Various",
  "dex": "5D+1",
  "per": "1D+2",
  "str": "0D+1",
  "move": "6",
  "size": "Diminutive",
  "orneriness": "",
  "special": "Affected by Light: DEX drops to 2D. Bite: STR+1. Death Seed Plague: Moderate STR or infected, lose 1D STR/day until cured or dead.",
  "source": "Power of the Jedi Sourcebook (page 78)"
 },
 {
  "name": "Droidbreaker",
  "type": "",
  "planet": "",
  "habitat": "Various",
  "dex": "2D",
  "per": "2D+2",
  "str": "4D+1",
  "move": "12",
  "size": "5m at shoulder",
  "orneriness": "",
  "special": "Acid Resistance: +3D. Armor: +2D physical, +1D energy. Energy Sense: Moderate search, 200m. Exceptional Reflexes: +1D initiative. Low-light Vision. Metal Sense: 10m. Poison Resistance: +2D. Proboscis: STR+2D.",
  "source": "Coruscant and the Core Worlds (page 54)"
 },
 {
  "name": "Druyza",
  "type": "Domesticated laborer",
  "planet": "Socorro",
  "habitat": "Desert",
  "dex": "2D",
  "per": "2D",
  "str": "7D",
  "move": "6",
  "size": "1.5m tall, 2.44m long",
  "orneriness": "1D+2",
  "special": "Head-Butt: STR. Teeth: 3D. Kick: STR+1D. Swimming: hold breath 1 hour.",
  "source": "Black Sands of Socorro (pages 21-22)"
 },
 {
  "name": "Durgolosk",
  "type": "Huge predator",
  "planet": "Barab I",
  "habitat": "Desert",
  "dex": "3D+1",
  "per": "3D",
  "str": "8D+1",
  "move": "15",
  "size": "Huge",
  "orneriness": "",
  "special": "Bite: STR+1D+2. Poisonous Tentacles: 3D + VD stamina or -1D then -2D DEX. Trample: STR+2D+1 (Heroic dodge = STR only). Darkvision: 20m. Low-light Vision.",
  "source": "Geonosis and the Outer Rim Worlds (pages 19-20)"
 },
 {
  "name": "Dxun Tomb Beast",
  "type": "",
  "planet": "Dxun",
  "habitat": "Jungle/Cave",
  "dex": "4D",
  "per": "2D+2",
  "str": "7D+1",
  "move": "10/12 (climb)",
  "size": "Huge",
  "orneriness": "",
  "special": "Claws: STR+1D+2. Exceptional Reflexes: +1D initiative. Horn: STR+1D+1 + poison (Difficult STR or 8D poison). Low Light Vision.",
  "source": "Dark Side Sourcebook (pages 112-113)"
 },
 {
  "name": "Eater",
  "type": "Huge predator",
  "planet": "Masterhome",
  "habitat": "Various",
  "dex": "3D",
  "per": "1D",
  "str": "3D",
  "move": "9",
  "size": "3m tall",
  "orneriness": "6D",
  "special": "Bite: STR+4D. Armor: +4D vs physical and energy.",
  "source": "WEG40108 Classic Campaigns p.38"
 },
 {
  "name": "Edan Tiger",
  "type": "",
  "planet": "",
  "habitat": "Forest",
  "dex": "3D",
  "per": "2D",
  "str": "4D",
  "move": "15",
  "size": "",
  "orneriness": "",
  "special": "Claws: 3D. Bite: 4D.",
  "source": "Introductory Adventure Game (page 6)"
 },
 {
  "name": "Endor Pony",
  "type": "Small mount",
  "planet": "Moon of Endor",
  "habitat": "Forest",
  "dex": "3D",
  "per": "2D+2",
  "str": "3D+1",
  "move": "16",
  "size": "1m at shoulder",
  "orneriness": "1D",
  "special": "Kick: STR+2.",
  "source": ""
 },
 {
  "name": "Eopie",
  "type": "Transport beast",
  "planet": "Naboo",
  "habitat": "Plains",
  "dex": "1D+1",
  "per": "1D+1",
  "str": "4D+1",
  "move": "6",
  "size": "2m tall",
  "orneriness": "2D",
  "special": "Bite: STR+2. Orneriness drops 1D per 10 years age (min 0D).",
  "source": ""
 },
 {
  "name": "Eriaduan Rat",
  "type": "Tiny vermin",
  "planet": "Eriadu",
  "habitat": "Urban/Various",
  "dex": "5D",
  "per": "3D",
  "str": "1D",
  "move": "4/2 (climbing)",
  "size": "Tiny",
  "orneriness": "",
  "special": "Poison: Easy stamina or 3D damage. Exceptional Reflexes: +1D initiative.",
  "source": "Geonosis and the Outer Rim Worlds (page 50)"
 },
 {
  "name": "Falumpaset",
  "type": "",
  "planet": "Naboo",
  "habitat": "Plains",
  "dex": "2D+2",
  "per": "1D",
  "str": "6D",
  "move": "30",
  "size": "3m high",
  "orneriness": "2D+1",
  "special": "Headbutt: STR+1D.",
  "source": "Secrets of Naboo (page 58)"
 },
 {
  "name": "Fambaa",
  "type": "",
  "planet": "Naboo",
  "habitat": "Swamp/Plains",
  "dex": "1D",
  "per": "1D",
  "str": "6D+2",
  "move": "20",
  "size": "5m high",
  "orneriness": "1D",
  "special": "Bite: STR+1D+1. Claw: STR+1D.",
  "source": "Secrets of Naboo (page 58)"
 },
 {
  "name": "Fear Moss",
  "type": "Large Force-using predator",
  "planet": "",
  "habitat": "Swamp/Forest",
  "dex": "1D",
  "per": "2D+2",
  "str": "5D",
  "move": "4/4 (climbing)",
  "size": "Large",
  "orneriness": "",
  "special": "Acidic Touch: 2D physical. Poison: VD stamina or panic then -1 DEX. Blindsight: 10m. Forcefear: Difficult willpower or call on dark side. Sense Force: 35m. +2D hide/sneak in swamp/forest. Force-Sensitive (Control 2D, Sense 2D+1, Alter 2D).",
  "source": "Ultimate Adversaries (pages 83-84)"
 },
 {
  "name": "Feeder",
  "type": "Mobile plant",
  "planet": "",
  "habitat": "Various",
  "dex": "2D+2",
  "per": "1D",
  "str": "1D+1",
  "move": "15",
  "size": "0.5m tall",
  "orneriness": "",
  "special": "Proboscis: STR+1D, then 1D blood drain/turn. Brawling: +1D per additional feeder attacking (max 5).",
  "source": "DarkStryder Campaign (pages 35-36)"
 },
 {
  "name": "Fenner's Rock",
  "type": "Algae eater",
  "planet": "",
  "habitat": "Various",
  "dex": "0D",
  "per": "3D",
  "str": "+2",
  "move": "1",
  "size": "25cm across",
  "orneriness": "",
  "special": "Camouflage: +2D hide. Screech: scare predators.",
  "source": "WEG40080 Creatures of the Galaxy p.25"
 },
 {
  "name": "Firaxan Shark",
  "type": "Aquatic predator",
  "planet": "Manaan",
  "habitat": "Aquatic",
  "dex": "3D+2",
  "per": "3D+2",
  "str": "5D",
  "move": "16 (swimming)",
  "size": "Large",
  "orneriness": "",
  "special": "Thick Hide: +1 STR to resist. Bite: STR+1D+1.",
  "source": ""
 },
 {
  "name": "Fire Breather",
  "type": "",
  "planet": "",
  "habitat": "Various",
  "dex": "1D",
  "per": "1D+2",
  "str": "8D+2 (walker)",
  "move": "6",
  "size": "30m tall",
  "orneriness": "",
  "special": "Anti-Laser Aerosol: +3D vs laser/blaster. Fire Breath: 5D walker damage, 30m cone (every 1D rounds). Force Immunity: vs Sense/Alter. Slam: STR+3D+2 (walker). Walker Scale.",
  "source": "New Jedi Order Sourcebook (page 86)"
 },
 {
  "name": "Firehead",
  "type": "",
  "planet": "",
  "habitat": "Aquatic/Arctic",
  "dex": "4D+2",
  "per": "2D+1",
  "str": "1D",
  "move": "20 (swim)/4/2 (ice burrow)",
  "size": "Tiny",
  "orneriness": "",
  "special": "Amphibious. Darkvision: 20m. Heat Attack: 3D+1. Scent: +1D smell search.",
  "source": "Coruscant and the Core Worlds (page 140)"
 },
 {
  "name": "Flailer",
  "type": "",
  "planet": "Mon Calamari",
  "habitat": "Aquatic",
  "dex": "2D+2",
  "per": "4D",
  "str": "2D",
  "move": "2",
  "size": "2m radius",
  "orneriness": "",
  "special": "(Minimal combat stats)",
  "source": "Death in the Undercity (page 55)"
 },
 {
  "name": "Floater Shark",
  "type": "Floating predator",
  "planet": "Yavin",
  "habitat": "Atmosphere",
  "dex": "3D",
  "per": "2D",
  "str": "6D",
  "move": "175 (500 km/h)",
  "size": "30-50m",
  "orneriness": "",
  "special": "Starfighter Scale.",
  "source": "Galaxy Guide 2 (page 11)"
 },
 {
  "name": "Floater Squid",
  "type": "Floater predator",
  "planet": "Yavin",
  "habitat": "Atmosphere",
  "dex": "2D",
  "per": "2D",
  "str": "6D",
  "move": "105 (300 km/h)",
  "size": "300m",
  "orneriness": "",
  "special": "Tentacles: STR+1D (10-50 tentacles). Starfighter Scale.",
  "source": "Galaxy Guide 2 (page 11)"
 },
 {
  "name": "Floating Glob",
  "type": "Waste removal unit",
  "planet": "Otherspace",
  "habitat": "Various",
  "dex": "3D",
  "per": "2D",
  "str": "3D",
  "move": "6 (floating)",
  "size": "1.5m diameter",
  "orneriness": "",
  "special": "Tendril-Maces: 4 attacks no penalty, all 4 hit = entangled and dragged. Acid Sac: 6D+1.",
  "source": "Otherspace (page 30)"
 },
 {
  "name": "Flutterer",
  "type": "Small fish",
  "planet": "Marca",
  "habitat": "Aquatic",
  "dex": "3D",
  "per": "2D",
  "str": "1D",
  "move": "8",
  "size": "6cm long",
  "orneriness": "",
  "special": "(Minimal stats)",
  "source": "Planet of the Mists (page 14)"
 },
 {
  "name": "Forntarch",
  "type": "Aggressive rodent",
  "planet": "Gorsh",
  "habitat": "Various",
  "dex": "2D",
  "per": "3D",
  "str": "2D",
  "move": "4 (crawl)/17 (leaping)",
  "size": "3m long",
  "orneriness": "",
  "special": "Slicers: STR+2D+2. Teeth: STR+1D.",
  "source": "Planets Collection (page 56)"
 },
 {
  "name": "Fwit",
  "type": "Tiny predator",
  "planet": "Maridun",
  "habitat": "Plains",
  "dex": "3D+1",
  "per": "2D+2",
  "str": "2D+1",
  "move": "10",
  "size": "<0.5m tall",
  "orneriness": "",
  "special": "Bite: STR+2. Attach: auto bite damage each round until unconscious. Difficult lifting to remove.",
  "source": "Ultimate Adversaries (page 85)"
 },
 {
  "name": "Gallaze",
  "type": "Herbivore",
  "planet": "Trinta",
  "habitat": "Forest",
  "dex": "3D",
  "per": "2D+1",
  "str": "2D",
  "move": "10",
  "size": "1m tall",
  "orneriness": "",
  "special": "(Harmless herbivore)",
  "source": "Domain of Evil (page 23)"
 },
 {
  "name": "Galub Sea Slug",
  "type": "Sea parasite",
  "planet": "Spira",
  "habitat": "Aquatic",
  "dex": "2D",
  "per": "",
  "str": "3D",
  "move": "8 (swimming)",
  "size": "2m long",
  "orneriness": "",
  "special": "Adhesion: suction to surfaces.",
  "source": "Adventure Journals 1-4 (page 71)"
 },
 {
  "name": "Ganjuko",
  "type": "Arctic predator",
  "planet": "Fenn",
  "habitat": "Arctic",
  "dex": "1D",
  "per": "2D",
  "str": "4D",
  "move": "13",
  "size": "3.5m tall, 5m long",
  "orneriness": "",
  "special": "Sneak 3D+1. Climbing/jumping 4D+2. Beak: STR+1D damage, digging 5D, +1D to physical/energy to resist damage. Temperature Sensitivity: above 5°C for 1hr: -1D STR, -3 Move; after 1 day: -2D STR, -10 Move; after 1 week: hibernation until cold returns. Creature scale.",
  "source": "WEG40080 Creatures of the Galaxy p.28"
 },
 {
  "name": "Garagon",
  "type": "",
  "planet": "Koda's World",
  "habitat": "Mountain",
  "dex": "3D",
  "per": "3D",
  "str": "7D",
  "move": "10",
  "size": "8m tall, 20m long",
  "orneriness": "",
  "special": "Breath Attack: 7D fire in 25m\u00d710m cone (every other round per head). Two heads.",
  "source": "Galaxy Guide 8 (page 90)"
 },
 {
  "name": "Garral",
  "type": "Genetically engineered guard beast",
  "planet": "",
  "habitat": "Various",
  "dex": "2D",
  "per": "3D",
  "str": "4D+2",
  "move": "16",
  "size": "2m long",
  "orneriness": "",
  "special": "Claws: STR+1D. Teeth: STR+1D. Attack Training. Repulsorlift Sensitivity: attacks active repulsorlifts within 200m.",
  "source": "Thrawn Trilogy Sourcebook (pages 157-158)"
 },
 {
  "name": "Gartro",
  "type": "",
  "planet": "",
  "habitat": "Urban",
  "dex": "3D+1",
  "per": "3D",
  "str": "1D+1",
  "move": "4/20 (flying)",
  "size": "Small",
  "orneriness": "",
  "special": "Bite: STR. Exceptional Reflexes: +1D initiative. Low-light Vision.",
  "source": "Coruscant and the Core Worlds (pages 34-35)"
 },
 {
  "name": "Gasnit",
  "type": "Airborne predator",
  "planet": "",
  "habitat": "Various",
  "dex": "2D",
  "per": "2D+2",
  "str": "8D",
  "move": "6/20 (fly)",
  "size": "Huge",
  "orneriness": "",
  "special": "Bite: STR+1D+1. Claws: STR+1D. Darkvision: 20m. Heat Resistant: -10 to heat damage. Keen Vision: +1D search daylight. Low Light Vision. Poison: VD STR or 5D venom. Poison Immunity. Wings: STR+2.",
  "source": ""
 },
 {
  "name": "Geonosian Hydra",
  "type": "Huge predator",
  "planet": "Geonosis",
  "habitat": "Desert/Cave",
  "dex": "3D+1",
  "per": "2D",
  "str": "7D",
  "move": "10/10 (climbing)",
  "size": "Huge",
  "orneriness": "",
  "special": "Bite: STR+3D. Exoskeleton: +1D/+1 resist. Darkvision: 20m.",
  "source": "Ultimate Adversaries (page 86)"
 },
 {
  "name": "Gharzr",
  "type": "Large forest predator",
  "planet": "Dxun/Onderon",
  "habitat": "Jungle",
  "dex": "4D+2",
  "per": "2D",
  "str": "3D+2",
  "move": "20/16 (climbing)",
  "size": "3m long",
  "orneriness": "",
  "special": "Bite: STR+2D. Claws: STR+1D. Poisonous Stingers: STR-1D + 6D poison on wound+. Low-Light Vision.",
  "source": "Ultimate Adversaries (pages 86-87)"
 },
 {
  "name": "Ghest",
  "type": "Cartilaginous reptile",
  "planet": "Rodia",
  "habitat": "Swamp/Aquatic",
  "dex": "1D",
  "per": "2D",
  "str": "7D",
  "move": "15 (swimming), 8 (walking)",
  "size": "6 meters long",
  "orneriness": "",
  "special": "Teeth: STR+2D damage. Claws: STR+1D damage.",
  "source": "WEG40080 Creatures of the Galaxy p.29"
 },
 {
  "name": "Giant Tauntaun",
  "type": "",
  "planet": "Hoth",
  "habitat": "Arctic",
  "dex": "2D",
  "per": "3D+2",
  "str": "5D+2",
  "move": "18",
  "size": "3.5m tall",
  "orneriness": "3D",
  "special": "Head Butt: STR+2D. Kick: STR+1D. Cold Resistant.",
  "source": "Galaxy Guide 3 (page 65)"
 },
 {
  "name": "Giant Thranta",
  "type": "",
  "planet": "Bespin",
  "habitat": "Atmosphere",
  "dex": "2D+2",
  "per": "2D",
  "str": "8D",
  "move": "35 (flying)",
  "size": "30m+ wingspan",
  "orneriness": "2D",
  "special": "Tail Slap: STR+2D. Mounts: can carry multiple riders/cargo. Walker Scale.",
  "source": "Galaxy Guide 3 (page 67)"
 },
 {
  "name": "Glacier Tauntaun",
  "type": "",
  "planet": "Hoth",
  "habitat": "Arctic",
  "dex": "2D+1",
  "per": "3D+2",
  "str": "4D+2",
  "move": "14",
  "size": "2m tall",
  "orneriness": "3D+1",
  "special": "Head Butt: STR+1D. Kick: STR+2. Cold Resistant. Climbing: +1D over ice.",
  "source": "Galaxy Guide 3 (page 65)"
 },
 {
  "name": "Glim Worm",
  "type": "Tunneling predator",
  "planet": "",
  "habitat": "Subterranean",
  "dex": "1D",
  "per": "1D",
  "str": "1D",
  "move": "10 (ground), 14 (burrowing)",
  "size": "Up to 1 meter long",
  "orneriness": "",
  "special": "Sneak 4D. Brawling: grappling 3D, digging 4D. Grappling Attack: wraps body around target, opposed brawling: grappling vs brawling or STR to pull to mouth. Creature scale.",
  "source": "WEG40080 Creatures of the Galaxy p.30"
 },
 {
  "name": "Glittering Mynock",
  "type": "Glowing parasite",
  "planet": "",
  "habitat": "Space",
  "dex": "3D",
  "per": "1D",
  "str": "1D",
  "move": "15 (flying)",
  "size": "1m long",
  "orneriness": "",
  "special": "Stun Resistant: absorbs stun energy. Energy Drain: rainbow glow while draining.",
  "source": "Twin Star of Kira (page 73)"
 },
 {
  "name": "Gorax",
  "type": "Colossal predator",
  "planet": "Moon of Endor",
  "habitat": "Forest/Mountain",
  "dex": "2D",
  "per": "2D+1",
  "str": "5D+2",
  "move": "20",
  "size": "20m tall",
  "orneriness": "",
  "special": "Claws: STR+2. Low-Light Vision. Walker Scale.",
  "source": ""
 },
 {
  "name": "Gorgand",
  "type": "Large predatory cat",
  "planet": "Trinta",
  "habitat": "Forest",
  "dex": "5D+2",
  "per": "3D",
  "str": "5D",
  "move": "15",
  "size": "Large",
  "orneriness": "",
  "special": "Teeth: STR+1D. Claws: STR+1D+1.",
  "source": "Domain of Evil (page 48)"
 },
 {
  "name": "Gorgodon",
  "type": "Large arctic predator",
  "planet": "Ilum",
  "habitat": "Arctic",
  "dex": "1D+1",
  "per": "2D+2",
  "str": "6D+2",
  "move": "10/10 (climbing)",
  "size": "Large",
  "orneriness": "",
  "special": "Claws: STR+2D+1. Bite: STR+2D. Tail: STR+2D+2. Poor Vision: -1D visual PER/search.",
  "source": "Ultimate Adversaries (page 88)"
 },
 {
  "name": "Gorm-Worm",
  "type": "Venomous reptile",
  "planet": "Gorm",
  "habitat": "Various",
  "dex": "5D",
  "per": "3D",
  "str": "1D",
  "move": "15",
  "size": "1-1.5m long",
  "orneriness": "",
  "special": "Bite: STR+1D. Lethal Venom: Moderate stamina or instantly Mortally Wounded.",
  "source": "WEG40082 Tales of the Jedi Companion pp.107-108"
 },
 {
  "name": "Gornt",
  "type": "Meat animal",
  "planet": "Hethar",
  "habitat": "Plains",
  "dex": "1D+2",
  "per": "1D",
  "str": "1D",
  "move": "6",
  "size": "1m long",
  "orneriness": "",
  "special": "Bite: STR+2.",
  "source": "WEG40080 Creatures of the Galaxy p.31"
 },
 {
  "name": "Gorryl",
  "type": "Scavenger",
  "planet": "Kashyyyk",
  "habitat": "Forest",
  "dex": "1D",
  "per": "2D",
  "str": "1D+1",
  "move": "2",
  "size": "1.5-2.5m long",
  "orneriness": "",
  "special": "Swallow Whole: successful envelop = 2D+2 acid/round.",
  "source": ""
 },
 {
  "name": "Graiveh",
  "type": "Presentient humanoid carnivore",
  "planet": "Ealor",
  "habitat": "Plains",
  "dex": "3D",
  "per": "3D",
  "str": "3D+1",
  "move": "12",
  "size": "2.5m tall",
  "orneriness": "",
  "special": "Melee combat 4D. Brawling 6D+1. Claws: STR+1D damage. Leaping Attack: up to 10m leap, +2D to melee combat or brawling, STR+3D damage. Eyesight: +2D bonus on all PER tasks involving sight. Creature scale.",
  "source": "WEG40080 Creatures of the Galaxy p.32"
 },
 {
  "name": "Granite Slug",
  "type": "Land mollusk",
  "planet": "",
  "habitat": "Mountain",
  "dex": "1D",
  "per": "1D",
  "str": "1D+1",
  "move": "3",
  "size": "1.2-2.3m tall",
  "orneriness": "",
  "special": "Rubbery hide: +1D+2 vs physical and energy.",
  "source": "Jedi Academy Sourcebook (page 120)"
 },
 {
  "name": "Grazer",
  "type": "Grazer",
  "planet": "Yavin 8",
  "habitat": "Plains",
  "dex": "1D",
  "per": "2D",
  "str": "1D-7D",
  "move": "10",
  "size": "Variable (sheep to bantha)",
  "orneriness": "5D",
  "special": "(Variable stats by size)",
  "source": "Galaxy Guide 2 (page 23)"
 },
 {
  "name": "Great Oopik",
  "type": "Flightless avian-reptile",
  "planet": "Paramatan",
  "habitat": "Forest",
  "dex": "1D+1",
  "per": "2D",
  "str": "1D+2",
  "move": "8 (walking), 10 (climbing in tree limbs)",
  "size": "1-1.3 meters tall, up to 2-meter wingspan",
  "orneriness": "",
  "special": "Search 5D. Climbing/jumping 5D+2. Sonic Motion Detection: nearly blind, tracks prey via ultrasonic echolocation (reflected in PER/search). Sonic Stun: 45° forward arc, 4D stun damage, range 5 meters. Claws: foot and wing hook claws STR+2 damage. Tail: STR+1 damage. Creature scale.",
  "source": "WEG40080 Creatures of the Galaxy p.33"
 },
 {
  "name": "Greater Krayt Dragon",
  "type": "Colossal desert predator",
  "planet": "Tatooine",
  "habitat": "Desert",
  "dex": "3D",
  "per": "2D",
  "str": "8D",
  "move": "14/20 (burrowing sand)",
  "size": "100m long",
  "orneriness": "",
  "special": "Claws: STR. Teeth: STR+1D. Walker Scale.",
  "source": "Ultimate Adversaries (pages 96-97)"
 },
 {
  "name": "Grotseth",
  "type": "Aquatic predator",
  "planet": "Baralou",
  "habitat": "Aquatic",
  "dex": "3D",
  "per": "2D",
  "str": "3D",
  "move": "16 (swimming)",
  "size": "3-4m long",
  "orneriness": "",
  "special": "Teeth: STR+1D. Razored Scales: 4D damage on contact (Moderate brawling parry/DEX to avoid).",
  "source": "Planets Collection (page 31)"
 },
 {
  "name": "Grutchin",
  "type": "",
  "planet": "",
  "habitat": "Space",
  "dex": "1D",
  "per": "2D",
  "str": "7D+2",
  "move": "8/210 (fly)/Space 1",
  "size": "Large",
  "orneriness": "",
  "special": "Bite: STR+3D. Blindsight: 2000m in space. Darkvision: 20m. Environmental Resistance: vacuum + hyperspace. Self-Guiding: Piloting 5D missile.",
  "source": "New Jedi Order Sourcebook (pages 39-41)"
 },
 {
  "name": "Grutchin Symbiote",
  "type": "Bio-engineered predator",
  "planet": "",
  "habitat": "Various",
  "dex": "4D+2",
  "per": "2D",
  "str": "3D+2",
  "move": "12",
  "size": "0.5m",
  "orneriness": "",
  "special": "Bite: STR+2. Claws: STR. Grapple: immobilizes, opposed STR to escape.",
  "source": ""
 },
 {
  "name": "Guda Fish",
  "type": "Aquatic predator",
  "planet": "Neftali",
  "habitat": "Aquatic",
  "dex": "2D",
  "per": "2D+1",
  "str": "3D",
  "move": "16 (swimming)",
  "size": "3m long",
  "orneriness": "",
  "special": "Bite: STR+2. Escape: +1D vs fishing hooks. Spur: STR+1D. Sonar: +1D detect underwater/surface.",
  "source": "Black Sands of Socorro (pages 103-104)"
 },
 {
  "name": "Gulletbeast",
  "type": "Colossal aquatic predator",
  "planet": "Lamaredd",
  "habitat": "Aquatic",
  "dex": "1D",
  "per": "1D",
  "str": "5D+2",
  "move": "90 (swim)",
  "size": "0.75 km long",
  "orneriness": "",
  "special": "Capital Scale. Swallow Whole: 4D capital acid/round. 100m bite radius (50m no dodge).",
  "source": ""
 },
 {
  "name": "Gundark",
  "type": "",
  "planet": "",
  "habitat": "Forest/Cave",
  "dex": "4D+2",
  "per": "3D+2",
  "str": "6D+2",
  "move": "12",
  "size": "1-2.5m tall",
  "orneriness": "",
  "special": "Bite: STR+1D. Crushing Attack: STR+1D+1 (grapple). Fearless: +1D resist intimidation/fear. Low Light Vision. Track by Scent: +3D search tracking. Weapons: clubs STR+2.",
  "source": "Alien Anthology (page 31)"
 },
 {
  "name": "Gweld",
  "type": "Territorial amphibian",
  "planet": "Sedri",
  "habitat": "Swamp/Aquatic",
  "dex": "2D+2",
  "per": "3D",
  "str": "4D",
  "move": "7/12 (water)",
  "size": "1.5m long",
  "orneriness": "",
  "special": "Long Limbs: +1D brawling. Hinged Jaws: STR+1D. Amphibious.",
  "source": "Secrets of the Sisar Run (page 69)"
 },
 {
  "name": "Hairy Savage",
  "type": "Violent anthropoid",
  "planet": "Kuras III",
  "habitat": "Forest",
  "dex": "5D",
  "per": "2D",
  "str": "5D",
  "move": "10/12 (trees)",
  "size": "1.7-2.2m tall",
  "orneriness": "",
  "special": "Equipment: rocks (STR+1D), stone knives (STR+1D), sharpened sticks (STR+2D).",
  "source": "Planets Collection (page 183)"
 },
 {
  "name": "Halkra",
  "type": "Arboreal carnivore",
  "planet": "Bothawui",
  "habitat": "Forest",
  "dex": "1D",
  "per": "3D",
  "str": "2D+2",
  "move": "3",
  "size": "Body 2m, tentacles 8m",
  "orneriness": "",
  "special": "Camouflage: VD search to detect. Multiple Attacks: up to 4 no penalty, brawling = 1D per tentacle used. Tentacle Crushing: opposed STR or damage.",
  "source": "Shadows of the Empire Planets Guide (pages 60-61)"
 },
 {
  "name": "Hanadak",
  "type": "",
  "planet": "",
  "habitat": "Forest",
  "dex": "1D+1",
  "per": "1D+1",
  "str": "6D",
  "move": "10/2 (burrow)",
  "size": "3m tall",
  "orneriness": "",
  "special": "Bite: STR+1D+1. Claws: STR+1D. Low Light Vision.",
  "source": "Alien Anthology (page 32)"
 },
 {
  "name": "Hawk-Bat",
  "type": "Small airborne predator",
  "planet": "",
  "habitat": "Urban/Cave",
  "dex": "5D",
  "per": "4D",
  "str": "2D",
  "move": "4/30 (flying)",
  "size": "1m high",
  "orneriness": "",
  "special": "Claws: STR+2. Sonic Stun: 3D stun, 10m range (1/day). Exceptional Reflexes: +1D initiative. Blindsight: 20m.",
  "source": "Ultimate Adversaries (pages 89-90)"
 },
 {
  "name": "Hela",
  "type": "Aerial amphibian",
  "planet": "Analeh",
  "habitat": "Various",
  "dex": "4D+2",
  "per": "3D",
  "str": "1D+1",
  "move": "150",
  "size": "2m long",
  "orneriness": "",
  "special": "Flight 4D. Repulsorlift Sense: all helas within 2km radius drawn to repulsorlift generators. Teeth: STR+2 damage. Creature scale.",
  "source": "WEG40080 Creatures of the Galaxy p.35"
 },
 {
  "name": "Heliost",
  "type": "Large airborne herd animal",
  "planet": "Bespin",
  "habitat": "Atmosphere",
  "dex": "4D",
  "per": "2D+2",
  "str": "4D",
  "move": "12 (flying)",
  "size": "1.6m long",
  "orneriness": "",
  "special": "Gore: STR+1D. Electricity/Ion Resistance: +2D. Ionization Bolt: 3D energy + 3D ion, range 3-10/30/120 (every 1D rounds).",
  "source": "Ultimate Adversaries (pages 90-91)"
 },
 {
  "name": "Herlixx",
  "type": "Swampland omnivore",
  "planet": "Kasiol-3",
  "habitat": "Swamp",
  "dex": "3D+2",
  "per": "2D",
  "str": "3D+1",
  "move": "9 (land), 12 (water)",
  "size": "40-50 centimeters tall",
  "orneriness": "",
  "special": "Climbing/jumping 5D+1, swimming 5D+1. Non-aggressive — no recorded attacks on sapients, flees at unusual sounds or sudden movement.",
  "source": "WEG40080 Creatures of the Galaxy p.35"
 },
 {
  "name": "High Hound",
  "type": "Airborne scavenger",
  "planet": "Aduba-3",
  "habitat": "Various",
  "dex": "2D+1",
  "per": "2D",
  "str": "2D+2",
  "move": "10/20 (air)",
  "size": "Medium",
  "orneriness": "",
  "special": "Bite: STR+1D. Claws: STR+2. Flight. Low Light Vision.",
  "source": ""
 },
 {
  "name": "Hitcher Crab",
  "type": "Desert forager",
  "planet": "Sevarcos",
  "habitat": "Desert",
  "dex": "1D",
  "per": "1D+2",
  "str": "1D",
  "move": "12",
  "size": "1.3m long",
  "orneriness": "",
  "special": "Shell: +1D energy, +2D physical. Claws: STR+1D. Poison: 2D+2 (claws) or 1D+2 (shell). Water Sacs.",
  "source": "WEG40080 Creatures of the Galaxy p.36"
 },
 {
  "name": "Hoska",
  "type": "Large grazing herd beast",
  "planet": "Essowyn",
  "habitat": "Plains",
  "dex": "3D",
  "per": "1D",
  "str": "3D",
  "move": "5",
  "size": "2.7m at shoulder",
  "orneriness": "",
  "special": "Horns: STR+1D. Trample: STR+1D+2.",
  "source": "Planets Collection (page 44)"
 },
 {
  "name": "Howlrunner",
  "type": "Pack hunter",
  "planet": "Kamar",
  "habitat": "Plains",
  "dex": "2D+1",
  "per": "1D+1",
  "str": "3D",
  "move": "11",
  "size": "1.1m long",
  "orneriness": "",
  "special": "Bite: STR+1D.",
  "source": "Han Solo and the Corporate Sector Sourcebook (page 142)"
 },
 {
  "name": "Hssiss (Dark Side Dragon)",
  "type": "Ferocious dark-side reptile",
  "planet": "Stenness",
  "habitat": "Swamp",
  "dex": "3D+2",
  "per": "2D+1",
  "str": "5D+2",
  "move": "8",
  "size": "1.75-2m long",
  "orneriness": "",
  "special": "Light-side Disruption: -2D penalty to non-dark-side Force skills nearby (2D or less = cannot use).",
  "source": "Dark Side Sourcebook (pages 113-114); WEG40082 Tales of the Jedi Companion p.108 (STR 3D in this source)"
 },
 {
  "name": "Humbaba",
  "type": "Lumbering beast",
  "planet": "Kashoon",
  "habitat": "Forest",
  "dex": "2D",
  "per": "3D",
  "str": "3D",
  "move": "12",
  "size": "2m at shoulder",
  "orneriness": "2D",
  "special": "Lifting 5D. Trunk: STR+1D damage, can grab/push/crush. Armor: +2D to resist physical and energy attacks. Bite: STR+1D+1 damage. Claws: STR+2 damage. Concealment: +2D to sneak when hiding (coloration and clinging debris). Creature scale.",
  "source": "WEG40080 Creatures of the Galaxy p.38"
 },
 {
  "name": "Iagoin",
  "type": "",
  "planet": "Demophon",
  "habitat": "Various",
  "dex": "2D",
  "per": "1D+2",
  "str": "8D",
  "move": "10",
  "size": "50m long",
  "orneriness": "",
  "special": "Tentacles: 1D attacks/round.",
  "source": "Supernova (page 42)"
 },
 {
  "name": "Ibbot (Flame)",
  "type": "",
  "planet": "",
  "habitat": "Aquatic/Various",
  "dex": "4D",
  "per": "1D+2",
  "str": "2D",
  "move": "10/60 (flying)",
  "size": "4m wingspan",
  "orneriness": "",
  "special": "Bite: STR. Quick Reflexes: +2 pip initiative. Scalding Water Jet: 3D, range 1-3/10/20.",
  "source": "Coruscant and the Core Worlds (page 84)"
 },
 {
  "name": "Ibbot (Great)",
  "type": "",
  "planet": "",
  "habitat": "Various",
  "dex": "3D+1",
  "per": "1D",
  "str": "5D+2",
  "move": "20/40 (flying)",
  "size": "10m wingspan",
  "orneriness": "",
  "special": "Bite: STR+1D. Quick Reflexes: +2 pip initiative.",
  "source": "Coruscant and the Core Worlds (page 84)"
 },
 {
  "name": "Ibliton",
  "type": "Swamp hazard",
  "planet": "Randorn 2",
  "habitat": "Swamp",
  "dex": "4D",
  "per": "2D",
  "str": "4D",
  "move": "8",
  "size": "2m long (5m with tentacles)",
  "orneriness": "",
  "special": "Brawling 5D. Entangling: Difficult brawling hit snares with tentacles, -2D DEX and immobilized; opposed STR roll, must succeed by 6+ to break free. Armor: head/body +2D to resist damage. Leg Claws: STR+1D damage.",
  "source": "WEG40080 Creatures of the Galaxy p.39"
 },
 {
  "name": "Ice Modrol",
  "type": "Arctic predator",
  "planet": "Neftali",
  "habitat": "Arctic",
  "dex": "3D",
  "per": "4D",
  "str": "7D",
  "move": "18",
  "size": "5m tall",
  "orneriness": "",
  "special": "Claws: STR+2D. Camouflage: white skin, +1D sneak.",
  "source": "Black Sands of Socorro (pages 98-99)"
 },
 {
  "name": "Ice Scrabbler",
  "type": "",
  "planet": "Hoth VI",
  "habitat": "Arctic",
  "dex": "4D+2",
  "per": "2D",
  "str": "0D+2",
  "move": "8/6 (burrow)",
  "size": "10-50cm",
  "orneriness": "",
  "special": "Bite: STR+2D. Targeted Olfactory: auto-locate food within 30m through ice/containers.",
  "source": "Alien Anthology (pages 32-33)"
 },
 {
  "name": "Ikov",
  "type": "Small scavenger",
  "planet": "Kashyyyk",
  "habitat": "Forest",
  "dex": "1D",
  "per": "1D+2",
  "str": "1D",
  "move": "8/16 (climbing)",
  "size": "Small",
  "orneriness": "",
  "special": "Teeth: STR+1D. Climbing Claws: +1D climbing.",
  "source": "Truce at Bakura Sourcebook (page 102)"
 },
 {
  "name": "Implanter",
  "type": "",
  "planet": "",
  "habitat": "Various",
  "dex": "4D",
  "per": "1D",
  "str": "0D+2",
  "move": "3",
  "size": "0.2m long",
  "orneriness": "",
  "special": "Implant: surge-coral seed on stunned target. Difficult STR or lose pips from STR/DEX. Low-Light Vision. Stunning Frond: Difficult STR or stunned 1D rounds.",
  "source": "New Jedi Order Sourcebook (pages 48-49)"
 },
 {
  "name": "Istician",
  "type": "Subsentient",
  "planet": "Istic II",
  "habitat": "Subterranean",
  "dex": "2D",
  "per": "2D",
  "str": "2D",
  "move": "11",
  "size": "2m tall",
  "orneriness": "",
  "special": "Fangs: STR+1. Burrowing Claws: STR+2. Exoskeleton: +1D energy, +2D physical.",
  "source": "Truce at Bakura Sourcebook (pages 105-106)"
 },
 {
  "name": "Ithorian Dragon",
  "type": "Huge predator",
  "planet": "Ithor",
  "habitat": "Forest",
  "dex": "3D",
  "per": "3D",
  "str": "8D",
  "move": "16",
  "size": "3m tall, 5-9m long",
  "orneriness": "",
  "special": "Poisonous Bite: STR+4D + Heroic stamina or -1D then -2D DEX. Tough Skin: +1D resist. Darkvision: 20m. Scent: detect 10m (20m upwind, 6m downwind).",
  "source": "Ultimate Adversaries (pages 92-93)"
 },
 {
  "name": "Jakobeast",
  "type": "",
  "planet": "",
  "habitat": "Arctic",
  "dex": "2D+2",
  "per": "3D",
  "str": "6D+2",
  "move": "8",
  "size": "Huge (bantha-sized)",
  "orneriness": "",
  "special": "Aid Another: +2 pip per jakobeast aiding Force Push (max 4). Camouflage: +1D+1 hide in arctic. Force-Sensitive (Alter 2D+1). Head Butt: STR+1D+1.",
  "source": "Power of the Jedi Sourcebook (pages 78-79)"
 },
 {
  "name": "Jer'usk (Widowmaker)",
  "type": "Gargantuan aquatic predator",
  "planet": "Goroth",
  "habitat": "Aquatic",
  "dex": "1D",
  "per": "2D",
  "str": "2D",
  "move": "21 (swimming)",
  "size": "75m long",
  "orneriness": "6D",
  "special": "Jaws: STR+1D. Tail-Slap: STR+2D (speeder scale). Broach: 5D speeder damage. Armor: +2D physical, +2 energy. Engulf: swallow whole, 3D acid/round. Speeder Scale.",
  "source": "Goroth, Slave of the Empire (page 94)"
 },
 {
  "name": "Jexxel",
  "type": "Predator",
  "planet": "Neftali",
  "habitat": "Arctic/Mountain",
  "dex": "5D",
  "per": "4D",
  "str": "3D",
  "move": "18-20",
  "size": "0.5m tall, 1m long",
  "orneriness": "",
  "special": "Claws: STR+1D+1. Fangs: STR+3D. Night Vision.",
  "source": "Wretched Hives (page 17)"
 },
 {
  "name": "K'lor'slug",
  "type": "",
  "planet": "",
  "habitat": "Various",
  "dex": "4D",
  "per": "4D",
  "str": "2D",
  "move": "10",
  "size": "2-3m long",
  "orneriness": "",
  "special": "Pincers: STR+1D+2. Tail sting: STR+1D + Difficult DEX or neurotoxin (Difficult stamina or immobile 1D min).",
  "source": "Alien Anthology (pages 34-35)"
 },
 {
  "name": "Kaadu",
  "type": "",
  "planet": "Naboo",
  "habitat": "Swamp/Plains",
  "dex": "2D+2",
  "per": "1D+2",
  "str": "3D+2",
  "move": "20",
  "size": "2.5-3m",
  "orneriness": "1D",
  "special": "Bite: STR+2. Kick: STR+1D. Low-Light Vision.",
  "source": "Secrets of Naboo (page 58)"
 },
 {
  "name": "Kaladur",
  "type": "Drone servant",
  "planet": "",
  "habitat": "Various",
  "dex": "3D",
  "per": "0D",
  "str": "1D+2",
  "move": "6",
  "size": "1.6m",
  "orneriness": "",
  "special": "Multiple Appendages: 2 extra actions no penalty.",
  "source": ""
 },
 {
  "name": "Kalaide",
  "type": "Small mollusk",
  "planet": "Cols",
  "habitat": "Aquatic",
  "dex": "0D",
  "per": "0D",
  "str": "0D",
  "move": "1",
  "size": "2cm long",
  "orneriness": "",
  "special": "Harmless. Produces masrizeen, basis for antidote to lethal JurrinexG and Jurr-5 hive viruses. Found only in Harvest Bay, Cols. Cannot be transplanted or synthesized.",
  "source": "WEG40080 Creatures of the Galaxy p.41"
 },
 {
  "name": "Kalan",
  "type": "Grazing herbivore",
  "planet": "Mutanda",
  "habitat": "Plains",
  "dex": "4D",
  "per": "3D",
  "str": "2D+1",
  "move": "28",
  "size": "3.4m at shoulder",
  "orneriness": "",
  "special": "Tusks: STR+2D. Trample: STR+2D+1.",
  "source": "Planets Collection (page 76)"
 },
 {
  "name": "Karstag",
  "type": "Swamp predator",
  "planet": "Rodia",
  "habitat": "Swamp",
  "dex": "2D+1",
  "per": "3D+1",
  "str": "3D+2",
  "move": "9/8 (swimming)",
  "size": "1.5m at shoulder, 4-5m long",
  "orneriness": "",
  "special": "Thick Hide: +1D physical. Bite: STR. Tail spikes: variable by age (STR+1D to STR+4D).",
  "source": "Lords of the Expanse (page 59)"
 },
 {
  "name": "Kashyyyk Bantha",
  "type": "Herd animal",
  "planet": "Kashyyyk",
  "habitat": "Forest",
  "dex": "1D+2",
  "per": "2D+1",
  "str": "8D",
  "move": "6",
  "size": "2-3 meters tall",
  "orneriness": "2D",
  "special": "Gore: STR+2D.",
  "source": ""
 },
 {
  "name": "Katarn",
  "type": "Predator",
  "planet": "Kashyyyk",
  "habitat": "Forest",
  "dex": "3D",
  "per": "4D",
  "str": "4D",
  "move": "15",
  "size": "0.6-0.8m long",
  "orneriness": "",
  "special": "Teeth: STR+1D. Claws: STR+1D. Horns: STR+1.",
  "source": "Alien Anthology (pages 33-34)"
 },
 {
  "name": "Kath Hound",
  "type": "Plains predator",
  "planet": "Dantooine",
  "habitat": "Plains",
  "dex": "2D+2",
  "per": "3D+1",
  "str": "4D+2",
  "move": "8",
  "size": "Large",
  "orneriness": "",
  "special": "Horns: STR+1D+2. Bite: STR+1. Head-Butt: +2 brawling, STR+2.",
  "source": ""
 },
 {
  "name": "Kayven Whistler",
  "type": "",
  "planet": "Kayven",
  "habitat": "Forest",
  "dex": "4D",
  "per": "2D",
  "str": "4D",
  "move": "15",
  "size": "1m long",
  "orneriness": "",
  "special": "Razored Teeth: 6D damage.",
  "source": "Galaxy Guide 7 (page 49)"
 },
 {
  "name": "Keejin (Cave Crawler)",
  "type": "Cave-dwelling omnivore",
  "planet": "",
  "habitat": "Subterranean",
  "dex": "3D",
  "per": "2D",
  "str": "2D+1",
  "move": "5",
  "size": "1-2m long",
  "orneriness": "",
  "special": "Camouflage: increase visual PER rolls by two difficulty levels to spot in caves. Clinging: barbed claws allow movement on vertical surfaces and ceilings. Creature scale.",
  "source": "WEG40080 Creatures of the Galaxy p.43"
 },
 {
  "name": "Keffi",
  "type": "",
  "planet": "",
  "habitat": "Plains",
  "dex": "4D+1",
  "per": "3D+1",
  "str": "3D",
  "move": "20",
  "size": "Medium",
  "orneriness": "",
  "special": "Bite: STR. Kick: STR+2.",
  "source": "Coruscant and the Core Worlds (page 50)"
 },
 {
  "name": "Kell Dragon",
  "type": "",
  "planet": "",
  "habitat": "Various",
  "dex": "1D",
  "per": "1D+1",
  "str": "12D+2",
  "move": "15",
  "size": "Colossal",
  "orneriness": "",
  "special": "Bite: STR+2D+2. Claws: STR+1D+2.",
  "source": "Power of the Jedi Sourcebook (page 156)"
 },
 {
  "name": "Kichicolia",
  "type": "Furry primitives",
  "planet": "Najarka",
  "habitat": "Forest",
  "dex": "4D",
  "per": "3D",
  "str": "1D",
  "move": "5",
  "size": "0.5m tall",
  "orneriness": "",
  "special": "Foreclaws: STR. Teeth: STR+1D. (Harmless herbivores, retreat if threatened.)",
  "source": "Abduction of Crying Dawn Singer (pages 10-11)"
 },
 {
  "name": "Kilassin",
  "type": "Huge predator",
  "planet": "",
  "habitat": "Aquatic/Swamp",
  "dex": "2D",
  "per": "2D",
  "str": "5D+1",
  "move": "20/10 (swimming)",
  "size": "Huge",
  "orneriness": "",
  "special": "Claws: STR+1D+2. Low-Light Vision. Thick Skin: +1D/+1 resist.",
  "source": "Ultimate Adversaries (page 94)"
 },
 {
  "name": "Killit",
  "type": "Hunting raptor",
  "planet": "Zelos II",
  "habitat": "Mountain",
  "dex": "4D",
  "per": "4D",
  "str": "3D",
  "move": "38 (flying)",
  "size": "1m long, 2m wingspan",
  "orneriness": "",
  "special": "Beak: STR+1D.",
  "source": "Planets Collection (page 252)"
 },
 {
  "name": "Kintan Strider",
  "type": "Large desert predator",
  "planet": "Kintan",
  "habitat": "Desert",
  "dex": "2D",
  "per": "1D",
  "str": "6D+1",
  "move": "10",
  "size": "2.5m tall",
  "orneriness": "",
  "special": "Natural Armor: +2D vs physical. Regenerative: heal at 4x rate.",
  "source": "Geonosis and the Outer Rim Worlds (pages 89-90)"
 },
 {
  "name": "Kirithin",
  "type": "",
  "planet": "",
  "habitat": "Aquatic",
  "dex": "7D",
  "per": "3D",
  "str": "0D+1",
  "move": "4/10 (swim)",
  "size": "Wookiee fist-sized",
  "orneriness": "",
  "special": "Bite: STR+2. Darkvision: 20m. Low Light Vision.",
  "source": "Alien Anthology (page 34)"
 },
 {
  "name": "Kkekkrrg Rro",
  "type": "Medium forest predator",
  "planet": "Kashyyyk",
  "habitat": "Forest",
  "dex": "4D+2",
  "per": "2D+2",
  "str": "4D+1",
  "move": "16",
  "size": "2m long",
  "orneriness": "",
  "special": "Claws: STR+2. Central Claw: STR+1D+2. Bite: STR+1D.",
  "source": "Ultimate Adversaries (pages 94-95)"
 },
 {
  "name": "Knobby Spider",
  "type": "Mobile predatory plant",
  "planet": "Dagobah",
  "habitat": "Swamp",
  "dex": "4D",
  "per": "2D+1",
  "str": "6D",
  "move": "11",
  "size": "4m tall",
  "orneriness": "",
  "special": "Bite: STR+1D. Eight Legs: 2 actions no penalty, 3rd = -1D etc.",
  "source": "WEG40125 Galaxy Guide 3: The Empire Strikes Back p.63; WEG40089 The Movie Trilogy Sourcebook SE p.176"
 },
 {
  "name": "Kouhun",
  "type": "",
  "planet": "",
  "habitat": "Various",
  "dex": "5D+1",
  "per": "1D",
  "str": "0D+1",
  "move": "6",
  "size": "30cm long",
  "orneriness": "",
  "special": "Poisonous Sting: 10D poison (no armor penetration \u2014 exposed skin only). Stealthy: +2D hide and sneak.",
  "source": "Revised d20 Core Rulebook (page 337)"
 },
 {
  "name": "Kououra",
  "type": "Small venomous carnivore",
  "planet": "Kashyyyk",
  "habitat": "Forest",
  "dex": "1D",
  "per": "1D",
  "str": "1D+2",
  "move": "6",
  "size": "0.8-1.2m long",
  "orneriness": "",
  "special": "Venom: 6D damage.",
  "source": "Truce at Bakura Sourcebook (page 106)"
 },
 {
  "name": "Krak'jya",
  "type": "Intelligent predator",
  "planet": "Bothawui",
  "habitat": "Forest/Mountain",
  "dex": "4D",
  "per": "3D",
  "str": "5D",
  "move": "12/3 (jumping)",
  "size": "2m long, 1.3m tall",
  "orneriness": "",
  "special": "Claws: STR+2D (+1D climbing). Charging: STR stun per target in path. Camouflage: +1D sneak native terrain. Roar: 5D intimidation (fail 9+ = paralyzed 3 rounds). Senses: +2D search/PER.",
  "source": "Shadows of the Empire Planets Guide (pages 61-62)"
 },
 {
  "name": "Krakana",
  "type": "",
  "planet": "Mon Calamari",
  "habitat": "Aquatic",
  "dex": "2D",
  "per": "2D",
  "str": "6D",
  "move": "24 (swimming)",
  "size": "14.5-25m",
  "orneriness": "",
  "special": "Tentacles: 4D+1 each (3+ = caught, drawn to mouth). Teeth: STR+2D+2. Speeder Scale.",
  "source": "Alien Anthology (page 37)"
 },
 {
  "name": "Krayt Dragon",
  "type": "",
  "planet": "Tatooine",
  "habitat": "Desert",
  "dex": "3D",
  "per": "1D+2",
  "str": "12D",
  "move": "15",
  "size": "10m tall, 15-30m long",
  "orneriness": "",
  "special": "Claws: 8D. Teeth: 15D. Motion Tracking: +1D PER vs moving targets (Difficult sneak to stand still enough).",
  "source": "Star Wars Trilogy Sourcebook SE (page 176)"
 },
 {
  "name": "Kril",
  "type": "Domesticated food animal",
  "planet": "Kidron",
  "habitat": "Mountain",
  "dex": "1D",
  "per": "1D",
  "str": "1D",
  "move": "7",
  "size": "1.5m at shoulder, 2.5m long",
  "orneriness": "2D",
  "special": "Hooves: STR+2. Horn: STR+1D+1.",
  "source": "Planets Collection (page 155)"
 },
 {
  "name": "Kro",
  "type": "Small reptile",
  "planet": "Zelos II",
  "habitat": "Various",
  "dex": "4D",
  "per": "2D",
  "str": "1D",
  "move": "32",
  "size": "1m long",
  "orneriness": "",
  "special": "Bite: STR+2D.",
  "source": "Planets Collection (page 252)"
 },
 {
  "name": "Kudana",
  "type": "Large plains herd animal",
  "planet": "Rutan",
  "habitat": "Plains",
  "dex": "2D",
  "per": "2D+1",
  "str": "7D",
  "move": "20",
  "size": "Large",
  "orneriness": "",
  "special": "(Herd animal \u2014 minimal combat)",
  "source": "Ultimate Adversaries (pages 99-100)"
 },
 {
  "name": "KyLessian Fruit Bat",
  "type": "Vicious flying predator",
  "planet": "KyLessia",
  "habitat": "Forest/Cave",
  "dex": "2D+2",
  "per": "1D+2",
  "str": "1D",
  "move": "35 (flying)",
  "size": "15-30cm long",
  "orneriness": "",
  "special": "Flight: +2D sneak. Bite: STR+2 (dangerous in groups). Infrared Vision. Ultrasonic Communication. Nocturnal.",
  "source": "Truce at Bakura Sourcebook (page 105)"
 },
 {
  "name": "Lampfish",
  "type": "Deep-ocean fish",
  "planet": "Mon Calamari",
  "habitat": "Aquatic",
  "dex": "3D",
  "per": "1D",
  "str": "8D-12D",
  "move": "1",
  "size": "10-40m long",
  "orneriness": "",
  "special": "(Variable STR by size)",
  "source": "Death in the Undercity (page 55)"
 },
 {
  "name": "Lava Dragon",
  "type": "Reptilian predator",
  "planet": "",
  "habitat": "Volcanic",
  "dex": "3D",
  "per": "5D",
  "str": "6D",
  "move": "6 (swimming lava)",
  "size": "2.6-5.8m tall, 6.6-9m long",
  "orneriness": "",
  "special": "Armor: +4D physical, +8D energy (reflective). Wounded = takes 5D lava damage/round (speeder scale). Teeth: STR+2D. Lava attack: 10D character scale. Speeder Scale.",
  "source": "Jedi Academy Sourcebook (page 121)"
 },
 {
  "name": "Lepusa",
  "type": "Subterranean burrowing herbivore",
  "planet": "Freliq",
  "habitat": "Subterranean",
  "dex": "4D+2",
  "per": "4D+1",
  "str": "3D",
  "move": "6 (burrowing), 9 (walking), 12 (jumping)",
  "size": "0.4-0.8 meters tall",
  "orneriness": "",
  "special": "Dodge 6D. Hide 5D, search 5D+1. Digging 5D, climbing/jumping 5D. Claws: STR+2 damage. Springing: powerful hind legs for long leaps (reflected in dodge). Tool-Making: Difficult PER to fashion rudimentary tools from nearby materials. Creature scale.",
  "source": "WEG40080 Creatures of the Galaxy p.44"
 },
 {
  "name": "Llwelkyn",
  "type": "Ferocious predator",
  "planet": "Bakura",
  "habitat": "Forest",
  "dex": "2D",
  "per": "2D",
  "str": "4D",
  "move": "26",
  "size": "3m long",
  "orneriness": "",
  "special": "Enhanced Smell: +1D search, detect repulsorlifts 200km. Claws: STR+2D. Bite: STR+1D+2. Pack: 5-12 creatures.",
  "source": "Truce at Bakura Sourcebook (pages 104-105)"
 },
 {
  "name": "Herbivore Burrower",
  "type": "Herbivore burrower",
  "planet": "Yavin 8",
  "habitat": "Tundra",
  "dex": "2D",
  "per": "2D",
  "str": "1D",
  "move": "12",
  "size": "0.5m long",
  "orneriness": "",
  "special": "Teeth: STR+1. Slipperiness: +1D to dodge. Shy — avoids confrontation but will scavenge food stores.",
  "source": "WEG40119 Galaxy Guide 2: Yavin and Bespin p.22"
 },
 {
  "name": "Loper",
  "type": "Carnivorous burrower",
  "planet": "Yavin 8",
  "habitat": "Jungle",
  "dex": "2D",
  "per": "2D",
  "str": "3D",
  "move": "9",
  "size": "1.5m long",
  "orneriness": "",
  "special": "Teeth: STR+1. Claws: STR+1. Barbed Tail: STR+1D.",
  "source": "Galaxy Guide 2 (page 23)"
 },
 {
  "name": "Moss-hopper",
  "type": "Carnivorous burrower",
  "planet": "Yavin 8",
  "habitat": "Tundra",
  "dex": "2D",
  "per": "3D",
  "str": "1D",
  "move": "9",
  "size": "0.3m long",
  "orneriness": "",
  "special": "Venomous Teeth: STR+1 damage with weak venom injected via hollow incisors (Moderate stamina or mild illness). Search 4D. Sneak 5D. Fearlessly attacks creatures many times its size.",
  "source": "WEG40119 Galaxy Guide 2: Yavin and Bespin p.23"
 },
 {
  "name": "Raith",
  "type": "Carnivorous rodent",
  "planet": "Yavin 8",
  "habitat": "Mountain / Equatorial",
  "dex": "2D",
  "per": "2D",
  "str": "1D",
  "move": "8",
  "size": "0.5–0.8m long (excluding tail)",
  "orneriness": "",
  "special": "Teeth: STR+1D. Hide 4D, Search 5D. Large black rodent with thick hairless tail; eats carrion, small animals, and eggs including Melodie eggs. Very common in equatorial region.",
  "source": "WEG40119 Galaxy Guide 2: Yavin and Bespin p.23"
 },
 {
  "name": "Ropedancer",
  "type": "Tundra serpent",
  "planet": "Yavin 8",
  "habitat": "Tundra marshes and ponds",
  "dex": "2D",
  "per": "3D",
  "str": "7D",
  "move": "12",
  "size": "12–18m long",
  "orneriness": "",
  "special": "Fangs: STR+2 damage. Plated skin: +1D STR for defense. Search 6D+1, Sneak 4D. Swallows prey whole; body expands to twice normal diameter. Migrates south ahead of winter freeze.",
  "source": "WEG40119 Galaxy Guide 2: Yavin and Bespin p.24"
 },
 {
  "name": "Lrashtash",
  "type": "Herbivore",
  "planet": "Usean II",
  "habitat": "Various",
  "dex": "1D",
  "per": "1D",
  "str": "1D",
  "move": "4",
  "size": "",
  "orneriness": "",
  "special": "Bluffing Teeth: intimidation 4D vs willpower/PER.",
  "source": "Truce at Bakura Sourcebook (page 107)"
 },
 {
  "name": "Ludos",
  "type": "Swampland carnivore",
  "planet": "Ganlihk",
  "habitat": "Swamp",
  "dex": "4D+1",
  "per": "3D+2",
  "str": "3D+2",
  "move": "6 (land), 12 (swimming)",
  "size": "30-45 cm diameter, tentacles to 80 cm",
  "orneriness": "",
  "special": "Climbing 4D+2, swimming 6D. Bioluminescence: phosphorescent light aids navigation in murky waters. Luminescence Control: bright burst (red to white) to scare predators, or change to dull black and play dead. Tentacles: sift through mud/silt, catch small game with suckers, +1D to climbing/jumping on land. Creature scale.",
  "source": "WEG40080 Creatures of the Galaxy p.46"
 },
 {
  "name": "Lylek",
  "type": "Huge desert predator",
  "planet": "Ryloth",
  "habitat": "Desert/Cave",
  "dex": "4D+2",
  "per": "3D+2",
  "str": "6D+2",
  "move": "30",
  "size": "4m tall and wide",
  "orneriness": "",
  "special": "Bite: STR+3D. Tentacles: STR+2D. Stinger: STR+3D. Tough Carapace: +1D resist. Multiattack: 2 no penalty. Low-Light Vision. Exceptional Reflexes: +1D initiative.",
  "source": "Ultimate Adversaries (page 100)"
 },
 {
  "name": "Maalraas",
  "type": "",
  "planet": "",
  "habitat": "Forest",
  "dex": "3D+1",
  "per": "4D+1",
  "str": "4D",
  "move": "10",
  "size": "2m long",
  "orneriness": "",
  "special": "Bite: STR+1D. Claws: STR+2. Energy Resistance: +2D vs fire/blaster. Keen Senses: +1D search (sound). Scent tracking.",
  "source": ""
 },
 {
  "name": "Maelibus",
  "type": "Large desert predator",
  "planet": "Iego",
  "habitat": "Desert/Space",
  "dex": "2D+2",
  "per": "3D",
  "str": "4D",
  "move": "16",
  "size": "Large",
  "orneriness": "",
  "special": "Claws: STR+1D. Blindsight. Vacuum Creature. Force-Sensitive (Control 2D+2, Alter 2D+2, Sense 2D). Force Powers: Waves of Darkness, Illusion.",
  "source": "Geonosis and the Outer Rim Worlds (page 72)"
 },
 {
  "name": "Magus",
  "type": "Desert browser",
  "planet": "Reopi",
  "habitat": "Desert",
  "dex": "2D+1",
  "per": "2D+1",
  "str": "1D+1",
  "move": "9 (above ground), 14 (through sand)",
  "size": "58 centimeters long when fully extended",
  "orneriness": "",
  "special": "Sneak 4D+1. Stamina 5D. Armor: +1D+1 vs all physical and energy attacks. Claws: STR+2 damage. Odors: defensive gland secretes foul oil, Very Difficult stamina to close within 3-4 meters.",
  "source": "WEG40080 Creatures of the Galaxy p.47"
 },
 {
  "name": "Maligator",
  "type": "Large serpent",
  "planet": "Trinta",
  "habitat": "Swamp",
  "dex": "2D+1",
  "per": "2D",
  "str": "3D+2",
  "move": "15 (swim)/10 (crawl)",
  "size": "2.5m long",
  "orneriness": "",
  "special": "Teeth: STR+1. Spiked Tail: STR+1.",
  "source": "Domain of Evil (page 23)"
 },
 {
  "name": "Mantellian Savrip",
  "type": "Desert predator",
  "planet": "Ord Mantell",
  "habitat": "Desert/Mountain",
  "dex": "2D+1",
  "per": "3D",
  "str": "5D",
  "move": "10",
  "size": "2-2.5m tall",
  "orneriness": "",
  "special": "Armor: +1D physical and energy. Claws: STR+1D. Bite: STR+2D.",
  "source": ""
 },
 {
  "name": "Mantessan Panthac",
  "type": "Small-game predator",
  "planet": "",
  "habitat": "Various",
  "dex": "3D+2",
  "per": "4D",
  "str": "4D",
  "move": "16",
  "size": "0.4-0.5m long",
  "orneriness": "",
  "special": "Teeth: STR+1D. Claws: STR+1D+1.",
  "source": "Alien Anthology (pages 37-38)"
 },
 {
  "name": "Marigar Snow Q'lk",
  "type": "Arctic herbivore",
  "planet": "Neftali",
  "habitat": "Arctic",
  "dex": "3D",
  "per": "3D+1",
  "str": "4D",
  "move": "17",
  "size": "1.4m at shoulder, 1.53m long",
  "orneriness": "",
  "special": "Awareness: +1D search/PER in open areas. Antlers: STR+1. Hoofstrike: STR+1D+2.",
  "source": "Black Sands of Socorro (page 98)"
 },
 {
  "name": "Marmaw",
  "type": "Amphibious carnivore",
  "planet": "Almar",
  "habitat": "Aquatic/Swamp",
  "dex": "1D",
  "per": "2D",
  "str": "3D",
  "move": "5/15 (swimming)",
  "size": "4m long",
  "orneriness": "",
  "special": "Teeth: STR+1D. Beat parry by 4+ = latched (+2 difficulty levels). Beat by 10+ = pulled into mouth (opposed STR +5 to free). Amphibious: 3hr/day minimum in water.",
  "source": "Truce at Bakura Sourcebook (pages 102-103)"
 },
 {
  "name": "Marsh Haunt",
  "type": "",
  "planet": "",
  "habitat": "Swamp",
  "dex": "2D+1",
  "per": "3D+2",
  "str": "7D+2",
  "move": "12",
  "size": "Huge",
  "orneriness": "",
  "special": "Camouflage: +1D+1 hide in swamp. Exceptional Reflexes: +1D initiative. Force-Sensitive (Control 3D+1, Sense 2D+2, Alter 2D+1). Slam: STR.",
  "source": "Power of the Jedi Sourcebook (pages 79-80)"
 },
 {
  "name": "Marsh Lizard",
  "type": "Large serpent",
  "planet": "Trinta",
  "habitat": "Swamp",
  "dex": "3D+1",
  "per": "2D+1",
  "str": "2D+2",
  "move": "15 (swim)/10 (crawl)",
  "size": "2.5m long",
  "orneriness": "",
  "special": "Teeth: STR+1D. Spiked Tail: STR+1D.",
  "source": "Classic Adventures Volume Two (page 86)"
 },
 {
  "name": "Marsh Wyrm",
  "type": "",
  "planet": "Gelgelar",
  "habitat": "Swamp",
  "dex": "2D+2",
  "per": "1D+2",
  "str": "8D",
  "move": "14",
  "size": "30m long avg",
  "orneriness": "",
  "special": "Lunge Attack: STR+2D.",
  "source": "WEG40107 Platt's Starport Guide p.74"
 },
 {
  "name": "Massiff",
  "type": "",
  "planet": "",
  "habitat": "Desert",
  "dex": "3D",
  "per": "2D+2",
  "str": "3D+1",
  "move": "10",
  "size": "2m long",
  "orneriness": "",
  "special": "Bite: STR+2. Resistant to Radiation: +1D STR. Track by Scent: +1D search tracking.",
  "source": ""
 },
 {
  "name": "Merdeth",
  "type": "Colossal desert predator",
  "planet": "Geonosis",
  "habitat": "Desert",
  "dex": "1D",
  "per": "2D+1",
  "str": "7D",
  "move": "4",
  "size": "30m+ diameter",
  "orneriness": "",
  "special": "Tentacles: STR+1D. Bite: STR+3D. Darkvision: 200m. Mip Swarm: 1D swarms as free action. Starfighter Scale.",
  "source": ""
 },
 {
  "name": "Millitile",
  "type": "Many-legged fungus eater",
  "planet": "Kothlis",
  "habitat": "Forest/Subterranean",
  "dex": "1D+2",
  "per": "2D",
  "str": "1D+1",
  "move": "5/8 (using legs, 5 min)",
  "size": "8m long",
  "orneriness": "",
  "special": "Bite: STR + Moderate stamina or ill 2 days (-1D all).",
  "source": "Shadows of the Empire Planets Guide (pages 88-89)"
 },
 {
  "name": "Miner's Horror",
  "type": "Vacuum predator",
  "planet": "",
  "habitat": "Space",
  "dex": "3D",
  "per": "1D",
  "str": "4D",
  "move": "3 (space)",
  "size": "20m+ long",
  "orneriness": "",
  "special": "Brawling 5D. Saw Teeth: vibrate at high speed, STR+2D damage. Armor: +2D vs physical/energy. Starfighter scale. Brawling = attack skill vs starships, STR (with armor) = hull code, DEX = maneuverability.",
  "source": "WEG40080 Creatures of the Galaxy p.48"
 },
 {
  "name": "Modrol",
  "type": "Jungle predator",
  "planet": "Port Haven",
  "habitat": "Jungle",
  "dex": "3D",
  "per": "4D",
  "str": "7D",
  "move": "18",
  "size": "5m tall",
  "orneriness": "",
  "special": "Claws: STR+2D. Camouflage: +1D sneak in jungle.",
  "source": "WEG40107 Platt's Starport Guide p.34"
 },
 {
  "name": "Mole Serpent",
  "type": "",
  "planet": "",
  "habitat": "Subterranean",
  "dex": "1D+1",
  "per": "1D+1",
  "str": "7D",
  "move": "10",
  "size": "10-18m long",
  "orneriness": "",
  "special": "Bite: STR+1D+2. Burrow: Move 10. Stealth: +15 sneak underground. Swallow: opposed STR or swallowed, 5D acid. Tremorsense: Moderate search, 20m.",
  "source": "Secrets of Tatooine (pages 93-94)"
 },
 {
  "name": "Monnok",
  "type": "Desert predator",
  "planet": "Socorro",
  "habitat": "Desert",
  "dex": "4D",
  "per": "3D",
  "str": "5D",
  "move": "13",
  "size": "2.3m tall",
  "orneriness": "",
  "special": "Equipment: club (STR+1D) or spear (STR+1D+1).",
  "source": "Black Sands of Socorro (page 23)"
 },
 {
  "name": "Morvak",
  "type": "Tunnel creature",
  "planet": "Isen IV",
  "habitat": "Subterranean",
  "dex": "2D",
  "per": "2D",
  "str": "5D",
  "move": "6",
  "size": "0.7-1.3m long",
  "orneriness": "",
  "special": "Armor: +1D energy, +2D physical. Claws: STR+1D. Tentacle Acid: 4D. Rock Eaters. Space Survival.",
  "source": "Planets Collection (pages 65-66)"
 },
 {
  "name": "Mosgoth",
  "type": "Aerial reptilian mount",
  "planet": "",
  "habitat": "Various",
  "dex": "4D",
  "per": "3D",
  "str": "4D",
  "move": "7/28 (flying)",
  "size": "12-17.5m long",
  "orneriness": "3D",
  "special": "Claws: STR+2. Teeth: STR+1D. Tail: STR. Night Vision: organic sonar. Speeder Scale.",
  "source": "Alliance Intelligence Reports (page 74)"
 },
 {
  "name": "Mucous Salamander",
  "type": "Large aquatic predator",
  "planet": "Yavin IV",
  "habitat": "Swamp/Aquatic",
  "dex": "6D+1",
  "per": "3D+1",
  "str": "3D+2",
  "move": "2/50 (swimming)",
  "size": "Large",
  "orneriness": "",
  "special": "Bite: STR+1D+2. Amorphous: in water immune to pierce/bludgeon, can't grapple, slip through 5mm gaps. On land: +1D+1 resist but -2D DEX. Swallow Whole: 3D acid/round. Camouflage: +1D+1 hide in water. Low-Light Vision. Exceptional Reflexes: +1D initiative.",
  "source": "Ultimate Adversaries (pages 103-104)"
 },
 {
  "name": "Mutant Acklay",
  "type": "Huge mutated predator",
  "planet": "Geonosis",
  "habitat": "Desert/Aquatic",
  "dex": "1D+1",
  "per": "2D+1",
  "str": "7D",
  "move": "12",
  "size": "6 meters long",
  "orneriness": "",
  "special": "Armor: +2D STR vs physical. Bite: STR+2. Claws: STR+1D+2. Exceptional Reflexes: +1D initiative. Resistant to Radiation: +1D STR vs radiation. Blindsight. Breathe Underwater.",
  "source": "Geonosis and the Outer Rim Worlds (page 65)"
 },
 {
  "name": "Mutriok",
  "type": "Desert scavenger/hunter",
  "planet": "Socorro",
  "habitat": "Desert",
  "dex": "4D",
  "per": "5D",
  "str": "2D+1",
  "move": "12/16 (running)",
  "size": "0.79-0.92m at shoulder",
  "orneriness": "",
  "special": "Paws: STR. Teeth: STR+1D. Perception: +1D search/PER in open desert.",
  "source": "Black Sands of Socorro (page 20)"
 },
 {
  "name": "Muttamok",
  "type": "",
  "planet": "",
  "habitat": "Forest",
  "dex": "4D",
  "per": "2D",
  "str": "1D+1",
  "move": "8",
  "size": "1m tall",
  "orneriness": "",
  "special": "Bite: STR+1. Force-Sensitive (Sense 2D+1, Alter 2D). Force Power: Friendship.",
  "source": "Power of the Jedi Sourcebook (page 80)"
 },
 {
  "name": "Mynock",
  "type": "Parasite",
  "planet": "",
  "habitat": "Space",
  "dex": "3D",
  "per": "1D",
  "str": "1D",
  "move": "9/1 (space)",
  "size": "1.5m wide, 0.5m tall",
  "orneriness": "",
  "special": "Energy Drain: drain from starships. Flight. Silicon life form: survive vacuum.",
  "source": "Star Wars Sourcebook (pages 88-89)"
 },
 {
  "name": "Myntor",
  "type": "Burrowing predator",
  "planet": "Kothlis",
  "habitat": "Subterranean",
  "dex": "2D",
  "per": "3D",
  "str": "2D+1",
  "move": "8/18 (speed burst)",
  "size": "1-1.5m long",
  "orneriness": "",
  "special": "Armor: +2D (blaster reflects on wild die 1). Speed Burst: Move 18 for 2 rounds (every 6 hrs). Claws: STR+2. Bite: STR+1D+1.",
  "source": "Shadows of the Empire Planets Guide (page 88)"
 },
 {
  "name": "Najarkan Creature",
  "type": "Huge rainforest predator",
  "planet": "Najarka",
  "habitat": "Jungle",
  "dex": "2D",
  "per": "1D",
  "str": "10D",
  "move": "4",
  "size": "30m long",
  "orneriness": "7D",
  "special": "Bite: STR-2D.",
  "source": "Abduction of Crying Dawn Singer (page 21)"
 },
 {
  "name": "Najoon",
  "type": "Wild avian",
  "planet": "Endor",
  "habitat": "Forest",
  "dex": "3D+2",
  "per": "2D+1",
  "str": "1D+2",
  "move": "3/15 (flying)",
  "size": "0.25-0.5m long, 0.45-0.7m wingspan",
  "orneriness": "",
  "special": "",
  "source": "WEG40126 Galaxy Guide 5: Return of the Jedi p.82"
 },
 {
  "name": "Narglatch",
  "type": "Swamp alpha predator",
  "planet": "Naboo",
  "habitat": "Swamp",
  "dex": "2D+1",
  "per": "3D",
  "str": "7D",
  "move": "12/18 (swimming)",
  "size": "3m long",
  "orneriness": "",
  "special": "Low-Light Vision. Bite: STR+2D. Claws: STR+1D+1.",
  "source": ""
 },
 {
  "name": "Nashtah/Dravian Hound",
  "type": "",
  "planet": "Dra III",
  "habitat": "Various",
  "dex": "2D+2",
  "per": "2D+2",
  "str": "4D+1",
  "move": "15",
  "size": "1.1m at shoulder, 1.5m long",
  "orneriness": "",
  "special": "Armor: +1D. Claws: STR+2D. Teeth: STR+1D+2. Barbed Tail: STR+1D + tracking venom (1 month). Tracking Venom: unerring tracking.",
  "source": "Alien Anthology (page 38)"
 },
 {
  "name": "Neek",
  "type": "Herbivorous reptile",
  "planet": "Onderon",
  "habitat": "Various",
  "dex": "2D+1",
  "per": "6D",
  "str": "1D+1",
  "move": "7",
  "size": "0.25-0.5m",
  "orneriness": "",
  "special": "Visual Acuity: +2D visual search. Audial Acuity: +1D+2 audial search.",
  "source": "WEG40082 Tales of the Jedi Companion pp.108-109"
 },
 {
  "name": "Nek Battle Dog",
  "type": "Trained attack animal",
  "planet": "",
  "habitat": "Various",
  "dex": "2D",
  "per": "3D",
  "str": "2D+2",
  "move": "12",
  "size": "0.9-1.3m long",
  "orneriness": "",
  "special": "Body Armor: +2D STR. Teeth: STR+2D+1. Claws: STR+1D.",
  "source": "Alien Anthology (page 38)"
 },
 {
  "name": "Nerf",
  "type": "",
  "planet": "Alderaan",
  "habitat": "Plains",
  "dex": "3D+2",
  "per": "2D+1",
  "str": "2D+2",
  "move": "12",
  "size": "1m long, 0.5m tall",
  "orneriness": "2D",
  "special": "Bite: STR+2. Head Butt: STR+2. Kick: STR+2. Low Light Vision. Spittle: 3 in 6 chance, range 1-3/6/10, -1D social interaction until washed.",
  "source": "Alien Anthology (pages 39-40)"
 },
 {
  "name": "Nexu",
  "type": "",
  "planet": "",
  "habitat": "Forest",
  "dex": "4D+2",
  "per": "3D",
  "str": "4D+1",
  "move": "16",
  "size": "2m long",
  "orneriness": "",
  "special": "Bite: STR+2. Claws: STR+1. Exceptional Reflexes: +1D initiative. Keen Hearing: +1D search (sound).",
  "source": ""
 },
 {
  "name": "Nightdevil",
  "type": "Semi-intelligent mountain predator",
  "planet": "Goroth",
  "habitat": "Mountain",
  "dex": "3D",
  "per": "2D",
  "str": "7D",
  "move": "10",
  "size": "2.5-2.8m tall",
  "orneriness": "",
  "special": "Claws: STR+1D. Jaws: STR+2. Low-Light Vision (bright light = +1D penalty). Smell: +2D search tracking by scent. Projectiles: thrown rocks STR+1D, range 2-3/5/10.",
  "source": "Goroth, Slave of the Empire (page 93)"
 },
 {
  "name": "Nighthunter",
  "type": "",
  "planet": "",
  "habitat": "Forest",
  "dex": "3D+1",
  "per": "4D+1",
  "str": "4D",
  "move": "10",
  "size": "2m long",
  "orneriness": "",
  "special": "Bite: STR+1D. Claws: STR+2. Energy Resistance: +3D vs fire/blaster. Force-Sensitive (Control 2D, Sense 2D+2). Keen Senses: +2D search (sound). Scent tracking.",
  "source": "Power of the Jedi Sourcebook (pages 80-81)"
 },
 {
  "name": "Nightsinger",
  "type": "Flying predator",
  "planet": "Nightsinger's Orb",
  "habitat": "Various",
  "dex": "3D",
  "per": "4D",
  "str": "4D+2",
  "move": "16 (flying)",
  "size": "Large",
  "orneriness": "",
  "special": "Claws: STR+2D. Beak: STR+3D.",
  "source": "Tapani Sector Instant Adventures (page 48)"
 },
 {
  "name": "Nuna (Swamp Turkey)",
  "type": "",
  "planet": "",
  "habitat": "Swamp",
  "dex": "1D+1",
  "per": "1D",
  "str": "1D+1",
  "move": "5",
  "size": "0.5m tall",
  "orneriness": "",
  "special": "Bite: STR+1. Kick: STR+2. Allergic Reaction: Rodians/Twi'leks \u2014 Moderate STR or throat swells (suffocation without antitoxin); success = -1D all actions 20hrs.",
  "source": "Secrets of Naboo (page 58)"
 },
 {
  "name": "Nyantolo",
  "type": "Carapaced aquatic mammal",
  "planet": "Wyndigal 2",
  "habitat": "Aquatic",
  "dex": "2D",
  "per": "2D",
  "str": "3D",
  "move": "5 (swimming)",
  "size": "2.5-3m long",
  "orneriness": "",
  "special": "Concealment: Difficult PER to spot when inactive. Bite: STR+3D. Song: moaning whistle attracts mates, disconcerts others.",
  "source": "WEG40080 Creatures of the Galaxy p.49"
 },
 {
  "name": "Oasis Children",
  "type": "Desert vermin",
  "planet": "Endregaad",
  "habitat": "Desert",
  "dex": "2D+2",
  "per": "2D+1",
  "str": "1D+2",
  "move": "16/8 (two legs)",
  "size": "1.2m tall",
  "orneriness": "",
  "special": "Claws: STR+1 + Difficult stamina or paralyzed 1D min. Blindsight: 50m (scent/hearing). Empathic Link: 25km to oasis mother. Vermin: +3D PER vs mind-influencing.",
  "source": "Tempest Feud (pages 40-42)"
 },
 {
  "name": "Oasis Mother",
  "type": "Desert predator",
  "planet": "Endregaad",
  "habitat": "Desert",
  "dex": "1D+1",
  "per": "2D+2",
  "str": "7D+1",
  "move": "0",
  "size": "Large",
  "orneriness": "",
  "special": "Tentacles: 7\u00d7 STR+1D (3D+2 STR each). Blindsight: 50m. Natural Armor: +2 vs physical (not vibro). Telepathy: 25km to children. Multiattack: -1 per extra tentacle.",
  "source": "Tempest Feud (pages 40-42)"
 },
 {
  "name": "Onahk",
  "type": "Curious aquatic hunter",
  "planet": "Osirrag",
  "habitat": "Aquatic",
  "dex": "3D",
  "per": "2D",
  "str": "1D+1",
  "move": "10 (running), 14 (swimming)",
  "size": "1 meter long, 2.3 meters tall (neck fully extended)",
  "orneriness": "",
  "special": "Brawling parry 4D, dodge 3D+2. Search 3D, sneak 4D. Brawling 3D+2, climbing/jumping 3D+1, swimming 4D+2. Constricting Attack: neck constriction, STR+1D+2 damage. Claws: STR+1D damage, +1D to climbing. Creature scale.",
  "source": "WEG40080 Creatures of the Galaxy p.50"
 },
 {
  "name": "Opee Sea Killer",
  "type": "",
  "planet": "Naboo",
  "habitat": "Aquatic",
  "dex": "2D+2",
  "per": "1D",
  "str": "8D+1",
  "move": "63 (180 km/h)",
  "size": "20m long",
  "orneriness": "",
  "special": "Bite: STR+3D. Low Light Vision. Tongue Attack: Heroic STR/Hull to break free or drawn to mouth.",
  "source": "Secrets of Naboo (page 59)"
 },
 {
  "name": "Oskan Blood Eater",
  "type": "Aggressive carnivore",
  "planet": "",
  "habitat": "Various",
  "dex": "3D+2",
  "per": "2D",
  "str": "4D",
  "move": "6",
  "size": "2.5 meters tall",
  "orneriness": "",
  "special": "Search: tracking 5D. Claws: STR+3D damage. Frenzy: sight of humanoid/large creature adds +2D to damage. Tracking: pursues escaped prey for up to 2 standard weeks. Creature scale.",
  "source": "WEG40080 Creatures of the Galaxy p.51"
 },
 {
  "name": "Peko Peko",
  "type": "",
  "planet": "Naboo",
  "habitat": "Swamp/Forest",
  "dex": "4D+2 (flight)/1D (ground)",
  "per": "1D",
  "str": "1D+2",
  "move": "5/30 (flying)",
  "size": "2.5m long",
  "orneriness": "",
  "special": "Bite: STR+2. Flight.",
  "source": "Secrets of Naboo (page 59)"
 },
 {
  "name": "Phosflea",
  "type": "Glowing forest parasite",
  "planet": "Kashyyyk",
  "habitat": "Forest",
  "dex": "3D",
  "per": "1D+2",
  "str": "0D+1",
  "move": "12",
  "size": "0.2-0.5cm",
  "orneriness": "",
  "special": "Glow: used as cheap light source.",
  "source": ""
 },
 {
  "name": "Photon Spider",
  "type": "Subterranean predator",
  "planet": "Varl",
  "habitat": "Subterranean",
  "dex": "4D+2",
  "per": "3D",
  "str": "6D",
  "move": "18 (climbing)",
  "size": "4m long",
  "orneriness": "",
  "special": "Bite: STR+1D. Spinneret: 4D stun bolt. Natural Armor: +1D physical, +1 energy. Darkvision: 20m. Exceptional Reflexes: +1D initiative.",
  "source": "Tempest Feud (pages 120-121)"
 },
 {
  "name": "Pierceskimmer",
  "type": "",
  "planet": "Drexel II",
  "habitat": "Aquatic",
  "dex": "1D+1",
  "per": "2D+1",
  "str": "11D",
  "move": "20 (swim)",
  "size": "16-18m long",
  "orneriness": "",
  "special": "Low Light Vision. Spike Attack: STR+2D. Spike Scales: 1D auto damage to grapplers.",
  "source": "Alien Anthology (pages 40-41)"
 },
 {
  "name": "Piranha Beetle",
  "type": "Carnivorous beetle",
  "planet": "Yavin IV",
  "habitat": "Jungle",
  "dex": "1D",
  "per": "2D",
  "str": "1D",
  "move": "12 (flying)",
  "size": "5cm long",
  "orneriness": "",
  "special": "Swarm: 3D+2, multiple targets, can't be blocked. Only evasion or gas works.",
  "source": "Jedi Academy Sourcebook (pages 121-122)"
 },
 {
  "name": "Piscator",
  "type": "Force-sensitive predator",
  "planet": "Alluuvia",
  "habitat": "Aquatic",
  "dex": "2D+2",
  "per": "2D",
  "str": "3D",
  "move": "12",
  "size": "3m long",
  "orneriness": "",
  "special": "Tusks: STR+3D. Tentacles: 8\u00d7 STR+1D each. Fear-Sensitivity: +1D brawling per fearful character.",
  "source": "Operation: Elrood (page 38)"
 },
 {
  "name": "Preducor",
  "type": "Nocturnal predator",
  "planet": "Endor",
  "habitat": "Forest",
  "dex": "4D",
  "per": "3D",
  "str": "6D",
  "move": "8",
  "size": "4m tall, 5m long",
  "orneriness": "",
  "special": "Teeth: 7D damage. Claws: 6D damage. Tail: 5D+1 damage. Sneak: forest 4D. Stamina 8D. Fights until wounded, then leaps back into forest to sneak around for a surprise attack.",
  "source": "WEG40126 Galaxy Guide 5: Return of the Jedi p.82"
 },
 {
  "name": "Preying Makthier",
  "type": "Cave-dwelling carnivore",
  "planet": "Makthierse",
  "habitat": "Cave/Subterranean",
  "dex": "3D+1",
  "per": "2D+1",
  "str": "3D+1",
  "move": "10 (flying)",
  "size": "1.2-1.7m long, 2m wingspan",
  "orneriness": "",
  "special": "Search 4D, sneak 4D+2. Lifting 5D+1. Sonic Motion Detection: blind in normal light, echolocation for 3D tracking, +1D+2 to all search rolls. Constriction: tail wraps victim, +1D lifting, 1D damage/round, -2D DEX penalty, Very Difficult STR to escape. Paralyzing Stinger: 2D damage (barb) + 4D stun damage for 5 successive rounds. Hunts in groups of 10+. Creature scale.",
  "source": "WEG40080 Creatures of the Galaxy p.52"
 },
 {
  "name": "Purbole",
  "type": "Small herd animal",
  "planet": "Dathomir",
  "habitat": "Forest",
  "dex": "4D+1",
  "per": "2D+2",
  "str": "1D+2",
  "move": "8",
  "size": "Small",
  "orneriness": "",
  "special": "Exceptional Reflexes: +1D initiative.",
  "source": "Geonosis and the Outer Rim Worlds (page 34)"
 },
 {
  "name": "Purella",
  "type": "Predatory arachnid",
  "planet": "Yavin 8",
  "habitat": "Cave",
  "dex": "4D",
  "per": "2D",
  "str": "2D",
  "move": "12",
  "size": "1.5-2m tall",
  "orneriness": "",
  "special": "Infravision. Pincers: STR+1D. Silent Movement: +1D+2 sneak. Spin Web: Easy STR first turn, +1 difficulty each subsequent. Venom: Difficult stamina or paralyzed up to 1 hour.",
  "source": "Galaxy Guide 2 (page 24)"
 },
 {
  "name": "Qormot",
  "type": "Forest omnivore",
  "planet": "Yeshocq",
  "habitat": "Forest",
  "dex": "2D+1",
  "per": "2D",
  "str": "2D+1",
  "move": "8 (walking), 14 (charging)",
  "size": "1.2 meters long, 1 meter tall at the shoulder",
  "orneriness": "",
  "special": "Running 5D+1. Search 5D, sneak 4D. Brawling 6D+1. Claws: STR+1 damage. Teeth: STR+2 damage. Quills: 3D damage close (0-4m), 2D damage medium/long (5-8/12m). Restricted Vision: single eye, targets beyond 5m get +1D dodge vs quills. Territorial: violent reaction to intruders, heightened in mating season. Creature scale.",
  "source": "WEG40080 Creatures of the Galaxy p.55"
 },
 {
  "name": "Quamin",
  "type": "Vicious flying menace",
  "planet": "Kidron",
  "habitat": "Various",
  "dex": "4D",
  "per": "2D",
  "str": "3D",
  "move": "16",
  "size": "1.7m wingspan, 2m long",
  "orneriness": "",
  "special": "Flight: +1D sneak approaching. Razor Tail Whip: STR+1D+2 (slow healing cuts). Sensing: smell-based, no penalty in smoke/darkness/weather.",
  "source": "WEG40080 Creatures of the Galaxy p.56"
 },
 {
  "name": "Quicker",
  "type": "Flying omnivore",
  "planet": "Kuras III",
  "habitat": "Various",
  "dex": "4D",
  "per": "4D",
  "str": "4D",
  "move": "14 (flying)",
  "size": "2m long, 4.5m wingspan",
  "orneriness": "",
  "special": "Beak: STR+1D. Talons: STR+2D.",
  "source": "Planets Collection (pages 183-184)"
 },
 {
  "name": "Quizzer",
  "type": "Scampering beast",
  "planet": "Gamorr",
  "habitat": "Forest",
  "dex": "4D",
  "per": "3D",
  "str": "1D",
  "move": "7 (running), 12 (climbing or swinging in trees)",
  "size": "0.5 meters tall",
  "orneriness": "",
  "special": "Climbing: +2D to sneaking and climbing rolls.",
  "source": "WEG40080 Creatures of the Galaxy p.57"
 },
 {
  "name": "Quosit",
  "type": "Fine parasite",
  "planet": "Rhommamool",
  "habitat": "Various",
  "dex": "8D+1",
  "per": "1D+1",
  "str": "1D",
  "move": "4/8 (flying)",
  "size": "25cm long",
  "orneriness": "",
  "special": "Exoskeleton: +1 resist. Parasitic Attack: lays eggs in sleeping hosts, 1D days incubation, -1D STR on hatching (Difficult first aid to prevent). Blindsight: 20m. Camouflage: +2 hide in rock/underground. Scent: detect 10m (20m upwind).",
  "source": "Ultimate Adversaries (pages 106-107)"
 },
 {
  "name": "Radark",
  "type": "Medium subterranean vermin",
  "planet": "Bothawui",
  "habitat": "Subterranean",
  "dex": "4D",
  "per": "3D+2",
  "str": "1D+1",
  "move": "12/8 (burrowing)",
  "size": "2-3m long",
  "orneriness": "",
  "special": "Bite: STR+1. Acid-Coated Spines: STR+1D+2. Acid Resistance: +2D. Darkvision: 20m.",
  "source": "Ultimate Adversaries (page 108)"
 },
 {
  "name": "Raen Sovra",
  "type": "Electricity parasite",
  "planet": "",
  "habitat": "Space",
  "dex": "4D+2",
  "per": "1D",
  "str": "2D+2",
  "move": "5",
  "size": "Up to 8 meters long, up to 5 cm diameter",
  "orneriness": "",
  "special": "Electricity Sense: +3D PER for all tasks involving perception of electricity. Electrocution: 5D+2 damage on physical contact. Space Survival: unharmed by cold or vacuum, survives in deep space indefinitely. Creature scale.",
  "source": "WEG40080 Creatures of the Galaxy p.58"
 },
 {
  "name": "Rakghoul",
  "type": "Infected predator",
  "planet": "Taris",
  "habitat": "Urban/Subterranean",
  "dex": "3D+2",
  "per": "2D",
  "str": "4D",
  "move": "10",
  "size": "Various",
  "orneriness": "",
  "special": "Low-light Vision. Rakghoul Disease: VD stamina or infected, VD stamina/hour or -1 STR until 0D = death + transformation. Exceptional Reflexes: +1D initiative.",
  "source": ""
 },
 {
  "name": "Rakghoul Fiend",
  "type": "Infected predator",
  "planet": "Taris",
  "habitat": "Urban/Subterranean",
  "dex": "5D",
  "per": "1D+1",
  "str": "5D",
  "move": "10",
  "size": "Various",
  "orneriness": "",
  "special": "Low-light Vision. Rakghoul Disease (same as above). Exceptional Reflexes: +1D initiative. Uses weapons.",
  "source": ""
 },
 {
  "name": "Ral",
  "type": "Rept-avian carnivore",
  "planet": "Bothawui",
  "habitat": "Forest",
  "dex": "2D",
  "per": "3D",
  "str": "1D",
  "move": "15 (flying)",
  "size": "70cm long",
  "orneriness": "",
  "special": "Claws and Teeth: STR+1D. Camouflage: VD search when sunbathing. Poison Stinger: 3D, stun+ = Moderate stamina or coma (15 min without antidote = VD first aid or death in 20hr).",
  "source": "Shadows of the Empire Planets Guide (page 63)"
 },
 {
  "name": "Rancor",
  "type": "Gigantic predator",
  "planet": "Dathomir",
  "habitat": "Forest/Cave, Jabba's Palace",
  "dex": "4D",
  "per": "1D",
  "str": "7D",
  "move": "20",
  "size": "5m tall",
  "orneriness": "",
  "special": "Claws: STR+3D. Teeth: STR+5D. Armor: +3D physical and energy.",
  "source": "Core Rulebook (page 224)"
 },
 {
  "name": "Ranphyx",
  "type": "Intelligent herd predator",
  "planet": "Elom",
  "habitat": "Various",
  "dex": "3D",
  "per": "3D",
  "str": "3D",
  "move": "10 (running), 12 (leaping)",
  "size": "1.5-2 meters long, 0.8 meters tall at the shoulder",
  "orneriness": "",
  "special": "Senses: +2D PER for smell and hearing. Horns: STR+1D gore. Stealth: +2D to tracking, hiding, and sneaking. Creature scale.",
  "source": "WEG40080 Creatures of the Galaxy p.60"
 },
 {
  "name": "Rawwk",
  "type": "Flying scavenger",
  "planet": "Bespin",
  "habitat": "Atmosphere",
  "dex": "3D+1",
  "per": "5D",
  "str": "1D+1",
  "move": "20 (flying)",
  "size": "0.7m wingspan",
  "orneriness": "",
  "special": "Claws: STR+1.",
  "source": "Galaxy Guide 3 (page 67)"
 },
 {
  "name": "Razort",
  "type": "",
  "planet": "",
  "habitat": "Various",
  "dex": "5D",
  "per": "3D",
  "str": "4D+2",
  "move": "",
  "size": "",
  "orneriness": "",
  "special": "Bite: 3D+1. Razor Tail: 5D. Quickness: 2 attacks count as 1 action.",
  "source": "Battle for the Golden Sun (pages 37-38)"
 },
 {
  "name": "Rearing Spider",
  "type": "Large desert vermin",
  "planet": "Moon of Endor",
  "habitat": "Forest",
  "dex": "2D",
  "per": "1D+1",
  "str": "3D+2",
  "move": "10",
  "size": "Large",
  "orneriness": "",
  "special": "Bite: STR+1.",
  "source": "Ultimate Adversaries (page 120)"
 },
 {
  "name": "Red Miont",
  "type": "Dangerous fish",
  "planet": "Cols",
  "habitat": "Aquatic",
  "dex": "1D",
  "per": "2D",
  "str": "1D",
  "move": "8",
  "size": "",
  "orneriness": "",
  "special": "Bite: STR+2 damage.",
  "source": "WEG40080 Creatures of the Galaxy p.42"
 },
 {
  "name": "Reek",
  "type": "",
  "planet": "",
  "habitat": "Plains",
  "dex": "1D+1",
  "per": "1D",
  "str": "6D",
  "move": "10",
  "size": "6m long",
  "orneriness": "",
  "special": "Enraged: +1D initiative when enraged. Gore: STR+1D. Thick Skin: +2D physical, +1D energy.",
  "source": ""
 },
 {
  "name": "Riding Lizard",
  "type": "",
  "planet": "",
  "habitat": "Various",
  "dex": "2D",
  "per": "2D",
  "str": "7D+1",
  "move": "14",
  "size": "5.3m long",
  "orneriness": "2D",
  "special": "Bite: STR+1D. Claws: STR+2. Force Immunity: vs Sense/Alter. Intractable: -3D beast riding for non-Yuuzhan Vong. Natural Armor: +1D+2 physical.",
  "source": "New Jedi Order Sourcebook (pages 102-103)"
 },
 {
  "name": "Rock Wart",
  "type": "",
  "planet": "",
  "habitat": "Desert",
  "dex": "3D+1",
  "per": "1D+1",
  "str": "0D+2",
  "move": "4",
  "size": "Diminutive",
  "orneriness": "",
  "special": "Venomous Bite: 5D neurotoxin. If victim dies/unconscious, lays 4D eggs (hatch in 4 days).",
  "source": "Secrets of Tatooine (pages 14-15)"
 },
 {
  "name": "Rolk-Mangir",
  "type": "",
  "planet": "Yinchorr",
  "habitat": "Various",
  "dex": "2D+2",
  "per": "2D+2",
  "str": "4D+1",
  "move": "12",
  "size": "4m tall",
  "orneriness": "",
  "special": "Energy Resistant: -8 from energy weapon damage. Exceptional Reflexes: +1D initiative. Gore: STR+2D. Good Hearing: +1D PER sounds. Low Light Vision.",
  "source": "Alien Anthology (page 41)"
 },
 {
  "name": "Ronto",
  "type": "",
  "planet": "Tatooine",
  "habitat": "Desert",
  "dex": "2D",
  "per": "3D",
  "str": "5D",
  "move": "10",
  "size": "4.25m tall",
  "orneriness": "1D",
  "special": "Heat endurance. Sense of smell: +1D PER (smell). Skittish: +3D orneriness near fast vehicles.",
  "source": "Star Wars Trilogy Sourcebook SE (page 178)"
 },
 {
  "name": "Runyip",
  "type": "Pack animal",
  "planet": "Tran Mariel",
  "habitat": "Various",
  "dex": "4D",
  "per": "3D",
  "str": "3D+2",
  "move": "9 (27 km/h)",
  "size": "1-1.4m at shoulder",
  "orneriness": "4D",
  "special": "Kick: STR+2D.",
  "source": "Rules of Engagement (page 54)"
 },
 {
  "name": "Salt Mynock",
  "type": "Desert parasite",
  "planet": "Crait",
  "habitat": "Desert",
  "dex": "3D",
  "per": "1D+1",
  "str": "1D+2",
  "move": "9/1 (space)",
  "size": "1.5m wide",
  "orneriness": "",
  "special": "Energy Drain: drain from vehicles/machinery. Flight. Salt Encrusted: +1D physical.",
  "source": ""
 },
 {
  "name": "Sand Beetle",
  "type": "",
  "planet": "Tatooine",
  "habitat": "Desert",
  "dex": "2D",
  "per": "1D",
  "str": "2D",
  "move": "6",
  "size": "0.5m long",
  "orneriness": "",
  "special": "Bite: STR+1D. Burrow: Move 4. Swarm: 4D damage in groups of 20+.",
  "source": "Secrets of Tatooine (page 93)"
 },
 {
  "name": "Sand Bear",
  "type": "Carnivore",
  "planet": "Kallistas",
  "habitat": "Desert",
  "dex": "3D+2",
  "per": "3D",
  "str": "3D",
  "move": "12 (under sand), 9 (on surface)",
  "size": "",
  "orneriness": "",
  "special": "Silent sand movement: +2D to sneak when moving under sand. Claws: STR+1D+2 damage. Primarily nocturnal ambush predators; will attack in daylight when food is scarce. Presently the only known carnivore on Kallistas.",
  "source": "WEG40073 Galaxy Guide 10: Bounty Hunters p.105"
 },
 {
  "name": "Sand Burrower",
  "type": "Aquatic/subterranean predator",
  "planet": "Mon Calamari",
  "habitat": "Aquatic/Subterranean",
  "dex": "2D+1",
  "per": "2D",
  "str": "6D",
  "move": "16 (burrowing)",
  "size": "4.5m long",
  "orneriness": "",
  "special": "Bite: STR+2D. Tremorsense: +2D search underground. Ambush: +2D sneak while burrowed.",
  "source": ""
 },
 {
  "name": "Sand Panther",
  "type": "Desert predator",
  "planet": "Tatooine",
  "habitat": "Desert",
  "dex": "4D",
  "per": "3D",
  "str": "3D+1",
  "move": "16",
  "size": "2m long",
  "orneriness": "",
  "special": "Claws: STR+1D. Bite: STR+2. Camouflage: +2D sneak in desert.",
  "source": "Secrets of Tatooine (pages 94-95)"
 },
 {
  "name": "Sandwhirl",
  "type": "Silicon predator",
  "planet": "Trinta",
  "habitat": "Desert",
  "dex": "4D",
  "per": "1D+2",
  "str": "4D+2",
  "move": "20 (flying)",
  "size": "2m tall",
  "orneriness": "",
  "special": "Sand Blast: 5D, 0-3/5/10m range. Envelop: 4D+2 each round, Difficult STR to escape. Energy Immune: immune to blasters.",
  "source": "Domain of Evil (pages 23-24)"
 },
 {
  "name": "Sarlacc",
  "type": "Solitary, stationary carnivore",
  "planet": "Tatooine",
  "habitat": "Desert, Great Pit of Carkoon",
  "dex": "4D",
  "per": "4D",
  "str": "5D",
  "move": "0",
  "size": "100m diameter pit",
  "orneriness": "",
  "special": "Tentacles: STR+2D, 4m range. Beak: STR+3D. Digestive Juices: 5D/round. 1000 years to digest. Difficult PER to see. Walker Scale.",
  "source": "Core Rulebook (page 226)"
 },
 {
  "name": "Sathog",
  "type": "Jungle herbivore",
  "planet": "Lan Barell",
  "habitat": "Jungle",
  "dex": "3D+2",
  "per": "2D+1",
  "str": "6D",
  "move": "12",
  "size": "2.5-3m tall, 3-5m long",
  "orneriness": "",
  "special": "Gore: STR+2D. Trample: STR+1D.",
  "source": "Creatures of the Galaxy (pages 60-61)"
 },
 {
  "name": "Savrip",
  "type": "Dangerous reptilian",
  "planet": "",
  "habitat": "Mountain/Cave",
  "dex": "3D+1",
  "per": "2D",
  "str": "6D",
  "move": "12",
  "size": "3-5m tall",
  "orneriness": "",
  "special": "Bite: STR+2D. Claws: STR+1D+2. Armor: +2D physical, +1D energy. Rock Throw: STR+1D, range 5-10/30/60.",
  "source": ""
 },
 {
  "name": "Scaly Tauntaun",
  "type": "",
  "planet": "Hoth",
  "habitat": "Arctic",
  "dex": "2D+2",
  "per": "3D+1",
  "str": "4D",
  "move": "16",
  "size": "1.8m tall",
  "orneriness": "2D+2",
  "special": "Head Butt: STR+1D. Kick: STR+2. Cold Resistant. Natural Armor: +1D physical.",
  "source": "Galaxy Guide 3 (page 66)"
 },
 {
  "name": "Scimitar Horned Tusk-Cat",
  "type": "",
  "planet": "Naboo",
  "habitat": "Plains",
  "dex": "2D",
  "per": "2D",
  "str": "3D",
  "move": "10",
  "size": "2m long",
  "orneriness": "",
  "special": "Claws: STR+1D. Tusks: STR+2D. Exceptional Reflexes: +1D initiative.",
  "source": "Secrets of Naboo (pages 59-60)"
 },
 {
  "name": "Scrange",
  "type": "Scavenger",
  "planet": "Goroth",
  "habitat": "Desert",
  "dex": "3D+1",
  "per": "2D+2",
  "str": "2D+1",
  "move": "8/14 (flight)",
  "size": "0.7-1.3m wingspan",
  "orneriness": "",
  "special": "Beak: STR+1D. Filthy Claws: STR+1 + Moderate stamina or infection (-1 STR/day until treated). Keen Sight: +2D search (visual).",
  "source": "Goroth, Slave of the Empire (page 93)"
 },
 {
  "name": "Sea Leviathan",
  "type": "",
  "planet": "Naboo",
  "habitat": "Aquatic",
  "dex": "2D",
  "per": "2D+2",
  "str": "12D",
  "move": "52 (150 km/h)",
  "size": "100m long",
  "orneriness": "",
  "special": "Bite: STR+3D. Low Light Vision. Swallow: Walker Scale. Tremorsense: 60m.",
  "source": "Secrets of Naboo (pages 60-61)"
 },
 {
  "name": "Seftin",
  "type": "Swamp mammal",
  "planet": "Cekbar",
  "habitat": "Swamp",
  "dex": "4D+2",
  "per": "3D+1",
  "str": "2D+2",
  "move": "8/12 (swim)",
  "size": "1-1.6m long",
  "orneriness": "",
  "special": "Teeth: STR+1D. Resistance to Gases: +2D STR vs gases. Stealth: +1D sneak in water.",
  "source": "Creatures of the Galaxy (pages 61-62)"
 },
 {
  "name": "Seluss",
  "type": "Herd browser",
  "planet": "Bothawui",
  "habitat": "Plains/Forest",
  "dex": "2D",
  "per": "3D",
  "str": "3D",
  "move": "14",
  "size": "1.3m at shoulder",
  "orneriness": "2D",
  "special": "Gore: STR+1D. Trample: STR+2 (prone only). Perception: +2D hearing.",
  "source": "Shadows of the Empire Planets Guide (page 62)"
 },
 {
  "name": "Shaak",
  "type": "Herd herbivore",
  "planet": "Naboo",
  "habitat": "Plains",
  "dex": "1D",
  "per": "2D",
  "str": "3D",
  "move": "6",
  "size": "2m long, 1.5m tall",
  "orneriness": "",
  "special": "Trample: STR+1. Docile: rarely attacks.",
  "source": "Secrets of Naboo (page 61)"
 },
 {
  "name": "Shadow Creature",
  "type": "Force predator",
  "planet": "Trinta",
  "habitat": "Various",
  "dex": "6D",
  "per": "1D+2",
  "str": "6D+2",
  "move": "18 (flying)",
  "size": "2m tall",
  "orneriness": "",
  "special": "Shadow Attack: 5D. Immune: physical attacks pass through. Vulnerable: Force/energy only. Force-Sensitive (Sense 3D, Alter 4D). Fear Aura: Moderate Willpower or freeze.",
  "source": "Domain of Evil (pages 24-25)"
 },
 {
  "name": "Shalk",
  "type": "",
  "planet": "Geonosis",
  "habitat": "Desert",
  "dex": "1D+1",
  "per": "1D+2",
  "str": "2D",
  "move": "5",
  "size": "0.5m tall",
  "orneriness": "",
  "special": "Bite: STR+1. Bioluminescence.",
  "source": ""
 },
 {
  "name": "Shlimik",
  "type": "Large cave predator",
  "planet": "Dathomir",
  "habitat": "Cave",
  "dex": "3D+1",
  "per": "2D+1",
  "str": "6D+1",
  "move": "14",
  "size": "3m long",
  "orneriness": "",
  "special": "Claws: STR+2D. Low-Light Vision. Keen Smell: +1D+2 search by scent.",
  "source": "Geonosis and the Outer Rim Worlds (page 34)"
 },
 {
  "name": "Shyracks",
  "type": "Cave roosting bat",
  "planet": "Korriban",
  "habitat": "Cave",
  "dex": "3D+1",
  "per": "3D",
  "str": "1D",
  "move": "18 (flying)",
  "size": "0.6m wingspan",
  "orneriness": "",
  "special": "Bite: STR+1. Screech: 2D stun, 5m radius. Swarm Tactics: +2D attack in groups of 10+.",
  "source": ""
 },
 {
  "name": "Siltcrawler",
  "type": "Desert omnivore",
  "planet": "Tatooine",
  "habitat": "Desert",
  "dex": "1D",
  "per": "1D+2",
  "str": "3D+2",
  "move": "8",
  "size": "2.5m long",
  "orneriness": "",
  "special": "Mandibles: STR+1D+2. Armor: +1D+2 physical, +1 energy. Burrow: Move 4.",
  "source": "Secrets of Tatooine (page 95)"
 },
 {
  "name": "Skyclaw",
  "type": "Large flying predator",
  "planet": "",
  "habitat": "Forest/Mountain",
  "dex": "3D+2",
  "per": "3D",
  "str": "4D",
  "move": "5/30 (flying)",
  "size": "3m wingspan",
  "orneriness": "",
  "special": "Talons: STR+2D. Beak: STR+1D+2. Dive Attack: +2D damage when diving.",
  "source": ""
 },
 {
  "name": "Slaatik Hagworm",
  "type": "",
  "planet": "",
  "habitat": "Swamp",
  "dex": "3D",
  "per": "1D+2",
  "str": "5D",
  "move": "10 (swim)/5 (land)",
  "size": "4-6m long",
  "orneriness": "",
  "special": "Bite: STR+2D. Constriction: STR+1D/round. Difficult PER to spot submerged.",
  "source": ""
 },
 {
  "name": "Slashrat",
  "type": "",
  "planet": "",
  "habitat": "Urban/Subterranean",
  "dex": "4D+1",
  "per": "2D",
  "str": "1D+2",
  "move": "10",
  "size": "0.6m long",
  "orneriness": "",
  "special": "Claws: STR+1D+1. Teeth: STR+2. Infection: Moderate stamina or disease.",
  "source": ""
 },
 {
  "name": "Sleeg",
  "type": "Slime beast",
  "planet": "Dagobah",
  "habitat": "Swamp",
  "dex": "1D",
  "per": "2D",
  "str": "3D",
  "move": "4",
  "size": "2m long",
  "orneriness": "",
  "special": "Acid Slime: 3D on contact. Teeth: STR+1D. Camouflage: +2D hide in swamp.",
  "source": "Galaxy Guide 3 (page 68)"
 },
 {
  "name": "Snapper",
  "type": "Ocean predator",
  "planet": "Mon Calamari",
  "habitat": "Aquatic",
  "dex": "3D+2",
  "per": "2D",
  "str": "3D",
  "move": "14 (swim)",
  "size": "1.5-2m long",
  "orneriness": "",
  "special": "Bite: STR+2D. Keen Smell: +2D search in water.",
  "source": ""
 },
 {
  "name": "Snow Demon",
  "type": "Arctic apex predator",
  "planet": "Toola",
  "habitat": "Arctic",
  "dex": "3D+1",
  "per": "4D",
  "str": "6D",
  "move": "14",
  "size": "3m tall",
  "orneriness": "",
  "special": "Claws: STR+2D. Bite: STR+1D+2. Camouflage: +3D sneak in snow. Darkvision: 60m.",
  "source": "Creatures of the Galaxy (pages 62-63)"
 },
 {
  "name": "Snow Falcon",
  "type": "Arctic raptor",
  "planet": "Toola",
  "habitat": "Arctic",
  "dex": "4D",
  "per": "3D+2",
  "str": "2D",
  "move": "5/25 (flying)",
  "size": "1.5m wingspan",
  "orneriness": "",
  "special": "Talons: STR+1D+2. Beak: STR+1D. Keen Sight: +2D search visual.",
  "source": ""
 },
 {
  "name": "Sochi",
  "type": "Long-necked grazer",
  "planet": "Ando",
  "habitat": "Plains",
  "dex": "2D+2",
  "per": "3D+1",
  "str": "3D+2",
  "move": "12",
  "size": "2.8m tall",
  "orneriness": "",
  "special": "Perception: +2D search. Kick: STR+2.",
  "source": "Creatures of the Galaxy (pages 63-64)"
 },
 {
  "name": "Space Slug",
  "type": "",
  "planet": "",
  "habitat": "Space",
  "dex": "1D",
  "per": "1D",
  "str": "7D",
  "move": "2 (space)",
  "size": "10-900m long",
  "orneriness": "",
  "special": "Swallow: can engulf ships. Silicon life form: vacuum survival. Capital Scale (large ones). Corrosive Saliva: 4D/round inside.",
  "source": "Core Rulebook (page 227)"
 },
 {
  "name": "Spice Spider",
  "type": "Web-spinning arachnid",
  "planet": "Kessel",
  "habitat": "Subterranean",
  "dex": "3D+2",
  "per": "3D+1",
  "str": "5D+2",
  "move": "16",
  "size": "3-4m long",
  "orneriness": "",
  "special": "Bite: STR+1D + paralytic venom (Moderate stamina or paralyzed 2D hours). Web: 5D entangle, 15m range. Darkvision: 30m. Glitterstim Production: webs = raw glitterstim.",
  "source": ""
 },
 {
  "name": "Spined Puc",
  "type": "Burrowing herbivore",
  "planet": "Algara II",
  "habitat": "Subterranean",
  "dex": "2D",
  "per": "3D",
  "str": "3D",
  "move": "6/8 (burrowing)",
  "size": "1.5m long",
  "orneriness": "",
  "special": "Quills: STR+1D (on contact). Burrow: Move 8.",
  "source": "Creatures of the Galaxy (pages 64-65)"
 },
 {
  "name": "Spore Creature",
  "type": "Subterranean fungal beast",
  "planet": "Trinta",
  "habitat": "Subterranean/Cave",
  "dex": "3D+2",
  "per": "1D",
  "str": "5D",
  "move": "8",
  "size": "2m tall",
  "orneriness": "",
  "special": "Slam: STR+1D. Spore Cloud: 3m radius, 3D stun + Moderate stamina or coughing (-1D all) for 1D rounds. Immune: mind-affecting. Vulnerable: fire (double damage).",
  "source": "Domain of Evil (page 25)"
 },
 {
  "name": "Squall",
  "type": "Forest creature",
  "planet": "Yavin IV",
  "habitat": "Forest",
  "dex": "3D",
  "per": "2D",
  "str": "2D",
  "move": "10/14 (climbing)",
  "size": "1m long",
  "orneriness": "",
  "special": "Claws: STR+1D. Bite: STR+2. Climbing: +2D climbing.",
  "source": "Galaxy Guide 2 (page 25)"
 },
 {
  "name": "Ssank",
  "type": "",
  "planet": "Kashyyyk",
  "habitat": "Forest",
  "dex": "3D",
  "per": "3D",
  "str": "5D",
  "move": "12",
  "size": "4m long",
  "orneriness": "",
  "special": "Bite: STR+2D. Constriction: STR+1D/round. Camouflage: +2D hide in wroshyr trees.",
  "source": ""
 },
 {
  "name": "Staga",
  "type": "Plains bison",
  "planet": "Ambria",
  "habitat": "Plains",
  "dex": "2D+1",
  "per": "2D",
  "str": "5D",
  "move": "14",
  "size": "2m tall, 3m long",
  "orneriness": "2D",
  "special": "Gore: STR+1D+2. Trample: STR+1D. Herd Stampede: 8D damage if caught in path.",
  "source": "WEG40082 Tales of the Jedi Companion p.109"
 },
 {
  "name": "Stalker Lizard",
  "type": "",
  "planet": "",
  "habitat": "Various",
  "dex": "3D+2",
  "per": "3D",
  "str": "2D+1",
  "move": "12",
  "size": "1.5m long",
  "orneriness": "",
  "special": "Bite: STR+1D+2. Chameleon Skin: +2D sneak. Tail Whip: STR+1.",
  "source": ""
 },
 {
  "name": "Stiletto Bug",
  "type": "Tiny venomous insect",
  "planet": "",
  "habitat": "Various",
  "dex": "4D",
  "per": "1D",
  "str": "0D+1",
  "move": "8 (flying)",
  "size": "3cm long",
  "orneriness": "",
  "special": "Sting: 4D poison (VD stamina or -2D all for 1D hours). Swarm: 6D sting in groups.",
  "source": ""
 },
 {
  "name": "Stintaril",
  "type": "Swamp scavenger",
  "planet": "Yavin IV",
  "habitat": "Swamp",
  "dex": "4D+1",
  "per": "2D+2",
  "str": "1D+1",
  "move": "8",
  "size": "0.3m long",
  "orneriness": "",
  "special": "Teeth: STR+2. Pack Tactics: +1D per 2 additional stintarils (max +3D). Venomous Bite: Moderate stamina or ill 1D days.",
  "source": "Galaxy Guide 2 (page 25)"
 },
 {
  "name": "Stone Mite",
  "type": "Silicon creature",
  "planet": "",
  "habitat": "Space/Subterranean",
  "dex": "3D",
  "per": "2D",
  "str": "1D+2",
  "move": "5",
  "size": "0.3-0.5m",
  "orneriness": "",
  "special": "Acid: 4D vs hull/materials. Silicon: vacuum survival. Armor: +2D physical.",
  "source": ""
 },
 {
  "name": "Storm Beast",
  "type": "",
  "planet": "Malachor V",
  "habitat": "Various",
  "dex": "3D+2",
  "per": "2D",
  "str": "8D",
  "move": "16",
  "size": "5m tall",
  "orneriness": "",
  "special": "Claws: STR+2D. Lightning Aura: 4D energy, 3m radius. Natural Armor: +2D. Force Attuned.",
  "source": ""
 },
 {
  "name": "Stratt",
  "type": "Grazers",
  "planet": "",
  "habitat": "Plains",
  "dex": "1D",
  "per": "2D",
  "str": "3D",
  "move": "14",
  "size": "Large",
  "orneriness": "",
  "special": "Kick: STR+1D. Herd: stampede 6D.",
  "source": ""
 },
 {
  "name": "Striped Wrix",
  "type": "Jungle cat",
  "planet": "Corellia",
  "habitat": "Jungle/Forest",
  "dex": "4D",
  "per": "3D+1",
  "str": "3D+2",
  "move": "18",
  "size": "1.5m long",
  "orneriness": "",
  "special": "Claws: STR+1D+1. Bite: STR+2. Keen Hearing: +1D PER sounds. Stealthy: +2D sneak.",
  "source": "Creatures of the Galaxy (pages 65-66)"
 },
 {
  "name": "Stunworm",
  "type": "Ambush predator",
  "planet": "",
  "habitat": "Various",
  "dex": "2D",
  "per": "2D",
  "str": "5D+2",
  "move": "8",
  "size": "4-6m long",
  "orneriness": "",
  "special": "Electric Shock: 5D stun, 3m range. Bite: STR+1D+2. Constriction: STR/round.",
  "source": ""
 },
 {
  "name": "Sugarbug",
  "type": "",
  "planet": "",
  "habitat": "Various",
  "dex": "2D",
  "per": "1D",
  "str": "0D+1",
  "move": "6 (flying)",
  "size": "Diminutive",
  "orneriness": "",
  "special": "Sweet Secretion: edible, harvested as sweetener.",
  "source": ""
 },
 {
  "name": "Sukka",
  "type": "",
  "planet": "Kashyyyk",
  "habitat": "Forest",
  "dex": "4D+2",
  "per": "3D",
  "str": "3D+1",
  "move": "12/16 (climbing)",
  "size": "1.5m long",
  "orneriness": "",
  "special": "Bite: STR+1D+2. Claws: STR+1D. Climbing: +2D. Keen Sight: +1D search visual.",
  "source": ""
 },
 {
  "name": "Swamp Lizard",
  "type": "",
  "planet": "Dagobah",
  "habitat": "Swamp",
  "dex": "2D+2",
  "per": "2D",
  "str": "3D+1",
  "move": "6/12 (swimming)",
  "size": "2m long",
  "orneriness": "",
  "special": "Bite: STR+1D. Camouflage: +1D+2 hide in swamp.",
  "source": ""
 },
 {
  "name": "Swamp Slug",
  "type": "",
  "planet": "Dagobah",
  "habitat": "Swamp",
  "dex": "1D",
  "per": "1D",
  "str": "4D",
  "move": "3",
  "size": "3m long",
  "orneriness": "",
  "special": "Acid Slime: 2D on contact. Teeth: STR+2. Camouflage: +2D hide in murky water.",
  "source": "Galaxy Guide 3 (page 68)"
 },
 {
  "name": "Swamp Wampa",
  "type": "",
  "planet": "Dromund Kaas",
  "habitat": "Swamp",
  "dex": "3D",
  "per": "3D+2",
  "str": "6D",
  "move": "12/8 (swimming)",
  "size": "2.8m tall",
  "orneriness": "",
  "special": "Claws: STR+2D. Bite: STR+1D. Camouflage: +1D+2 sneak in swamp. Amphibious.",
  "source": ""
 },
 {
  "name": "Swamp Worm",
  "type": "",
  "planet": "Dagobah",
  "habitat": "Swamp",
  "dex": "2D",
  "per": "1D+1",
  "str": "6D",
  "move": "5/10 (swimming)",
  "size": "6-10m long",
  "orneriness": "",
  "special": "Bite: STR+2D. Constriction: STR+1D/round. Ambush: +2D sneak when submerged.",
  "source": "Galaxy Guide 3 (page 69)"
 },
 {
  "name": "Sycophant",
  "type": "Symbiotic insect",
  "planet": "",
  "habitat": "Various",
  "dex": "2D+2",
  "per": "3D",
  "str": "0D+2",
  "move": "8 (flying)",
  "size": "10-20cm",
  "orneriness": "",
  "special": "Symbiosis: attaches to host, +1D PER for host. Detach Shock: 2D stun to host on removal.",
  "source": "Creatures of the Galaxy (pages 66-67)"
 },
 {
  "name": "Taozin",
  "type": "Force-resistant predator",
  "planet": "Va'art",
  "habitat": "Subterranean",
  "dex": "2D",
  "per": "3D",
  "str": "9D",
  "move": "10",
  "size": "12m long",
  "orneriness": "",
  "special": "Bite: STR+2D. Tentacles: STR+1D, 6m reach. Force Stealth: invisible to Force Sense. Natural Armor: +3D physical and energy. Swallow Whole: 5D acid/round. Walker Scale.",
  "source": ""
 },
 {
  "name": "Taunling",
  "type": "Young tauntaun",
  "planet": "Hoth",
  "habitat": "Arctic",
  "dex": "3D",
  "per": "3D",
  "str": "2D",
  "move": "10",
  "size": "1m tall",
  "orneriness": "2D",
  "special": "Head Butt: STR+1. Kick: STR+2.",
  "source": ""
 },
 {
  "name": "Tauntaun",
  "type": "",
  "planet": "Hoth",
  "habitat": "Arctic",
  "dex": "2D+1",
  "per": "3D+2",
  "str": "4D+1",
  "move": "15",
  "size": "2m tall, 2.5m long",
  "orneriness": "3D",
  "special": "Head Butt: STR+1D. Kick: STR+2. Cold Resistant.",
  "source": "Core Rulebook (page 228)"
 },
 {
  "name": "Teek",
  "type": "Fast-moving creature",
  "planet": "Moon of Endor",
  "habitat": "Forest",
  "dex": "5D",
  "per": "3D",
  "str": "1D+1",
  "move": "25",
  "size": "1m tall",
  "orneriness": "",
  "special": "Exceptional Speed: Move 25. Blur: +2D dodge due to speed.",
  "source": ""
 },
 {
  "name": "Tekret",
  "type": "Nocturnal scavenger",
  "planet": "Dalicron-4",
  "habitat": "Various",
  "dex": "3D+1",
  "per": "3D+2",
  "str": "2D+2",
  "move": "10",
  "size": "1-1.3m long",
  "orneriness": "",
  "special": "Bite: STR+1D. Darkvision: 30m. Scavenger: +1D search for food/carrion. Pack: +1D per 3 additional (max +3D).",
  "source": "Creatures of the Galaxy (pages 67-68)"
 },
 {
  "name": "Terrorsquid",
  "type": "Ocean apex predator",
  "planet": "Mon Calamari",
  "habitat": "Aquatic",
  "dex": "3D",
  "per": "2D+1",
  "str": "8D",
  "move": "25 (swimming)",
  "size": "15m long",
  "orneriness": "",
  "special": "Tentacles: 8\u00d7 STR. Beak: STR+3D. Ink Cloud: 10m radius, -3D vision for 1D rounds. Walker Scale.",
  "source": ""
 },
 {
  "name": "Thaelos",
  "type": "Mountain lizard",
  "planet": "Alluuvia",
  "habitat": "Mountain",
  "dex": "2D",
  "per": "2D+2",
  "str": "3D+2",
  "move": "10/8 (climbing)",
  "size": "2m long",
  "orneriness": "3D",
  "special": "Bite: STR+1D. Claws: STR+2. Sure-footed: +1D climbing.",
  "source": "Operation: Elrood (page 38)"
 },
 {
  "name": "Thernbee",
  "type": "",
  "planet": "Anoat",
  "habitat": "Various",
  "dex": "3D",
  "per": "2D",
  "str": "4D",
  "move": "12",
  "size": "2m tall",
  "orneriness": "",
  "special": "Horns: STR+2D. Trample: STR+1D. Herd Stampede: 7D.",
  "source": ""
 },
 {
  "name": "Thi-Thi",
  "type": "Flying primate",
  "planet": "Koba",
  "habitat": "Forest",
  "dex": "3D+2",
  "per": "3D",
  "str": "1D+2",
  "move": "6/14 (flying)",
  "size": "0.5m tall",
  "orneriness": "",
  "special": "Teeth: STR+1. Group Tactics: swarm to steal objects. Flight.",
  "source": ""
 },
 {
  "name": "Thranta",
  "type": "",
  "planet": "Alderaan/Bespin",
  "habitat": "Atmosphere",
  "dex": "3D+1",
  "per": "2D",
  "str": "5D",
  "move": "30 (flying)",
  "size": "6-10m wingspan",
  "orneriness": "2D",
  "special": "Tail Slap: STR+1D. Mounts: ridden by Alderaanian nobility. Speeder Scale.",
  "source": ""
 },
 {
  "name": "Tidal Bore Squid",
  "type": "Ocean predator",
  "planet": "Phindar",
  "habitat": "Aquatic",
  "dex": "2D+1",
  "per": "2D",
  "str": "6D",
  "move": "18 (swimming)",
  "size": "8m long",
  "orneriness": "",
  "special": "Tentacles: STR. Beak: STR+2D. Jet Propulsion: burst speed 30.",
  "source": ""
 },
 {
  "name": "Tirginni Herd Beast",
  "type": "Large herbivore",
  "planet": "Naboo",
  "habitat": "Plains",
  "dex": "2D",
  "per": "2D",
  "str": "4D",
  "move": "10",
  "size": "3m long",
  "orneriness": "",
  "special": "Trample: STR+1D. Docile. Herd: stampede 7D.",
  "source": "Secrets of Naboo (page 61)"
 },
 {
  "name": "Tkath",
  "type": "Desert pack predator",
  "planet": "Tatooine",
  "habitat": "Desert",
  "dex": "3D",
  "per": "3D+1",
  "str": "2D+2",
  "move": "14",
  "size": "1.2m long",
  "orneriness": "",
  "special": "Bite: STR+1D. Pack Tactics: +1D per 2 additional (max +4D). Tracking: +2D search by scent.",
  "source": ""
 },
 {
  "name": "Tra'cor",
  "type": "Predatory worm",
  "planet": "Dathomir",
  "habitat": "Forest/Swamp",
  "dex": "2D",
  "per": "1D+2",
  "str": "5D+1",
  "move": "6",
  "size": "4m long",
  "orneriness": "",
  "special": "Bite: STR+1D+2. Swallow Whole: Medium or smaller, 4D acid/round. Tremorsense: 15m.",
  "source": "Geonosis and the Outer Rim Worlds (page 35)"
 },
 {
  "name": "Trandoshan T-Gator",
  "type": "Amphibious predator",
  "planet": "Trandosha",
  "habitat": "Swamp/Aquatic",
  "dex": "2D+2",
  "per": "2D+1",
  "str": "5D",
  "move": "8/14 (swimming)",
  "size": "4m long",
  "orneriness": "",
  "special": "Bite: STR+2D. Tail Slap: STR+1D. Natural Armor: +1D+2. Death Roll: grappled targets take STR+2D.",
  "source": ""
 },
 {
  "name": "Treetapper",
  "type": "Arboreal feeder",
  "planet": "Kashyyyk",
  "habitat": "Forest",
  "dex": "3D+1",
  "per": "2D+2",
  "str": "2D",
  "move": "8/12 (climbing)",
  "size": "1m long",
  "orneriness": "",
  "special": "Claws: STR+1 (+2D climbing). Beak: STR+2. Camouflage: +1D hide in wroshyr bark.",
  "source": ""
 },
 {
  "name": "Tresher",
  "type": "Floating gas creature",
  "planet": "Genarius",
  "habitat": "Atmosphere",
  "dex": "3D",
  "per": "1D",
  "str": "3D",
  "move": "10 (flying)",
  "size": "2-4m diameter",
  "orneriness": "",
  "special": "Tentacle Sting: STR+1D + 3D stun poison. Gas Bladder: buoyant. Camouflage: +2D hide in clouds.",
  "source": ""
 },
 {
  "name": "Tripion",
  "type": "Desert crustacean",
  "planet": "Yavin 13",
  "habitat": "Desert/Rocky plains",
  "dex": "3D+2",
  "per": "1D+2",
  "str": "1D–5D",
  "move": "12",
  "size": "1–18 meters long",
  "orneriness": "8",
  "special": "Pincers: seize and hold prey — target must make Moderate DEX roll or suffer -2D DEX; breaking free requires opposed STR roll beating tripion by 6+. Three venomous tails: all three can strike in the same turn; smaller species 1D–2D stun, larger species 3D–4D+1 regular damage. Over 600 species logged; nocturnal/crepuscular — shelters under stones by day. Will not attack character-sized prey unless provoked.",
  "source": "WEG40119 Galaxy Guide 2: Yavin and Bespin p.29"
 },
 {
  "name": "Tunnel Snakes",
  "type": "",
  "planet": "",
  "habitat": "Subterranean",
  "dex": "3D",
  "per": "2D",
  "str": "2D+2",
  "move": "10",
  "size": "2-3m long",
  "orneriness": "",
  "special": "Bite: STR+1D + Moderate stamina or venom (-1D DEX for 1D hours). Constriction: STR/round. Darkvision: 20m.",
  "source": ""
 },
 {
  "name": "Tusk Cat",
  "type": "",
  "planet": "Naboo",
  "habitat": "Plains/Forest",
  "dex": "3D+1",
  "per": "2D+2",
  "str": "4D",
  "move": "14",
  "size": "2.5m long",
  "orneriness": "",
  "special": "Claws: STR+1D. Tusks: STR+2D. Keen Senses: +1D search. Stealthy: +1D+2 sneak.",
  "source": "Secrets of Naboo (page 62)"
 },
 {
  "name": "Twilight Lizard",
  "type": "Small lizard",
  "planet": "Yavin 13",
  "habitat": "Desert/Rocky plains",
  "dex": "2D",
  "per": "1D+1",
  "str": "1D",
  "move": "8",
  "size": "0.6–1 meter long",
  "orneriness": "2",
  "special": "Teeth: STR+2. Claws: STR+2. Running 4D, Sneak 3D+1. Crepuscular — active only during twilight periods. Retains nearly its entire body weight as liquid; an excellent source of water and nourishment. Valuable trade commodity with the Slith (though this is not common knowledge).",
  "source": "WEG40119 Galaxy Guide 2: Yavin and Bespin p.28"
 },
 {
  "name": "Tynnan Furbog",
  "type": "Amphibious vermin",
  "planet": "Tynna",
  "habitat": "Aquatic/Swamp",
  "dex": "2D+1",
  "per": "2D",
  "str": "2D+1",
  "move": "6/10 (swimming)",
  "size": "0.8m long",
  "orneriness": "",
  "special": "Bite: STR+1D. Burrowing: Move 4. Territorial: attacks on sight near burrows.",
  "source": "Creatures of the Galaxy (pages 68-69)"
 },
 {
  "name": "Ubebrian Tusk-Cat",
  "type": "Arctic feline predator",
  "planet": "Ubebra",
  "habitat": "Arctic",
  "dex": "3D+2",
  "per": "3D",
  "str": "4D+1",
  "move": "16",
  "size": "2.5m long",
  "orneriness": "",
  "special": "Claws: STR+1D+1. Tusks: STR+2D. Cold Resistant. Keen Smell: +2D search by scent. Camouflage: +1D+2 sneak in snow.",
  "source": ""
 },
 {
  "name": "Ugnorrt",
  "type": "Moss-covered herbivore",
  "planet": "Kashyyyk",
  "habitat": "Forest",
  "dex": "1D+2",
  "per": "2D+1",
  "str": "5D+1",
  "move": "8",
  "size": "3m tall",
  "orneriness": "2D",
  "special": "Trample: STR+1D. Camouflage: +2D hide (moss-covered hide). Docile unless startled.",
  "source": ""
 },
 {
  "name": "Uller",
  "type": "Pack predator",
  "planet": "Tatooine",
  "habitat": "Desert",
  "dex": "3D+2",
  "per": "3D",
  "str": "3D",
  "move": "14",
  "size": "1.5m long",
  "orneriness": "",
  "special": "Bite: STR+1D+2. Pack Tactics: +1D per 2 additional (max +3D). Keen Smell: +1D search.",
  "source": "Secrets of Tatooine (page 96)"
 },
 {
  "name": "Ummarid",
  "type": "Hive insect",
  "planet": "Goroth",
  "habitat": "Various",
  "dex": "3D",
  "per": "2D",
  "str": "1D+2",
  "move": "8/12 (flying)",
  "size": "0.5-1m long",
  "orneriness": "",
  "special": "Sting: STR+1D + 3D poison (Difficult stamina or paralyzed 1D hrs). Swarm: 5D in groups of 10+. Hive Mind: coordinated attacks.",
  "source": "Goroth, Slave of the Empire (page 94)"
 },
 {
  "name": "Ur-Dian",
  "type": "",
  "planet": "Kashyyyk",
  "habitat": "Forest",
  "dex": "2D+2",
  "per": "2D+1",
  "str": "6D",
  "move": "10/8 (climbing)",
  "size": "3m tall",
  "orneriness": "",
  "special": "Claws: STR+2D. Bite: STR+1D+2. Climbing: +1D. Territorial: attacks all intruders.",
  "source": ""
 },
 {
  "name": "Ursod",
  "type": "Ursoid carnivore",
  "planet": "Yavin 8",
  "habitat": "Tundra / Ice cap",
  "dex": "4D",
  "per": "3D",
  "str": "5D",
  "move": "10",
  "size": "3m tall",
  "orneriness": "",
  "special": "Brawling 6D. Teeth: STR+1D. Claws: STR+1D. Thick hide: +2D STR for defense. Camouflage: chameleon-like fur shifts from white to mottled colors; +1D to sneak in snow or lichen terrain. Search 4D+1, Sneak 5D. Hunts by following songbuk herds on seasonal migrations.",
  "source": "WEG40119 Galaxy Guide 2: Yavin and Bespin p.24"
 },
 {
  "name": "Varactyl",
  "type": "Reptilian mount",
  "planet": "Utapau",
  "habitat": "Mountain/Cave",
  "dex": "3D+2",
  "per": "2D+2",
  "str": "4D+1",
  "move": "18/12 (climbing)",
  "size": "4m long, 2.2m tall",
  "orneriness": "2D",
  "special": "Claws: STR+1D. Beak: STR+1D+2. Climbing: +2D. Sure-footed: no terrain penalties on cliffs. Loyal: +1D beast riding once bonded.",
  "source": ""
 },
 {
  "name": "Veermok",
  "type": "",
  "planet": "Naboo",
  "habitat": "Forest/Swamp",
  "dex": "3D+1",
  "per": "2D+2",
  "str": "5D+2",
  "move": "12",
  "size": "2.5m tall",
  "orneriness": "",
  "special": "Claws: STR+1D+2. Bite: STR+1D. Exceptional Reflexes: +1D initiative. Climbing: +1D.",
  "source": "Secrets of Naboo (page 62)"
 },
 {
  "name": "Velker",
  "type": "Flying predator",
  "planet": "Bespin",
  "habitat": "Atmosphere",
  "dex": "4D",
  "per": "3D",
  "str": "3D+2",
  "move": "25 (flying)",
  "size": "6m wingspan",
  "orneriness": "",
  "special": "Talons: STR+2D. Electric Shock: 4D stun on contact. Speeder Scale.",
  "source": "Galaxy Guide 3 (page 67)"
 },
 {
  "name": "Velker (large)",
  "type": "Flying predator",
  "planet": "Bespin",
  "habitat": "Atmosphere",
  "dex": "2D",
  "per": "2D",
  "str": "4D",
  "move": "140 (400 km/h flying)",
  "size": "200–350 meters wingspan",
  "orneriness": "15",
  "special": "Energy Bolt: ionized electrical bolt — 10D stun damage; used to stun prey before landing. Teeth: STR+1D. Sneak 4D, Search 4D. Walker Scale. Packs of several dozen; electrolocation range 10km (100km in large groups). Attacks ships, cloud cars, and Cloud City itself. Feasts on beldons — lands along beldon surface, attaches with meter-long claws, feeds until beldon sinks to lethal altitude.",
  "source": "WEG40119 Galaxy Guide 2: Yavin and Bespin p.54–55"
 },
 {
  "name": "Vine Snake",
  "type": "Arboreal ambusher",
  "planet": "",
  "habitat": "Forest/Jungle",
  "dex": "3D+2",
  "per": "2D+2",
  "str": "2D",
  "move": "8/12 (climbing)",
  "size": "2-3m long",
  "orneriness": "",
  "special": "Bite: STR+1 + 4D poison (Difficult stamina or -2D DEX for 2D hours). Constriction: STR+1D/round. Camouflage: +2D+1 hide in trees/vines. VD PER to spot.",
  "source": ""
 },
 {
  "name": "Vine Stalker",
  "type": "Plant-animal hybrid predator",
  "planet": "Various",
  "habitat": "Jungle",
  "dex": "3D",
  "per": "3D+1",
  "str": "4D+2",
  "move": "6",
  "size": "3m tall",
  "orneriness": "",
  "special": "Vine Lash: STR+1D, 5m reach. Constriction: STR/round. Photosynthetic: heals 1D/day in sunlight. Camouflage: +3D hide in dense vegetation. Tremorsense: 10m.",
  "source": ""
 },
 {
  "name": "Void Spider",
  "type": "",
  "planet": "",
  "habitat": "Space",
  "dex": "3D+1",
  "per": "2D",
  "str": "4D",
  "move": "8/3 (space)",
  "size": "3m long",
  "orneriness": "",
  "special": "Bite: STR+1D + 5D acid. Web: 5D entangle in vacuum. Silicon: vacuum survival. Armor: +1D physical and energy.",
  "source": ""
 },
 {
  "name": "Vornskr",
  "type": "Force-hunting predator",
  "planet": "Myrkr",
  "habitat": "Forest",
  "dex": "4D",
  "per": "5D",
  "str": "4D+1",
  "move": "14",
  "size": "1.5m long + 1m tail",
  "orneriness": "",
  "special": "Bite: STR+1D. Claws: STR+2. Venomous Tail Whip: STR+1D + 3D stun poison. Force Tracking: +3D search vs Force-sensitives. Keen Senses: +1D PER.",
  "source": ""
 },
 {
  "name": "Vornskr (Domesticated)",
  "type": "Force-sensitive predator",
  "planet": "Myrkr",
  "habitat": "Domesticated (Talon Karrde's pets Sturm and Drang)",
  "dex": "4D",
  "per": "2D+2",
  "str": "3D+2",
  "move": "15",
  "size": "0.8m tall, up to 1.6m long (including tail)",
  "orneriness": "",
  "special": "Teeth: STR+2D damage. Force Sense: 3D ability to detect Force-users (Very Easy within 10m, Easy 11-25m, Moderate 26-50m, Difficult 51-100m, Heroic 100m+); become very agitated and aggressive upon sensing Force-users, confusing them with ysalamiri. Sneak 5D.",
  "source": "WEG40131 The Thrawn Trilogy Sourcebook p.142"
 },
 {
  "name": "Voxyn",
  "type": "Engineered Force-hunting beast",
  "planet": "",
  "habitat": "Various",
  "dex": "4D+1",
  "per": "4D+2",
  "str": "5D",
  "move": "14",
  "size": "2.5m long",
  "orneriness": "",
  "special": "Claws: STR+1D+2. Bite: STR+2D + acid (2D/round). Acid Spit: 4D, 10m range. Sonic Shriek: 4D stun, 5m cone. Tail Barb: STR+1D + paralytic (VD stamina or -2D DEX). Force Tracking: +4D search vs Force-sensitives. Natural Armor: +1D+2.",
  "source": "New Jedi Order Sourcebook (pages 103-104)"
 },
 {
  "name": "Vulptex (Crystal Fox)",
  "type": "",
  "planet": "Crait",
  "habitat": "Subterranean/Desert",
  "dex": "4D",
  "per": "4D+1",
  "str": "1D+2",
  "move": "16",
  "size": "0.6m tall",
  "orneriness": "",
  "special": "Crystal Hide: +1D physical, reflects light (+1D sneak in crystal caves via refraction). Keen Senses: +2D search. Burrowing: can navigate crystal tunnels.",
  "source": ""
 },
 {
  "name": "Vynock",
  "type": "Energy parasite",
  "planet": "",
  "habitat": "Space/Urban",
  "dex": "3D+2",
  "per": "2D",
  "str": "2D",
  "move": "6/12 (flying)",
  "size": "1m wingspan",
  "orneriness": "",
  "special": "Energy Drain: drains from power sources. Bite: STR+1. Silicon: vacuum survival. Flight.",
  "source": ""
 },
 {
  "name": "Wampa",
  "type": "",
  "planet": "Hoth",
  "habitat": "Arctic",
  "dex": "3D+1",
  "per": "4D",
  "str": "6D+1",
  "move": "13",
  "size": "3m tall",
  "orneriness": "",
  "special": "Claws: STR+2D. Bite: STR+1D. Camouflage: +2D sneak in snow. Cold Resistant. Darkvision: 20m.",
  "source": "Core Rulebook (page 229)"
 },
 {
  "name": "Wandrella",
  "type": "",
  "planet": "Circarpous V",
  "habitat": "Subterranean",
  "dex": "2D",
  "per": "2D",
  "str": "12D",
  "move": "15",
  "size": "10-15m long",
  "orneriness": "",
  "special": "Bite: STR+3D. Swallow: Walker Scale. Tremorsense: 30m. Natural Armor: +2D. Capital Scale (largest).",
  "source": "Splinter of the Mind's Eye (pages 56-57)"
 },
 {
  "name": "War Wasp",
  "type": "",
  "planet": "Kashyyyk",
  "habitat": "Forest",
  "dex": "4D+1",
  "per": "2D",
  "str": "1D",
  "move": "14 (flying)",
  "size": "0.3m long",
  "orneriness": "",
  "special": "Sting: STR+2 + 3D poison (Moderate stamina or -1D DEX for 1D hours). Swarm: 5D+2 in groups of 20+. Hive Mind.",
  "source": ""
 },
 {
  "name": "Webweaver",
  "type": "Forest arachnid",
  "planet": "Kashyyyk",
  "habitat": "Forest",
  "dex": "3D+2",
  "per": "2D+2",
  "str": "3D+1",
  "move": "12/14 (climbing)",
  "size": "2m long",
  "orneriness": "",
  "special": "Bite: STR+1D + 3D poison (Difficult stamina or paralyzed 1D hours). Web: 4D+2 entangle, 10m range. Darkvision: 20m. Climbing: +2D.",
  "source": ""
 },
 {
  "name": "Whisper Bird",
  "type": "",
  "planet": "Yavin IV",
  "habitat": "Forest",
  "dex": "3D+2",
  "per": "4D+1",
  "str": "1D",
  "move": "4/18 (flying)",
  "size": "0.3m long",
  "orneriness": "",
  "special": "Mimicry: reproduces sounds/voices perfectly. Keen Hearing: +2D search (sound). Talons: STR+1.",
  "source": "Galaxy Guide 2 (page 26)"
 },
 {
  "name": "Winged Quarra",
  "type": "Oceanic flying predator",
  "planet": "Mon Calamari",
  "habitat": "Aquatic/Atmosphere",
  "dex": "3D+1",
  "per": "2D+2",
  "str": "3D",
  "move": "10 (swimming)/16 (flying)",
  "size": "2m wingspan",
  "orneriness": "",
  "special": "Bite: STR+1D+2. Talons: STR+1D. Flight. Diving Attack: +1D damage from height.",
  "source": ""
 },
 {
  "name": "Wishgranter",
  "type": "Force-sensitive empath",
  "planet": "",
  "habitat": "Forest",
  "dex": "2D",
  "per": "5D",
  "str": "1D+2",
  "move": "6",
  "size": "0.5m tall",
  "orneriness": "",
  "special": "Empathy: senses emotions at 30m. Telepathic Illusion: projects desired images (Moderate Willpower to resist). Force-Sensitive (Sense 4D, Alter 3D). Harmless: never attacks physically.",
  "source": ""
 },
 {
  "name": "Womp Rat",
  "type": "",
  "planet": "Tatooine",
  "habitat": "Desert",
  "dex": "3D+1",
  "per": "2D+1",
  "str": "2D+1",
  "move": "13",
  "size": "2m long",
  "orneriness": "",
  "special": "Bite: STR+1D. Claws: STR+2. Pack Tactics: +1D per 3 additional (max +3D).",
  "source": "Star Wars Sourcebook (page 89)"
 },
 {
  "name": "Woodoo",
  "type": "Avian carrion-eater",
  "planet": "Tatooine",
  "habitat": "Desert",
  "dex": "2D+1",
  "per": "3D+2",
  "str": "2D+2",
  "move": "5/14 (flying)",
  "size": "1m tall, 2m wingspan",
  "orneriness": "",
  "special": "Beak: STR+1D. Talons: STR+2. Keen Sight: +2D search (visual, spotting carrion from great height).",
  "source": ""
 },
 {
  "name": "Woolamander",
  "type": "",
  "planet": "Yavin IV",
  "habitat": "Forest/Jungle",
  "dex": "3D+1",
  "per": "3D",
  "str": "2D+2",
  "move": "10/14 (climbing)",
  "size": "1.3m tall",
  "orneriness": "",
  "special": "Bite: STR+1D. Climbing: +2D. Group Tactics: shriek alerts pack. Territorial: defends Massassi ruins.",
  "source": "Galaxy Guide 2 (page 26)"
 },
 {
  "name": "Worrt",
  "type": "Voracious insipient predator",
  "planet": "Tatooine",
  "habitat": "Desert wilderness",
  "dex": "1D",
  "per": "3D",
  "str": "1D",
  "move": "3",
  "size": "0.5-1.5 meters tall",
  "orneriness": "",
  "special": "Tongue Attack: Brawling 4D, 1D damage. Attacks anything its size or smaller that moves.",
  "source": "WEG40080 Creatures of the Galaxy p.92"
 },
 {
  "name": "Wrix (Common)",
  "type": "Mountain-dwelling carnivore",
  "planet": "Various",
  "habitat": "Mountains, settlements fringe",
  "dex": "1D",
  "per": "2D",
  "str": "3D",
  "move": "13",
  "size": "Up to 2.5 meters long",
  "orneriness": "",
  "special": "Brawling parry 4D. Search 3D, sneak 3D+2. Brawling 5D, climbing/jumping 4D+1. Claws: STR+1D damage. Howl: intimidation 5D, terrifies many creatures. Very territorial, hunts in packs vs larger predators. Attacks on sapients always at night. Sub-species: Corellian wrix (larger ears, more ferocious), Tanthior wrix (3.5m, faster, reddish-brown). Creature scale.",
  "source": "WEG40080 Creatures of the Galaxy p.90"
 },
 {
  "name": "Wulam",
  "type": "Aquatic lurker",
  "planet": "Bothawui",
  "habitat": "Aquatic",
  "dex": "2D",
  "per": "3D",
  "str": "4D+2",
  "move": "3/14 (swimming)",
  "size": "3m long",
  "orneriness": "",
  "special": "Bite: STR+2D. Tail Slap: STR+1D. Ambush: +2D sneak in water. Armored Hide: +1D physical.",
  "source": "Shadows of the Empire Planets Guide (page 63)"
 },
 {
  "name": "Wyrwulf",
  "type": "Pack predator",
  "planet": "Dathomir",
  "habitat": "Forest/Plains",
  "dex": "3D+2",
  "per": "3D+1",
  "str": "4D+2",
  "move": "16",
  "size": "1.8m long, 1m at shoulder",
  "orneriness": "",
  "special": "Bite: STR+1D+2. Claws: STR+1D. Pack Tactics: +1D per 2 additional (max +4D). Keen Senses: +1D+2 search.",
  "source": "Geonosis and the Outer Rim Worlds (page 35)"
 },
 {
  "name": "Xandank",
  "type": "Burrowing predator",
  "planet": "Ord Cestus",
  "habitat": "Subterranean/Desert",
  "dex": "3D+1",
  "per": "2D+2",
  "str": "5D+2",
  "move": "10/8 (burrowing)",
  "size": "3m long",
  "orneriness": "",
  "special": "Claws: STR+2D. Mandibles: STR+1D+2. Natural Armor: +2D physical, +1D energy. Tremorsense: 20m. Ambush: +2D sneak when burrowed.",
  "source": "The Cestus Deception"
 },
 {
  "name": "Xinkra",
  "type": "Aerial raptor",
  "planet": "",
  "habitat": "Mountain/Various",
  "dex": "4D+1",
  "per": "3D+2",
  "str": "3D",
  "move": "6/22 (flying)",
  "size": "2.5m wingspan",
  "orneriness": "",
  "special": "Talons: STR+2D. Beak: STR+1D. Keen Sight: +3D search visual. Dive Attack: +2D damage from height.",
  "source": ""
 },
 {
  "name": "Yavinian Runyip",
  "type": "Swamp marsupial",
  "planet": "Yavin IV",
  "habitat": "Swamp",
  "dex": "4D",
  "per": "3D",
  "str": "3D+2",
  "move": "8",
  "size": "1-1.4m tall",
  "orneriness": "4D",
  "special": "Head Butt: STR+1D.",
  "source": "Galaxy Guide 12 (pages 19-20)"
 },
 {
  "name": "Ysalamiri",
  "type": "",
  "planet": "Myrkr",
  "habitat": "Forest",
  "dex": "1D",
  "per": "2D",
  "str": "1D",
  "move": "2",
  "size": "0.5m long",
  "orneriness": "",
  "special": "Force Bubble: creates 10m radius Force-neutral zone (no Force powers work). Multiple ysalamiri extend radius. Tree Clinger: claws fused to branch, Difficult STR to remove without killing.",
  "source": ""
 },
 {
  "name": "Yurtzen",
  "type": "Semi-aquatic herd animal",
  "planet": "Naboo",
  "habitat": "Swamp/Aquatic",
  "dex": "1D+2",
  "per": "2D",
  "str": "4D",
  "move": "6/10 (swimming)",
  "size": "3m long",
  "orneriness": "2D",
  "special": "Trample: STR+1D. Docile. Thick Hide: +1D physical.",
  "source": "Secrets of Naboo (page 63)"
 },
 {
  "name": "Zakkeg",
  "type": "Armored predator",
  "planet": "Dxun",
  "habitat": "Jungle",
  "dex": "2D+2",
  "per": "2D+1",
  "str": "7D",
  "move": "12",
  "size": "3m long, 2m tall",
  "orneriness": "",
  "special": "Claws: STR+1D+2. Bite: STR+2D. Horn Gore: STR+2D+1. Natural Armor: +3D physical, +2D energy. Fearless: immune to fear/intimidation.",
  "source": ""
 },
 {
  "name": "Zalaaca",
  "type": "Large swamp predator",
  "planet": "Naboo",
  "habitat": "Swamp/Aquatic",
  "dex": "2D",
  "per": "2D",
  "str": "7D+2",
  "move": "10/16 (swimming)",
  "size": "8m long",
  "orneriness": "",
  "special": "Bite: STR+2D. Tail Slap: STR+1D+2. Swallow: Medium or smaller creatures. Natural Armor: +1D+2.",
  "source": "Secrets of Naboo (page 63)"
 },
 {
  "name": "Desert Worm",
  "type": "Desert reptile",
  "planet": "Tatooine",
  "habitat": "Desert (Dune Sea)",
  "dex": "3D+1",
  "per": "2D",
  "str": "11D",
  "move": "15",
  "size": "10m tall, 20–50m long",
  "orneriness": "20",
  "special": "Claws: 8D damage. Teeth: 15D damage. Burrowing: Desert worms travel just under the surface of soft sand for several kilometers, ambushing prey from below.",
  "source": "WEG40124 Galaxy Guide 1: A New Hope (Wayfar scenario)"
 },
 {
  "name": "Zuxu",
  "type": "Trained attack fish",
  "planet": "Gala",
  "habitat": "Aquatic",
  "dex": "3D+1",
  "per": "2D+1",
  "str": "3D+2",
  "move": "16 (swimming)",
  "size": "2m long",
  "orneriness": "",
  "special": "Bite: STR+2D. Keen Smell: +2D search in water. Trainable: responds to sonic commands. Armor: +1D physical (scales).",
  "source": ""
 },
 {
  "name": "Cracian Thumper",
  "type": "Domesticated transport mount",
  "planet": "Craci",
  "habitat": "Various",
  "dex": "3D",
  "per": "3D+2",
  "str": "3D",
  "move": "12",
  "size": "1.8m at shoulder (4.8m with tail)",
  "orneriness": "1D",
  "special": "Claws: STR+1 damage. Tail: STR+1D+2 damage. Silent Movement: +1D+2 to sneak if making two moves or less per round. Can carry up to 100kg cargo, or a rider and up to 50kg.",
  "source": "WEG40091 Rebel Alliance Sourcebook p.105"
 },
 {
  "name": "Warbeast",
  "type": "Armored avian flying mount",
  "planet": "Dxun",
  "habitat": "Jungle/Forest",
  "dex": "3D+1",
  "per": "2D",
  "str": "7D",
  "move": "22 (flying), 3 (walking)",
  "size": "2-2.5m long",
  "orneriness": "3D+1",
  "special": "Flight: can fly but cannot hover. Speeder scale. Brawling 7D+2, stamina 8D, dodge 4D+2.",
  "source": "WEG40082 Tales of the Jedi Companion p.109"
 },
 {
  "name": "Kabus-Dabeh Monstrosity",
  "type": "Aquatic predator",
  "planet": "Muzara",
  "habitat": "Ruins of Kabus-Dabeh (submerged/aquatic)",
  "dex": "3D",
  "per": "2D",
  "str": "6D",
  "move": "2 (crawling), 14 (swimming)",
  "size": "Large",
  "orneriness": "4D",
  "special": "Brawling 7D+2, swimming 8D. Maw: 7D damage. Tentacle barbs: 6D stun damage. 4000 BBY era.",
  "source": "WEG40082 Tales of the Jedi Companion pp.149-169"
 },
 {
  "name": "Togarn",
  "type": "Nocturnal predator",
  "planet": "Muzara",
  "habitat": "Ruins of Kabus-Dabeh",
  "dex": "1D+2",
  "per": "3D",
  "str": "3D",
  "move": "4",
  "size": "Medium",
  "orneriness": "3D",
  "special": "All other stats 2D. Sneak 3D+2, survival 3D, brawling 4D. Spiked tail: 4D damage. Light sensitivity: when exposed to bright light suffers -1D penalty to all attribute and skill rolls. Character Points: 3. 4000 BBY era.",
  "source": "WEG40082 Tales of the Jedi Companion pp.149-169"
 },
 {
  "name": "Bogey",
  "type": "Kessel energy being",
  "planet": "Kessel",
  "habitat": "Spice tunnels (natural, deep)",
  "dex": "5D",
  "per": "1D",
  "str": "1D",
  "move": "14 (flying)",
  "size": "1 meter wide",
  "orneriness": "1D",
  "special": "Dodge 8D. Does not attack; phases into rock when it reaches spice-free stone. Activates glitterstim veins as it passes, sparking tunnel walls. Serves as a food source for spice spiders. Sightings rare; only found in deeper natural tunnels.",
  "source": "WEG40114 The Jedi Academy Sourcebook p.120"
 },
 {
  "name": "Granite Slug",
  "type": "Land mollusk",
  "planet": "Coruscant",
  "habitat": "Undercity",
  "dex": "1D",
  "per": "1D",
  "str": "1D+1",
  "move": "3",
  "size": "1.2–2.3 meters tall",
  "orneriness": "1D",
  "special": "Rubbery Hide: +1D+2 bonus against physical and energy attacks due to thick blubbery hide. Harmless to human-sized creatures. Introduced to Coruscant 346 years ago to clean undercity refuse; population has since exploded, fueling predator population booms.",
  "source": "WEG40114 The Jedi Academy Sourcebook p.120"
 },
 {
  "name": "Coruscani Ogre",
  "type": "Semi-intelligent Undercity denizen",
  "planet": "Coruscant",
  "habitat": "Undercity",
  "dex": "2D",
  "per": "3D",
  "str": "5D",
  "move": "11",
  "size": "Large",
  "orneriness": "4D",
  "special": "Brawling parry 4D, dodge 4D+1, melee combat 3D+2, melee parry 4D, thrown weapons 3D. Survival: Coruscant Undercity 5D+2. Hide 6D, search 6D, sneak 7D+2. Brawling 7D+1, climbing/jumping 6D+2, lifting 6D, stamina 7D+1. Equipment: chains, large club (STR+2D). Smell: hunts as much by smell as sight — does not incur penalties using Perception skills in the dark. Character Points: 2.",
  "source": "WEG40114 The Jedi Academy Sourcebook p.40"
 },
 {
  "name": "Krakana",
  "type": "Deep-sea predator",
  "planet": "Mon Calamari",
  "habitat": "Ocean rifts (deep)",
  "dex": "2D",
  "per": "2D",
  "str": "6D",
  "move": "24 (swimming)",
  "size": "14.5–26 meters",
  "orneriness": "4D",
  "special": "Sneak 7D. Scale: Speeder. Tentacles: each tentacle does 4D+1 damage; if more than three tentacles hit a target the victim is caught fast and drawn to the mouth the following turn. Teeth: crushing jaws do STR+2D damage. Seldom rises to sunlit depths; hunts primarily by sound — advanced hearing offsets deficient sight.",
  "source": "WEG40114 The Jedi Academy Sourcebook p.121"
 },
 {
  "name": "Lava Dragon",
  "type": "Reptilian predator",
  "planet": "Eol Sha",
  "habitat": "Volcanic lava pools and cave fissures",
  "dex": "3D",
  "per": "5D",
  "str": "6D",
  "move": "6 (swimming in lava)",
  "size": "2.6–5.8 meters tall, 6.6–9 meters long",
  "orneriness": "4D",
  "special": "Dodge 4D+1. Hide 7D, search 6D+1, sneak 6D. Brawling 8D. Scale: Speeder. Armor: crystalline silicon scales give -4D bonus vs. physical attacks and +8D bonus vs. energy attacks (reflective); if the creature is wounded its scales are breached and it takes 5D speeder-scale lava damage per round. Teeth: STR+2D. Lava Attack: sucks lava down its armored gullet and spews it at attackers for 10D character-scale damage. Subsists on lesser lava-dwelling reptiles.",
  "source": "WEG40114 The Jedi Academy Sourcebook p.121"
 },
 {
  "name": "Piranha Beetle",
  "type": "Carnivorous beetle",
  "planet": "Yavin 4",
  "habitat": "Jungle",
  "dex": "1D",
  "per": "2D",
  "str": "1D",
  "move": "12 (flying)",
  "size": "5 centimeters long",
  "orneriness": "2D",
  "special": "Dodge 3D+1, flight 4D. Hunts in small groups; subsonically summons swarm to prey within minutes. Swarm Attack (madness phase — triggered when swarm grows too large): attacks for 3D+2 damage, can target multiple creatures simultaneously, cannot be blocked; only eluding or gassing the swarm offers survival. Normally shy away from humans and larger beings; concentrate on smaller prey.",
  "source": "WEG40114 The Jedi Academy Sourcebook p.121"
 },
 {
  "name": "Ratidillo",
  "type": "Undercity scavenger",
  "planet": "Coruscant",
  "habitat": "Undercity",
  "dex": "2D+1",
  "per": "1D+2",
  "str": "1D",
  "move": "11",
  "size": "0.3–0.6 meters long (excluding tail)",
  "orneriness": "1D",
  "special": "Running 4D. Armor: thick leathery plates add +1D+2 against all physical and energy attacks. Tusks: STR+1D damage. Also known as kragget rat. Scavenger; flees from larger foes but will kill and eat smaller animals. Occasionally forms large packs that swarm through undercity tunnels consuming everything.",
  "source": "WEG40114 The Jedi Academy Sourcebook p.122"
 },
 {
  "name": "Spice Spider",
  "type": "Subterranean predator",
  "planet": "Kessel",
  "habitat": "Deep spice tunnels",
  "dex": "1D",
  "per": "8D",
  "str": "4D",
  "move": "15",
  "size": "2.4–6.8 meters wide",
  "orneriness": "3D",
  "special": "Search 9D. Brawling: impale 5D, stamina 6D+2. Impaling: impales prey on dagger-like legs for STR+1D damage plus +2 additional damage per turn from jostling; opposed Strength roll to break free. Drain Energy: each impaled character takes 1D damage per turn from energy drain. Spins crystalline glitterstim webs to trap bogeys (primary food source); trapped bogey activates photosensitive glitterstim, alerting the spider. Invisible on infrared scanners — undetectable to guards.",
  "source": "WEG40114 The Jedi Academy Sourcebook p.123"
 },
 {
  "name": "Umgullan Racing Blob",
  "type": "Umgullan blob subspecies",
  "planet": "Umgul",
  "habitat": "Blob raceways",
  "dex": "2D",
  "per": "1D",
  "str": "2D",
  "move": "15",
  "size": "1–1.2 meters",
  "orneriness": "1D",
  "special": "\"Running\" 8D. Stamina 4D. Defusion: can detach portions of its body, move them separately to another location, and reform; each detached portion is as intelligent as the main mass. Cannot thrive offworld; derives essential nutrients from Umgul's fogs. Bred for speed and fluidity; must meet strict age, mass, and viscosity requirements to race.",
  "source": "WEG40114 The Jedi Academy Sourcebook p.124"
 },
 {
  "name": "Renan Bloodwolf",
  "type": "Pack predator",
  "planet": "Rena",
  "habitat": "Mountain ranges above Renan vineyards",
  "dex": "2D+1",
  "per": "2D",
  "str": "2D+2",
  "move": "12",
  "size": "Just under 1 meter tall at shoulder",
  "orneriness": "3D",
  "special": "Brawling parry 3D, dodge 3D+1, sneak 3D+2, brawling 4D, climbing/jumping 3D+2. Teeth: STR+2D damage. Camouflage Fur: mottled broad-patch patterning grants +1D to sneak rolls in wooded or mountainous terrain. Mature bloodwolves develop dusky red-brown tooth enamel from iron deposits in the outer enamel layer. Two pair of slashing teeth, abnormally large relative to skull size.",
  "source": "WEG40085 The Truce at Bakura p.102"
 },
 {
  "name": "Ikov",
  "type": "Tree-level scavenger",
  "planet": "Kashyyyk",
  "habitat": "Below tree-top level in wroshyr-tree forests",
  "dex": "1D",
  "per": "1D+2",
  "str": "1D",
  "move": "8 (running), 16 (climbing)",
  "size": "Small (exact size unrecorded)",
  "orneriness": "1D+2",
  "special": "Sneak 3D+1, climbing/jumping 5D+2. Teeth: STR+1D damage; exquisitely serrated blade teeth with iridescent, reflective inner surfaces (uncommon among predators). Climbing Claws: +1D to climbing rolls. Scavenger subsisting primarily on prey falling from uppermost wroshyr branches; streamlined body for furtive climbing. Typically found in fur-lined tree hollows.",
  "source": "WEG40085 The Truce at Bakura p.102"
 },
 {
  "name": "Cooha",
  "type": "Avian predator",
  "planet": "Gandeid IV",
  "habitat": "Low-gravity open terrain near noli nests",
  "dex": "2D",
  "per": "1D",
  "str": "2D+2",
  "move": "18 (low-gravity worlds), 9 (standard-gravity worlds)",
  "size": "Up to 1.5 meters tall",
  "orneriness": "2D+2",
  "special": "Brawling parry 2D+2, dodge 3D, search 2D+2, sneak 1D+2, brawling 3D, climbing/jumping 3D+2. Beak: STR+2 damage. Claws: STR+1D damage. Tall and lanky; adapted to Gandeid IV's low gravity. Feeds primarily on flightless noli nestlings; slashing teeth and beak provide self-defense when adult noli (with powerful beaks and claws) defend nests.",
  "source": "WEG40085 The Truce at Bakura p.102"
 },
 {
  "name": "Marmaw",
  "type": "Amphibious carnivore",
  "planet": "Almar",
  "habitat": "Shores and shallow seas",
  "dex": "1D",
  "per": "2D",
  "str": "3D",
  "move": "5 (land), 15 (swimming)",
  "size": "Up to 4 meters long",
  "orneriness": "3D",
  "special": "Dodge 1D+2, brawling parry 4D+2, search 3D, brawling 4D, swimming 4D+1. Teeth: STR+1D damage; teeth form in 20–30 bud rows throughout the mouth and throat and migrate toward the edge as outer teeth are lost — continuously replaced. Latching Attack: if brawling roll beats target's brawling parry by 4+ points, the marmaw has latched on — increase difficulty of all victim's actions by two levels. If it beats the parry by 10+ points, the victim is pulled into the marmaw's mouth — must make an opposed Strength roll beating the marmaw by 5+ points to escape. Amphibious: possesses both lungs and gills; must spend at least 3 hours per day in water to avoid dehydration.",
  "source": "WEG40085 The Truce at Bakura p.103"
 },
 {
  "name": "Sinosai",
  "type": "Highland predator",
  "planet": "Ithor",
  "habitat": "Wildlife-rich upper plateau",
  "dex": "2D",
  "per": "2D+2",
  "str": "2D+2",
  "move": "18",
  "size": "Unrecorded",
  "orneriness": "3D+1",
  "special": "Brawling parry 3D, dodge 3D, search 3D+2, sneak 4D+1, brawling 3D, climbing/jumping 4D+2. Bite: STR+1D damage. Teeth have a periodically shed outer layer; when extracted after death, teeth can be separated layer by layer to reveal a nested pagoda-like structure. High intelligence noted for an animal. Stealth and speed are evolutionary specializations.",
  "source": "WEG40085 The Truce at Bakura p.104"
 },
 {
  "name": "Vertha",
  "type": "Small predator",
  "planet": "Morseer",
  "habitat": "Methane-shrouded surface",
  "dex": "1D+2",
  "per": "2D+1",
  "str": "1D",
  "move": "11",
  "size": "Small (exact size unrecorded)",
  "orneriness": "2D",
  "special": "Sneak 4D. Bite: STR+2 damage; classic enameled dentition with chisel-wedge incisors and bluish-tinted enamel. Camouflage: +1D to sneak rolls in native environments; coloration adapted to a world where native 'vegetation' is barely recognizable as such. Morseer was recently claimed by the Empire over Drackmarian objections.",
  "source": "WEG40085 The Truce at Bakura p.104"
 },
 {
  "name": "Ketrann",
  "type": "Solitary predator",
  "planet": "Alk'lellish",
  "habitat": "Open terrain; follows migrating herds of herbivorous metarshi",
  "dex": "2D+1",
  "per": "2D",
  "str": "3D+1",
  "move": "23",
  "size": "Unrecorded",
  "orneriness": "3D+2",
  "special": "Dodge 3D+2, running 4D+1, search 3D+2, brawling 4D+1, climbing/jumping 5D, stamina 5D. Bite: STR+2D damage. Killing Bite: if brawling roll beats target's brawling parry by 10 or more points, the ketrann has positioned for a killing bite through the neck and receives +1D to damage. Stamina: can run for over 25 standard hours without tiring. Four long white saber-fangs; leaps onto the back of prey to deliver a single killing bite. A popular sport hunting target on Alk'lellish.",
  "source": "WEG40085 The Truce at Bakura p.104"
 },
 {
  "name": "Llwelkyn",
  "type": "Ferocious predator",
  "planet": "Drong II",
  "habitat": "Jungles",
  "dex": "2D",
  "per": "2D",
  "str": "4D",
  "move": "26",
  "size": "Up to 3 meters long",
  "orneriness": "4D",
  "special": "Brawling parry 4D, dodge 5D+2, running 3D+2, search 3D+2, sneak 4D, brawling 5D+1, climbing/jumping 4D+2. Pack Creatures: llwelkyn normally hunt in packs of 5–12. Enhanced Senses (Smell): +1D to search; can detect the scent of repulsorlift vehicles up to 200km away (search difficulties: Very Easy 0–5km / Easy 25km / Moderate 50km / Difficult 100km / Very Difficult 200km) — machinery scent maddens them. Claws: STR+2D damage. Bite: STR+1D+2 damage. Drongians have cut clear paths through their jungles to enable hunting on foot; llwelkyn hunting is a major revenue source for the impoverished Drongian people.",
  "source": "WEG40085 The Truce at Bakura p.105"
 },
 {
  "name": "KyLessian Fruit Bat",
  "type": "Vicious flying predator",
  "planet": "KyLessia",
  "habitat": "Wulfruit tree canopy; hunts on moonless cloudy nights",
  "dex": "2D+2",
  "per": "1D+2",
  "str": "1D",
  "move": "35 (flying)",
  "size": "15–30 cm long",
  "orneriness": "1D+2",
  "special": "Dodge 4D, sneak 4D. Flight: KyLessian fruit bats are very quiet when flying, adding +2D to sneak. Bite: STR+2 damage; a lone fruit bat is unlikely dangerous but large groups can be lethal. Infrared Vision: sees in complete darkness with no penalties. Ultrasonic Communication: communicates via ultrasonic calls above the range of normal human hearing. Nocturnal: active only at night; if encountered during daylight (normally hide in caves or sheds), they are easily subdued. Despite the name, does not eat fruit — hunts in swarms by burrowing simultaneously into prey. Small black bodies with webbed wings and feather-like scales; dark gray, near-black tooth enamel for invisibility at night.",
  "source": "WEG40085 The Truce at Bakura p.105"
 },
 {
  "name": "Orycat",
  "type": "Pack predator",
  "planet": "Orryxia",
  "habitat": "Woodland",
  "dex": "1D",
  "per": "1D",
  "str": "1D",
  "move": "6",
  "size": "Unrecorded",
  "orneriness": "3D",
  "special": "Brawling parry 2D, dodge 1D+2, sneak 2D+2, brawling 1D+2. Pack Creatures: found in packs of 12–24. Described as a 'rather foul creature'; full survey not available.",
  "source": "WEG40085 The Truce at Bakura p.105"
 },
 {
  "name": "Istician",
  "type": "Territorial burrower",
  "planet": "Istic II",
  "habitat": "Underground burrows; emerges after dark to feed",
  "dex": "2D",
  "per": "2D",
  "str": "2D",
  "move": "11",
  "size": "2 meters tall",
  "orneriness": "3D+1",
  "special": "Dodge 2D+1, search 3D, sneak 3D+1, brawling 2D+1. Fangs: STR+1 damage. Burrowing Claws: STR+2 damage; also used for underground locomotion. Exoskeleton: thick exoskeleton provides +1D energy and +2D physical armor. Very territorial (+1D to brawling when defending territory). Subsentient according to official classification. Highly variable in size, shape, and coloration due to heavy radiation from Istic II's young yellow-white star; prone to mutations despite thick exoskeletons. Feed on insect-like surface herbivores. Local Imperial government actively encourages sport hunting to subdue the population.",
  "source": "WEG40085 The Truce at Bakura p.106"
 },
 {
  "name": "Ursmal",
  "type": "Large omnivore",
  "planet": "Ottega",
  "habitat": "Forests near honey hive sites",
  "dex": "2D",
  "per": "2D",
  "str": "5D+1",
  "move": "9",
  "size": "Large (bear-like; exact size unrecorded)",
  "orneriness": "3D+2",
  "special": "Brawling parry 6D, search 4D, sneak 4D+2, brawling 6D+2, climbing/jumping 5D+2. Bite: STR+1D damage; 12 large front incisors with mottled, superficially 'cracked' appearance. Claws: STR+2 damage. Hibernation: ursmals in hibernation cannot act to fend off attacks. Raids honey hives maintained for Ottegan mead production. Primitive nervous system sustains only two states: deeply asleep or fully awake. Known to continue fighting large hunting parties when mortally wounded.",
  "source": "WEG40085 The Truce at Bakura p.106"
 },
 {
  "name": "Cratsch",
  "type": "Territorial tree-dweller",
  "planet": "Bakura",
  "habitat": "Subtropical forests; namana tree stands",
  "dex": "1D",
  "per": "2D",
  "str": "1D+1",
  "move": "12 (running and climbing)",
  "size": "Unrecorded (ferret-like)",
  "orneriness": "3D",
  "special": "Brawling parry 2D+2, search 2D+1, brawling 2D+2, climbing/jumping 2D+1. Kick: STR+1 damage. Climbing Claws: retractable; +1D to climbing rolls. Very Territorial: +1D to brawling when defending its territory. Males stake claims to namana tree stands using musk glands at the base of the tail, defending against all rival males. During mating season a prime male can defend a large timberland. Attracted to namana trees by fruit-eating birds; waits for birds to drop fruit then feeds on the ground. Wedge-shaped head with black 'mask' markings; long lithe body; short strong legs; prehensile black tail.",
  "source": "WEG40085 The Truce at Bakura p.106"
 },
 {
  "name": "Sled Fox",
  "type": "Sliding predator",
  "planet": "Berea",
  "habitat": "Cold icy terrain; slopes and snowfields",
  "dex": "2D",
  "per": "2D",
  "str": "2D",
  "move": "18 (sliding), 8 (running)",
  "size": "Unrecorded",
  "orneriness": "2D+2",
  "special": "Dodge 3D+1, search 2D+2, brawling 3D. Fangs: STR+2 damage. Only two true legs; vestigial hind appendages used for steering when sliding downhill or preventing backsliding when moving uphill. Preys on small herbivores including the flightless Berean snow stork and a lagomorph species called chickatilis. Chickatilis are named for their peculiar mating calls.",
  "source": "WEG40085 The Truce at Bakura p.106"
 },
 {
  "name": "Kououra",
  "type": "Small venomous carnivore",
  "planet": "Kashyyyk",
  "habitat": "Forested terrain",
  "dex": "1D",
  "per": "1D",
  "str": "1D+2",
  "move": "6",
  "size": "0.8–1.2 meters long",
  "orneriness": "2D+2",
  "special": "Search 2D, brawling 3D, climbing/jumping 2D+1. Venom: does 6D damage. Brilliant red fur; slow-moving and physically weak but extremely toxic venom ensures survival on Kashyyyk. Flesh is pale, tender, and succulent. Survived on one of the most hazardous planets in the galaxy purely through venom toxicity.",
  "source": "WEG40085 The Truce at Bakura p.106"
 },
 {
  "name": "Lrashtash",
  "type": "Equatorial herbivore",
  "planet": "Usean II",
  "habitat": "Equatorial green valleys; rocket grass plains",
  "dex": "1D",
  "per": "1D",
  "str": "1D",
  "move": "4",
  "size": "Unrecorded",
  "orneriness": "1D",
  "special": "Sneak 2D. Bluffing Teeth: enormous frontal bluffing teeth act as intimidation 4D, rolled against target's willpower or Perception; teeth are not actually used to gnaw primary food (tall woody rocket grass stalks) — the temporomandibular joint does not open widely enough. Also possesses 'chisel teeth' set deeper in the dental arch for actual chewing. Hollow pseudo-incisors serve no feeding function beyond frightening predators.",
  "source": "WEG40085 The Truce at Bakura p.107"
 },
 {
  "name": "Scurrier",
  "type": "Scavenger",
  "planet": "Tatooine",
  "habitat": "Wastes, urban alleys",
  "dex": "3D",
  "per": "2D+2",
  "str": "2D+1",
  "move": "15",
  "size": "1.2m long",
  "orneriness": "2D",
  "special": "Running 4D. Sneak 3D+2. Climbing/jumping 3D+2. Bite: STR damage. Horns (males only): STR+1D. Manipulation: forepaws can manipulate small objects and pick apart machinery as if with repair skills at 4D.",
  "source": "WEG40089 The Movie Trilogy Sourcebook SE p.179"
 },
 {
  "name": "Nek Cyborrean Battle Dog",
  "type": "Trained Attack Animal",
  "planet": "Cyborrea",
  "habitat": "Various (genetically/cybernetically engineered)",
  "dex": "2D",
  "per": "3D",
  "str": "2D+2",
  "move": "12",
  "size": "0.9-1.3 meters long",
  "orneriness": "",
  "special": "Brawling 4D+2. Body Armor: +2D to STR. Teeth: STR+2D+1. Claws: STR+1D. Neks are very aggressive creatures used by many scavengers, including those on Coruscant. They are the product of genetic and cybernetic engineering from the heavy-grav planet Cyborrea. Given computer and bionic treatments to enhance combat skills, they are fearless in combat. They leap onto prey and bite deeply with multiple rows of razor-sharp teeth, then rake cyborg claws along the victim's body.",
  "source": "WEG40071 Dark Empire Sourcebook p.60"
 },
 {
  "name": "Tumnor",
  "type": "Vicious Winged Predator",
  "planet": "Pinnacle Moon (Da Soocha V)",
  "habitat": "Pinnacle spires",
  "dex": "3D+2",
  "per": "4D",
  "str": "4D",
  "move": "10 (walking), 25 (flying)",
  "size": "5-meter wingspan",
  "orneriness": "",
  "special": "Search 5D+1. Teeth: STR+2D damage. Claws: STR+1D damage.",
  "source": "WEG40071 Dark Empire Sourcebook p.87"
 },
 {
  "name": "Bloodsniffer",
  "type": "Predator",
  "planet": "Kamar",
  "habitat": "Desert/Mountains",
  "dex": "3D",
  "per": "2D",
  "str": "3D",
  "move": "16",
  "size": "1-2.1 meters long",
  "orneriness": "",
  "special": "Blood Drain: must consume own weight in blood every two days to survive. Claws: STR+2 damage. Thorny Spur: tackles or claws victim then attaches tongue's spur — STR+1D damage; if wound or better, hits a major vein/artery (Difficult first aid or Moderate with medpac to stop bleeding to death). Lives in small packs of 3-8 adults plus pups. Extremely territorial.",
  "source": "WEG40042 Han Solo and the Corporate Sector Sourcebook p.141"
 },
 {
  "name": "Dravian Hound (Nashtah)",
  "type": "Domesticated Predator",
  "planet": "Dra III",
  "habitat": "Mountains",
  "dex": "2D+2",
  "per": "2D+2",
  "str": "4D+1",
  "move": "15",
  "size": "1.1m tall, 1.5m long",
  "orneriness": "",
  "special": "Search: tracking venom 7D. Brawling 7D. Stamina 10D+2. Armor: +1D. Claws: STR+2D. Teeth: STR+1D+2. Barbed Tail: STR+1D, marks prey with tracking venom. Tracking Venom: once marked by tailbarb venom or saliva, can track victim unerringly for one month. Six-legged with diamond-hard claws and teeth for digging prey out of basalt. Three sets of jagged teeth. Attack in packs. Used as guard beasts by crimelords but as likely to attack owners as defend them.",
  "source": "WEG40042 Han Solo and the Corporate Sector Sourcebook p.142"
 },
 {
  "name": "Howlrunner",
  "type": "Pack Hunter",
  "planet": "Kamar",
  "habitat": "Mountains/Scrublands",
  "dex": "2D+1",
  "per": "1D+1",
  "str": "3D",
  "move": "11",
  "size": "1.1 meters long",
  "orneriness": "",
  "special": "Search: tracking 5D. Brawling 4D. Bite: STR+1D. Pack animals that work cooperatively to track and kill larger herd creatures. Heads have an unnerving resemblance to a Human skull. Named for their terrifying howling while tracking prey. Not very fast but tirelessly follow wounded prey until it collapses. The Incom Corporation named their I-7 prototype snubfighter after this beast.",
  "source": "WEG40042 Han Solo and the Corporate Sector Sourcebook p.142"
 },
 {
  "name": "Ball Creature of Duroon (Bouncebeast)",
  "type": "Timid Herbivore",
  "planet": "Duroon",
  "habitat": "Various",
  "dex": "2D+1",
  "per": "2D",
  "str": "1D+2",
  "move": "5 (bounce), 2 (rolling)",
  "size": "0.3-0.5 meters diameter",
  "orneriness": "",
  "special": "Danger Sense: famed ability to detect danger; sometimes used as guard animals by Duroon's insurgent tribes. Pseudopod Skin: no limbs or sensory organs — muscles form and extrude limbs as needed (mouths, eyestalks, etc.). Attack: never intentionally attacks, but accidental hits cause STR+1D from springing force. Smooth spheres with widely varying coloration (green, blue, yellow, fluorescent). Bright colors are a survival mechanism — predators focus on the most colorful individuals while the rest of the herd bounces away. Reproduce very quickly. Mascot of the Bonadan Bouncebeasts gravball team.",
  "source": "WEG40042 Han Solo and the Corporate Sector Sourcebook p.140"
 },
 {
  "name": "Tranthebar Mountain Ripper",
  "type": "Avian Predator",
  "planet": "Tranthellix",
  "habitat": "Mountains",
  "dex": "3D",
  "per": "1D+2",
  "str": "2D+1",
  "move": "40 (flying), 70 (diving)",
  "size": "2.2-2.8m tall, up to 5m wingspan",
  "orneriness": "5D",
  "special": "Search 6D. Sneak 5D. Stamina 4D. Beak: STR+1D+2. Talons: STR+2D. Vision: spots 10cm objects at 200m. Silent Movement: glides on mountain thermals. Speed: dives approaching 200 kmh. Solitary predators, intelligent and cunning. Never completely domesticated — even raised from hatchlings, they eventually revert to wild form. Used in tranthebar racing on Tranthellix (strict export regulations).",
  "source": "WEG40025 Galladinium's Fantastic Technology p.91"
 },
 {
  "name": "Sid'Han (Mudcrawler)",
  "type": "Predator",
  "planet": "Seikosha",
  "habitat": "Mudflats and jungle",
  "dex": "4D",
  "per": "2D (sneak 8D)",
  "str": "6D+1",
  "move": "12",
  "size": "Up to 10 meters long",
  "orneriness": "18D",
  "special": "Teeth (4D damage). Tail (6D+1 damage; on any attack which incapacitates, mortally wounds, or kills, the victim is entangled and cannot escape without an opposed Strength roll). Burrows into soft mud and senses victims by vibrations from footsteps. Ground water collects in their concave dorsal areas, making them appear as large puddles. Ambush predators that emit a terrifying high-pitched shriek. They never run from combat.",
  "source": "WEG40083 Classic Adventures p.56"
 },
 {
  "name": "Kichicolia",
  "type": "Herbivore",
  "planet": "Najarka",
  "habitat": "Sub-tropical rainforest",
  "dex": "4D (dodge 6D)",
  "per": "3D (hide 4D, sneak 4D+1)",
  "str": "1D (climbing/jumping 3D)",
  "move": "12",
  "size": "0.2-0.5 meters tall",
  "orneriness": "",
  "special": "Teeth (STR+1D damage). Half-meter tall primates with prehensile fingers and toes, long thick fur — white with black markings around joints, mouths, and eyes. Distinctive long white-furred tail. Loud trilling squawk call. Live in small packs and are harmless herbivores; may attack if cornered but more likely to retreat.",
  "source": "WEG40083 Classic Adventures p.67"
 },
 {
  "name": "Tree Viper",
  "type": "Predator",
  "planet": "Najarka",
  "habitat": "Sub-tropical rainforest",
  "dex": "2D",
  "per": "2D",
  "str": "3D (brawling 4D, climbing/jumping 4D+2)",
  "move": "11",
  "size": "2.5-3 meters long",
  "orneriness": "",
  "special": "Fangs (STR damage plus venom). Constriction (STR+1D damage each round; subtract 2D from victim's Dexterity until successful opposed Strength roll to break free). Venom (bitten characters must make Moderate stamina roll or be paralyzed; paralyzed characters cease breathing in 10 rounds and suffer irreversible brain damage in 8 minutes unless countered by general antitoxin from standard medpacs). Hunt in groups of 2-6, accustomed to bringing down prey much larger than themselves. Bold and fearless predators.",
  "source": "WEG40083 Classic Adventures p.68"
 },
 {
  "name": "Najarkan Wilderbeast",
  "type": "Predator",
  "planet": "Najarka",
  "habitat": "Sub-tropical rainforest",
  "dex": "2D",
  "per": "1D",
  "str": "8D",
  "move": "14",
  "size": "Up to 30 meters long",
  "orneriness": "7D",
  "special": "Bite (STR damage). Armor (+2D to Strength to resist damage). Huge mouth over two meters in diameter. Large dull red eyes are low-resolution photoreceptors adapted to detect the black and white patterns of its natural prey, the kichicolia.",
  "source": "WEG40083 Classic Adventures p.78"
 },
 {
  "name": "Space Slug (Small)",
  "type": "Predator",
  "planet": "Alderaan (Asteroid 7785)",
  "habitat": "Asteroids, vacuum",
  "dex": "2D",
  "per": "1D",
  "str": "6D",
  "move": "6",
  "size": "",
  "orneriness": "",
  "special": "Can survive in vacuum with no penalties. Teeth do STR+1D damage. Found nesting in the ruined steam baths of the Alderaanian Royal Palace.",
  "source": "WEG40128 Classic Adventures Vol 2 p.40"
 },
 {
  "name": "Monsail",
  "type": "Predator",
  "planet": "Trinta",
  "habitat": "Swamp water",
  "dex": "3D",
  "per": "3D",
  "str": "4D",
  "move": "15 (swimming)",
  "size": "2 meters",
  "orneriness": "",
  "special": "Teeth (STR+1 damage). Predatory fish with a horizontally flattened body and narrow tail. Normally attack individually but have been known to travel in schools of 3+. Food value: 4 meals.",
  "source": "WEG40128 Classic Adventures Vol 2 p.79"
 },
 {
  "name": "Skinwing",
  "type": "Predator",
  "planet": "Trinta",
  "habitat": "Aerial, swamp canopy",
  "dex": "3D+1",
  "per": "2D+1",
  "str": "2D+2",
  "move": "18 (flying)",
  "size": "1m wingspan, 1.5m length",
  "orneriness": "",
  "special": "Teeth (STR+1D damage). Large avian reptiles with tough leathery wings. Emit a loud screech to announce attacks. Spectacularly unintelligent — swoop to investigate any stirring in water. Flee as soon as one is hit. Food value: 2 meals.",
  "source": "WEG40128 Classic Adventures Vol 2 p.79"
 },
 {
  "name": "Marsh Lizard (Trinta)",
  "type": "Predator",
  "planet": "Trinta",
  "habitat": "Shallow swamp, islands, mudbanks",
  "dex": "3D+1",
  "per": "2D+1",
  "str": "2D+2",
  "move": "15 (swimming), 10 (crawling)",
  "size": "2.5 meters",
  "orneriness": "",
  "special": "Teeth (STR+1D damage). Spiked tail (STR+1D damage). Large scaly serpents preferring shallow areas. Attack with one charging while others circle behind. Feed on gallazes and low-flying skinwings. Appear in groups of 1D. Food value: 3 meals.",
  "source": "WEG40128 Classic Adventures Vol 2 p.80"
 },
 {
  "name": "Morril",
  "type": "Herbivore",
  "planet": "Trinta",
  "habitat": "Trees, canopy",
  "dex": "3D (dodge 5D)",
  "per": "2D+2",
  "str": "1D (climbing/jumping 6D)",
  "move": "15",
  "size": "30 centimeters",
  "orneriness": "",
  "special": "Tree-dwelling marsupials, extremely docile and skittish. Very difficult to catch. Most common prey of skinwings. Herbivores that occasionally eat carrion. Food value: 1 meal.",
  "source": "WEG40128 Classic Adventures Vol 2 p.80"
 },
 {
  "name": "Water Snake (Trinta)",
  "type": "Predator",
  "planet": "Trinta",
  "habitat": "Mudbanks, islands, underwater",
  "dex": "3D (dodge 4D)",
  "per": "2D+2 (sneak 4D+2)",
  "str": "2D",
  "move": "10",
  "size": "3 meters",
  "orneriness": "",
  "special": "Teeth (STR+2D damage). Long black serpents living in holes in mudbanks and islands. Most likely to attack while characters climb out of water. Raise heads above surface before striking. Food value: 2 meals.",
  "source": "WEG40128 Classic Adventures Vol 2 p.80"
 },
 {
  "name": "Gallaz",
  "type": "Herbivore",
  "planet": "Trinta",
  "habitat": "Islands, swamp edges",
  "dex": "3D",
  "per": "2D+1",
  "str": "2D (running 4D, swimming 4D)",
  "move": "10",
  "size": "1 meter tall",
  "orneriness": "",
  "special": "Quadrupeds with reddish short fur and long feathery ears. Travel in herds of 12. Capable of silent communication. Flee when anything within 20 meters, often swimming away with powerful hindquarters. Only true herbivores on Trinta. Food value: 3 meals.",
  "source": "WEG40128 Classic Adventures Vol 2 p.80"
 },
 {
  "name": "Dark Lizard",
  "type": "Predator",
  "planet": "Trinta",
  "habitat": "Islands, swamp undergrowth",
  "dex": "4D",
  "per": "3D+1",
  "str": "4D+2",
  "move": "15",
  "size": "3 meters",
  "orneriness": "",
  "special": "Teeth (STR+1D damage). Corrupted by the dark side — glows with a black light and has red glowing eyes. Stalks prey before attacking.",
  "source": "WEG40128 Classic Adventures Vol 2 p.100"
 },
 {
  "name": "Dark Tongue",
  "type": "Predator",
  "planet": "Trinta",
  "habitat": "Islands",
  "dex": "4D",
  "per": "3D",
  "str": "4D",
  "move": "",
  "size": "Large",
  "orneriness": "",
  "special": "Teeth (STR+1D damage, 5D total). Tongue attack (Easy to hit, drags victim toward teeth next round unless victim beats tongue STR 2D+1 in opposed roll; any damage to tongue causes it to release and not be used again). Dark side corrupted — huge black lizard with burning red eyes and glowing black saliva. Pursues relentlessly.",
  "source": "WEG40128 Classic Adventures Vol 2 p.103"
 },
 {
  "name": "Swamp Beast",
  "type": "Predator",
  "planet": "Trinta",
  "habitat": "Islands",
  "dex": "4D",
  "per": "3D",
  "str": "4D+2",
  "move": "10",
  "size": "Large",
  "orneriness": "",
  "special": "Claws (STR+1D damage). A mound of semi-intelligent vegetation powered by the dark side. Ensnared characters are hugged into its rotting chest and must overcome its Strength to break free (suffocation rules apply). Moderate stamina roll or suffer 1D reduction from stench during combat.",
  "source": "WEG40128 Classic Adventures Vol 2 p.104"
 },
 {
  "name": "Carnivorous Plant (Trinta)",
  "type": "Predator",
  "planet": "Trinta",
  "habitat": "Islands",
  "dex": "3D",
  "per": "2D",
  "str": "3D+2 (brawling: tendrils 4D+2)",
  "move": "0",
  "size": "Large",
  "orneriness": "",
  "special": "Tendrils (STR 2D each, 3 per target; roll 1D for hit location: 1=left arm, 2=right arm, 3=left leg, 4=right leg, 5=body, 6=body and both arms). Teeth (STR+1D damage, digestive acid mouth). Captured characters dragged toward mouth next round. Damaged tendrils retract for one round then shoot out again. For each trapped arm, subtract 1D from victim's Strength.",
  "source": "WEG40128 Classic Adventures Vol 2 p.102"
 },
 {
  "name": "Gorgand",
  "type": "Predator",
  "planet": "Trinta",
  "habitat": "Islands near the dark side nexus",
  "dex": "5D+2",
  "per": "3D",
  "str": "5D",
  "move": "15",
  "size": "Large",
  "orneriness": "",
  "special": "Teeth (STR+1D damage). Claws (STR+1D+1 damage). Dark side corrupted — body and eyes glow red. When incapacitated, its intestines burst out as a dreambeast (DEX 6D, PER 1D, STR 4D) that attempts to strangle the nearest character: 1D damage first round, +1D per round attached. Intestines can be pulled off with opposed Strength roll. Intestine damage is mental (dreambeast) damage. Once intestines are defeated they fade away and the gorgand is seen to be still intact.",
  "source": "WEG40128 Classic Adventures Vol 2 p.114"
 },
 {
  "name": "Morodin",
  "type": "Herbivore (sentient)",
  "planet": "Varonat",
  "habitat": "Jungle, plains",
  "dex": "2D+2",
  "per": "3D",
  "str": "4D+1",
  "move": "12/15",
  "size": "15 meters long",
  "orneriness": "",
  "special": "Attribute Dice: 14D. Average: DEX 2D+2, KNO 3D, MEC 0D+2, PER 3D, STR 4D+1, TEC 0D+1. Biochemical Agriculture: Extensive knowledge of Varonat's plant life — by digesting certain plants and spreading nutrient slime over others, they produce new strains of plant food. Nutrient Slime: Secreted from a gland in their underbellies, changes in nutrient value depending on diet and encourages growth and mutation in plant life. Maze-running is their means of fertilizing areas — the patterns track which combinations were applied. Sentient species — not native to Varonat. They established an agricultural colony 1,000 years before the Old Republic, transforming immense plains into the Great Jungle. Giant herbivores with six stubby legs, spoon-billed snouts, and flat teeth. Mottled yellow coloring. Deep rumbling growl.",
  "source": "WEG40129 Adventure Journal 1 p.21"
 },
 {
  "name": "Camray Eel",
  "type": "Sea predator",
  "planet": "Spira",
  "habitat": "Ocean (Shinkai Abyss trench, 75-150m depth)",
  "dex": "4D",
  "per": "2D",
  "str": "3D",
  "move": "16",
  "size": "4-5 meters long",
  "orneriness": "4D",
  "special": "Coils: 5D constriction damage. Teeth: STR+2D damage. Camouflage: +2D to sneak when moving close to seabed. Swimming 6D, sneak 4D+2. Long snake-like marine predator haunting the Shinkai Abyss trench. Huge gaping mouth with fearsome teeth, lithe muscular body that can seize and crush prey. A loner that drifts among scored trench walls hunting sliverfish. Seldom rises above 75m depth, sleeps in sandy silt on the trench floor at 150m. Fearless and aggressive — investigates and attacks suited divers. Popular trophy among upper-class game hunters. Dead specimens have an incredibly potent smell requiring sealed storage tanks.",
  "source": "WEG40129 Adventure Journal 1 p.65"
 },
 {
  "name": "Galub Sea Slug",
  "type": "Sea parasite",
  "planet": "Spira",
  "habitat": "Ocean",
  "dex": "2D",
  "per": "2D",
  "str": "3D",
  "move": "8",
  "size": "2 meters long",
  "orneriness": "",
  "special": "Adhesion: Can adhere to solid surfaces using powerful suction. Large grub-white sea parasite that attaches to larger sea animals to suck blood. Incredibly stupid — sometimes mistakes boat hulls for potential hosts, slowing vessels considerably. Spears have little effect; must be blasted off. Mishap when shooting risks putting a hole through the hull.",
  "source": "WEG40129 Adventure Journal 1 p.66"
 },
 {
  "name": "Oskan Blood Eater",
  "type": "Predator",
  "planet": "Osk",
  "habitat": "Underground, captivity",
  "dex": "3D+2",
  "per": "2D",
  "str": "4D",
  "move": "6",
  "size": "2.5 meters tall",
  "orneriness": "",
  "special": "Claws (STR+3D). Frenzy: Sight of a humanoid creature incites a frenzy, adding +2D to damage. Tracking: Blood eaters are effective and persistent trackers (search: tracking 5D). Dark-skinned with four long saw-like claws and a head that appears to be all maw.",
  "source": "WEG40132 Operation: Elrood p.26"
 },
 {
  "name": "Mynock",
  "type": "Parasite",
  "planet": "Various (space-dwelling)",
  "habitat": "Space, starship hulls",
  "dex": "3D",
  "per": "1D",
  "str": "1D",
  "move": "9 (flight) or 1 (space)",
  "size": "",
  "orneriness": "",
  "special": "Energy Drain: Mynocks drain energy from starships by attaching to power cables and hull. Silicon Life-Form: Can survive in a vacuum. Will flee if possible but attack if cornered.",
  "source": "WEG40132 Operation: Elrood p.33"
 },
 {
  "name": "Space Mite",
  "type": "Parasite",
  "planet": "Various (space-dwelling)",
  "habitat": "Space, starship interiors",
  "dex": "2D",
  "per": "1D",
  "str": "1D",
  "move": "5",
  "size": "1 meter long",
  "orneriness": "",
  "special": "Bite (1D+2 damage, injects silica into victim's bloodstream causing lethargy for 24 hours: -1D penalty on all skill rolls and -2D penalty to all Dexterity rolls). Sneak 2D+1, brawling: bite 3D. Silicon-based life-forms that resemble white arachnids with wicked mandibles. Spend youth as fist-sized parasites on mynocks; upon maturity detach, rapidly grow to one meter, spin hardened egg cases and deposit up to a dozen eggs.",
  "source": "WEG40132 Operation: Elrood p.33"
 },
 {
  "name": "Piscator",
  "type": "Predator",
  "planet": "Alluuvia",
  "habitat": "Submerged caves",
  "dex": "2D+1",
  "per": "2D",
  "str": "3D",
  "move": "12",
  "size": "3 meters long",
  "orneriness": "",
  "special": "Tusks (STR+3D). Tentacles (STR+1D). Fear-Sensitivity: Somehow attuned to the Force, feeds on fear given off by terrified victims. Gains +1D to brawling for each character who fails a willpower roll. Instinctively attacks paralyzed victims first, fighting able characters only if attacked. Has a powerful tail, very small mouth, eight short tentacles, and a pair of curved tusks. Brawling 3D.",
  "source": "WEG40132 Operation: Elrood p.38"
 },
 {
  "name": "Albino Farlek",
  "type": "Predator",
  "planet": "Various",
  "habitat": "Wilderness",
  "dex": "3D",
  "per": "3D+2",
  "str": "6D",
  "move": "12",
  "size": "2 meters tall at shoulder, 4 meters long",
  "orneriness": "",
  "special": "Fangs (STR+1D). Snare-Tongue: Sticky adhesive tongue stretches almost two meters, Difficult Strength roll to break free. Tail (STR+2D). Dodge 6D+2, brawling parry 4D, running 7D+2, hide 6D+2, search 4D+2, sneak 7D+2, brawling 7D, climbing/jumping 7D, lifting 9D, stamina 7D. Highly offensive odor that clings to surroundings. Four-legged with thick scaly plates and wide heavily-armored head. The albino white variety is extremely rare — considered the stuff of legends.",
  "source": "WEG40132 Operation: Elrood p.58"
 },
 {
  "name": "Vervikk (Space Rat)",
  "type": "Scavenger",
  "planet": "Various (space-dwelling)",
  "habitat": "Starship ventilation shafts",
  "dex": "2D",
  "per": "1D+1",
  "str": "2D+1",
  "move": "5",
  "size": "1 meter long",
  "orneriness": "",
  "special": "Claws (STR+1). Teeth (STR+1D). Commonly known as 'space rats,' Vervikks inhabit ventilation shafts aboard starships and attack when hungry.",
  "source": "WEG40133 Classic Adventures Vol 3 p.32"
 },
 {
  "name": "Flailer",
  "type": "Predator",
  "planet": "Calamari",
  "habitat": "Ocean surface",
  "dex": "2D+2",
  "per": "4D",
  "str": "2D",
  "move": "15 (swimming)",
  "size": "2 meters long",
  "orneriness": "",
  "special": "Camouflage: At ranges over 20 meters, receives +2D to sneak to remain hidden. Can be mistaken for an oil slick. Sneak 4D+1. Teeth (2D damage; stunned or worse result causes cuts, blood in water may attract other creatures). Round, flat, jet black and shiny creatures inhabiting the surface layer of the southern Calamari ocean. Ringed with tiny razor-sharp tooth-like appendages, rakes teeth across victims and ingests blood from water.",
  "source": "WEG40133 Classic Adventures Vol 3 p.40"
 },
 {
  "name": "Lampfish",
  "type": "Predator",
  "planet": "Calamari",
  "habitat": "Deep ocean",
  "dex": "3D",
  "per": "1D",
  "str": "8D-12D",
  "move": "5 (swimming)",
  "size": "10-40 meters",
  "orneriness": "",
  "special": "Lure: Lures targets closer with luminescent tongue; victims mesmerized by light swim into its jaws. Brawling 9D+ (one higher than Strength). All mouth — huge head, jagged teeth, beady eyes, tiny body. Lives in perpetual darkness. Not particularly aggressive, hunts by swimming lazily with mouth open. Territorial — fights to drive off others of their type. Has been known to mistake lighted bathyspheres for rival lampfish.",
  "source": "WEG40133 Classic Adventures Vol 3 p.40"
 },
 {
  "name": "Choarn",
  "type": "Scavenger",
  "planet": "Calamari",
  "habitat": "Ocean",
  "dex": "3D",
  "per": "2D+1",
  "str": "2D-4D",
  "move": "15-35 (swimming)",
  "size": "1-8 meters",
  "orneriness": "",
  "special": "Blood Frenzy: Drawn by scent of blood at ranges up to 1 kilometer. Within 20 meters of blood, goes into frenzy gaining +1D to attack but cannot parry. Attacks anything that moves near them when frenzied. Blue eel-like creature with rows of sharp teeth. Blood-crazed packs have been known to batter themselves to death against clear windows of Quarren cities. Strength and Move increase with size — young adults have minimum stats, mature adults maximum. Quarren consider them good eating.",
  "source": "WEG40133 Classic Adventures Vol 3 p.41"
 },
 {
  "name": "Krayt Dragon",
  "type": "Predator",
  "planet": "Tatooine",
  "habitat": "Desert, canyons",
  "dex": "3D",
  "per": "1D+2",
  "str": "12D",
  "move": "15",
  "size": "10 meters tall, 30 meters long",
  "orneriness": "",
  "special": "Claws (8D damage). Teeth (15D damage). Swats victims with claws, then moves in to bite stunned prey. Flees after being incapacitated or suffering six hits (stun or better).",
  "source": "WEG40133 Classic Adventures Vol 3 p.107"
 },
 {
  "name": "Womp Rat",
  "type": "Predator",
  "planet": "Tatooine",
  "habitat": "Desert, ruins",
  "dex": "2D",
  "per": "1D+1",
  "str": "2D+1",
  "move": "5",
  "size": "2.1 meters long",
  "orneriness": "",
  "special": "Claws (STR+1 damage). Teeth (STR+1D damage). Extremely vicious, attack quickly and in packs.",
  "source": "WEG40133 Classic Adventures Vol 3 p.108"
 },
 {
  "name": "Bantha",
  "type": "Beast of Burden",
  "planet": "Tatooine",
  "habitat": "Desert",
  "dex": "2D",
  "per": "1D",
  "str": "5D",
  "move": "15",
  "size": "2.5 meters at the shoulder",
  "orneriness": "2D",
  "special": "Hardy Constitution: Can go weeks without food or water and quickly adapt to new environments. Horns (STR+1D damage). Stamina 8D.",
  "source": "WEG40133 Classic Adventures Vol 3 p.113"
 },
 {
  "name": "B'aerlak Beast",
  "type": "Herd Beast",
  "planet": "Various (jungle worlds)",
  "habitat": "Jungle",
  "dex": "2D",
  "per": "2D",
  "str": "6D",
  "move": "30 kph",
  "size": "",
  "orneriness": "2D",
  "special": "Running 7D. Search 6D. Brawling 8D. Jumping 7D. Slither: Can move very quickly, up to 30 kph. Tough Hide: +2D physical, +1D energy.",
  "source": "WEG40137 Instant Adventures"
 },
 {
  "name": "Trunk Ripper",
  "type": "Herbivore (Insectivore)",
  "planet": "Various (jungle worlds)",
  "habitat": "Jungle, arboreal",
  "dex": "3D",
  "per": "2D",
  "str": "3D+2",
  "move": "8, 12 (climbing)",
  "size": "2 meters long",
  "orneriness": "",
  "special": "Claws (STR+2 damage). Body Spines (+2 protection vs physical; anyone falling against spines takes STR damage). Climbing/jumping 6D. Arboreal mammals that rend tree bark to feed on insects. Prehensile tail for maneuvering in branches. Somewhat slothful on the ground.",
  "source": "WEG40137 Instant Adventures"
 },
 {
  "name": "Zobberan Hound",
  "type": "Guard Beast",
  "planet": "Various",
  "habitat": "Estates, domesticated",
  "dex": "4D",
  "per": "2D",
  "str": "4D",
  "move": "",
  "size": "3 meters long",
  "orneriness": "",
  "special": "Running 7D. Search 5D. Sneak 6D. Brawling 6D. Jumping 5D. Armored Hide (+2D vs physical, +1D vs energy). Teeth (STR+2D damage). Large, vicious-looking beasts that are extremely loyal and obedient once trained. Not very bright but merciless and deathly silent.",
  "source": "WEG40137 Instant Adventures"
 },
 {
  "name": "Karran Beetle",
  "type": "Insect",
  "planet": "Karra",
  "habitat": "Plateaus, jungles",
  "dex": "2D",
  "per": "2D",
  "str": "4D",
  "move": "15",
  "size": "",
  "orneriness": "",
  "special": "Mandibles (STR+2D damage). Forelegs (STR damage). Attacks any non-Karran being on contact.",
  "source": "WEG40138 Classic Adventures Vol 4"
 },
 {
  "name": "Leaper",
  "type": "Insect",
  "planet": "Karra",
  "habitat": "Plateaus, jungles",
  "dex": "2D",
  "per": "2D",
  "str": "7D",
  "move": "15",
  "size": "50 centimeters (body)",
  "orneriness": "",
  "special": "Mandibles (STR damage). Can leap 60 meters. Small, almost spherical body surrounded by six three-meter-long legs.",
  "source": "WEG40138 Classic Adventures Vol 4"
 },
 {
  "name": "Swarmers",
  "type": "Insect swarm",
  "planet": "Karra",
  "habitat": "Plateaus, jungles",
  "dex": "0D",
  "per": "0D",
  "str": "0D",
  "move": "15",
  "size": "Microscopic",
  "orneriness": "",
  "special": "Fly into eyes and ears, causing temporary deafness and blindness (-2D penalty to all skills involving sight or hearing). Almost microscopic flying creatures that can do little harm individually but cause significant interference in swarms.",
  "source": "WEG40138 Classic Adventures Vol 4"
 },
 {
  "name": "Legworm",
  "type": "Insect",
  "planet": "Karra",
  "habitat": "Jungles",
  "dex": "2D",
  "per": "2D",
  "str": "4D",
  "move": "10",
  "size": "10 meters long, 1 meter diameter",
  "orneriness": "",
  "special": "Claws (STR damage). Mandibles (STR+2D damage). Can attack up to 10 targets per round. Treat STR as 10D for determining damage sustained. Ten-meter-long segmented worm with ten pairs of two-meter-long legs. Head segment has two large silvery eyes and sharp pinching mandibles.",
  "source": "WEG40138 Classic Adventures Vol 4"
 },
 {
  "name": "Karran Mantis",
  "type": "Large carnivorous insect",
  "planet": "Karra",
  "habitat": "Plateau caves",
  "dex": "5D",
  "per": "2D",
  "str": "10D",
  "move": "20",
  "size": "15 meters tall, 30 meters long",
  "orneriness": "",
  "special": "Walker scale. Foreclaws (STR+2D damage). Last surviving specimen — the largest insect on Karra. The Karrans can induce it to hibernate in a cave at the base of a plateau, awakening only when called. Mantis-like in appearance with a head resembling a giant Arconan, it walks on four rear legs holding head and slender thorax upright while the thick abdomen remains horizontal. Strikes with long spear-like foreclaws.",
  "source": "WEG40138 Classic Adventures Vol 4"
 },
 {
  "name": "Rormaroo",
  "type": "Gentle grazer",
  "planet": "Pallaxides",
  "habitat": "Highland plateaus, canyons",
  "dex": "3D",
  "per": "3D",
  "str": "2D-8D",
  "move": "18",
  "size": "200-300 meters long (adults)",
  "orneriness": "1D",
  "special": "Walker scale. Camouflage: Carry abundant living vegetation on their backs — if next to a plateau they are indistinguishable from the surface until they fly away. Armored head (+3D physical, +2D energy), STR+1D ramming damage. Large dorsoventrally flattened animals with chambered gas-filled wings running the length of their limbless bodies. Soil and vegetation accumulate on their backs as they grow, giving the appearance of flying islands from above. Spend days grazing on airborne pollen and insects, drinking from cliffside waterfalls. Powerful creatures able to carry many tons. Make excellent riding beasts — loyal and affectionate. Immune to tamack spores. In old age, decades of soil accumulation prevent them from climbing high enough to graze, and they slowly starve. Dodge 4D, search 5D.",
  "source": "WEG40151 No Disintegrations"
 },
 {
  "name": "Ravenbeast",
  "type": "Predator",
  "planet": "Seregar",
  "habitat": "Jungle canopy",
  "dex": "4D",
  "per": "4D",
  "str": "6D",
  "move": "15",
  "size": "Large bipedal",
  "orneriness": "3D",
  "special": "Claws (STR+1D+2). Poison: affects any victim stunned or worse by claw attack, causes 5D stun damage for 1D rounds. Enormous bipedal beast with violet fur, horns, large clear fangs, and curving claws. Attacks from treetops by surprise. Flees if wounded, or attacks until it drops one character and drags them off. A complete set of claws and fangs is worth 1,000 credits. Brawling parry 5D+2, search 6D, sneak 6D, brawling 8D.",
  "source": "WEG40151 No Disintegrations"
 },
 {
  "name": "Redbeast",
  "type": "Omnivorous scavenger/hunter",
  "planet": "Reaper's World",
  "habitat": "Urban lowest levels, tundra",
  "dex": "2D",
  "per": "2D",
  "str": "2D",
  "move": "18",
  "size": "1 meter tall, 2 meters long, 1.5 meter tail",
  "orneriness": "2D",
  "special": "Bite (STR+1D). Travel in packs of 2-12. Sinuous quadrupeds with thick coarse gray fur with black spots and crimson eyes. Remarkably enduring and adaptive — more resistant to poison and extreme cold than humans. Strong jaws and chisel-like teeth can gnaw through anything short of durasteel given enough time. Can be frightened off by fire, blaster shots, or explosions. Search 4D, sneak 3D, brawling 3D, climbing/jumping 4D, stamina 6D.",
  "source": "WEG40151 No Disintegrations"
 },
 {
  "name": "Neph",
  "type": "Docile flying beast",
  "planet": "Dalos IV",
  "habitat": "Atmosphere, jungle",
  "dex": "1D-2D",
  "per": "2D",
  "str": "3D-5D",
  "move": "2 (walking), 14 (flying)",
  "size": "2-8 meters thick, 6-24 meters long, 12-48 meter wingspan",
  "orneriness": "1D",
  "special": "Speeder scale. Tail-lash (STR damage). Huge docile flying beasts that keep aloft by generating hydrogen from water vapor and storing it in internal sacs; vent gas to drop in altitude. Maneuver via flat rubbery wings and long rudder-like tail. Feed on insects and pollen permeating Dalos IV's atmosphere. Juveniles make perfect one-person riding mounts (Very Easy beast riding, Easy during trophy chase games). Mature creatures carry up to 20 people in strapped gondolas. Used in the popular trophy chase sport.",
  "source": "WEG40151 No Disintegrations"
 },
 {
  "name": "Trompa",
  "type": "Fierce Predator",
  "planet": "Questal",
  "habitat": "Temperate",
  "dex": "2D",
  "per": "2D",
  "str": "5D",
  "move": "12",
  "size": "2.5-2.9 meters tall",
  "orneriness": "3D",
  "special": "Teeth (STR damage). Claws (STR+2 damage). Distant cousin to the arctic wampa of Hoth, adapted to temperate climates. Adults stand almost three meters tall with arms hanging to the knees and six-inch claws. Grotesque face with simian and reptilian features. In its natural environment, the trompa has no predators and is sovereign over all it surveys. Can move very quietly and strike with speed that belies its size — death is usually instantaneous. Highly developed senses react to the Hurlothrumbic Generator's fear waves, becoming extremely agitated. Can be trained with an electric prod, but remains dangerous if provoked.",
  "source": "WEG40165 Classic Adventures Vol 5"
 },
  {
    name: "Slug-Rat",
    type: "Scavenger",
    planet: "Ruten",
    habitat: "Urban",
    dex: "4D",
    per: "3D",
    str: "2D",
    move: "9",
    size: "0.5-1.0 meters long",
    orneriness: "3D",
    special: "Bite (3D damage). Will attack anybody nearby and flee at the first opportunity if agitated. Considered vermin.",
    source: "WEG40060 Twin Stars of Kira p.69"
  },
  {
    name: "Bear Spider",
    type: "Predator",
    planet: "",
    habitat: "Caves, Forests",
    dex: "3D",
    per: "2D",
    str: "4D",
    move: "14",
    size: "1.5 meters",
    orneriness: "2D",
    special: "Fangs and claws (STR+1D damage). Can weave sticky snares and webs of 3D Strength. Nocturnal predator that lives in caves and weaves webs in surrounding area. Moderate Perception roll to spot webs. Unlikely to attack Humans unless cave is entered or feeding is interrupted.",
    source: "WEG40060 Twin Stars of Kira p.71"
  },
  {
    name: "Glittering Mynock",
    type: "Parasite",
    planet: "",
    habitat: "Space, Various",
    dex: "3D",
    per: "1D",
    str: "1D",
    move: "15 (flying)",
    size: "1.0 meter long",
    orneriness: "1D",
    special: "Power drain: Will mindlessly suck on any nearby power source until drained. When feeding on a power source, becomes entirely docile and emits a rainbow of colored light from its wings — the more power, the brighter the display. Can absorb small energy amounts through its hide (stun bolts are merely a snack, but blaster bolts would kill it). Slightly less revolting than standard mynocks.",
    source: "WEG40060 Twin Stars of Kira p.72"
  },
  {
    name: "Quexius Bird",
    type: "Herbivore",
    planet: "Endor",
    habitat: "Forest",
    dex: "3D",
    per: "2D",
    str: "1D",
    move: "30 (flying)",
    size: "0.5 meters",
    orneriness: "3D",
    special: "Hooked beak (STR+1D damage), normally used for digging roots. Multicolored bright green and red plumage. Beats wings like a hummingbird. Surprisingly aggressive — will peck at nearby beings for no reason and antagonize predators many times its size. Endangered species due to suicidal aggression.",
    source: "WEG40060 Twin Stars of Kira p.71"
  },
  {
    name: "Lagoin",
    type: "Herbivore",
    planet: "Tyed Kant",
    habitat: "Atmospheric (gas giant upper clouds)",
    dex: "2D",
    per: "1D+2",
    str: "1D-12D (varies by size)",
    move: "20",
    size: "Centimeters to 60 meters",
    orneriness: "1D",
    special: "Tentacles: One per meter of length, do Strength damage. Semi-intelligent floating gelatinous ovoids with a single mobile eye and conical filter-mouth. Paddle-like tentacles used for propulsion, stability, and defense. Docile unless injured or confused, then attempt to grasp and crush smaller foes.",
    source: "WEG40066 Supernova p.37"
  },
  {
    name: "Ssither",
    type: "Sapient",
    planet: "Jatee",
    habitat: "Underground Caverns",
    dex: "2D-4D",
    per: "2D+1-4D+1",
    str: "3D-5D",
    move: "10-12",
    size: "1.75-2.5 meters tall",
    orneriness: "2D",
    special: "Telepathy: Can selectively broadcast and receive thoughts and emotions using Ssither Telepathy skill. Cannot read minds — telepathy must be actively used. Difficulty is Moderate plus doubled normal Force proximity/relationship modifiers. Non-Ssither receive +15 modifier unless they have Ssither Telepathy Familiarity. Hairless reptilian beings with serpentine lower bodies, heavily muscled upper torsos, and large gold iris-less eyes. Hunter/gatherers at stone age technology level.",
    source: "WEG40066 Supernova p.77"
  },
  {
   "name": "Arqet",
   "type": "Mountain carnivore",
   "planet": "Pellastrallas",
   "habitat": "Mountains",
   "dex": "3D+2",
   "per": "3D",
   "str": "3D",
   "move": "9 (walking), 12 (charging)",
   "size": "1.8-2.4 meters long, up to 2 meters tall",
   "orneriness": "",
   "special": "Sneak 5D. Armor: dorsal/spinal plates +2D vs physical and energy attacks. Camouflage: skin varies hue with solar radiation, blends with rocky surroundings (included in sneak). Claws: STR+2 damage. Horns: STR+2D damage. Teeth: STR+1D damage. Feigned Immobility: extremely patient, remains motionless to lure prey to point blank range. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.8"
  },
  {
   "name": "Yo'uqiol",
   "type": "Carnivorous mobile plant",
   "planet": "Ooo-sek",
   "habitat": "Swamp, jungle",
   "dex": "0D",
   "per": "1D",
   "str": "1D",
   "move": "2",
   "size": "Normally up to 30 cm across (up to 5 meters in deep jungle)",
   "orneriness": "",
   "special": "Brawling 3D. Poison Tendrils: 5D damage. Digestive Acid: 4D+2 damage. Detects temperature changes, motion, and sound. Regrows within weeks unless completely destroyed. Dormant spores survive 7-8 years. Used by Empire as perimeter 'land mines.' Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.93"
  },
  {
   "name": "Zuxu (Ganlihk)",
   "type": "Aquatic carnivore",
   "planet": "Ganlihk",
   "habitat": "Lakes, rivers",
   "dex": "3D+2",
   "per": "2D+1",
   "str": "2D",
   "move": "18 (swimming), 3 (walking)",
   "size": "Up to 1.1 meters long",
   "orneriness": "",
   "special": "Sneak 3D+1. Swimming 4D. Teeth: STR+3D damage. Protective oil keeps skin moist for up to 3 hours on land. Padded front flippers for land locomotion. Normally solitary but bands together when food is scarce. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.93"
  },
  {
   "name": "Yeomet",
   "type": "Urban pest",
   "planet": "Various (artificial/urban worlds)",
   "habitat": "Space stations, starships, urbanized planets",
   "dex": "4D",
   "per": "2D",
   "str": "1D+1",
   "move": "8",
   "size": "60 centimeters tall",
   "orneriness": "",
   "special": "Teeth: STR+2D damage. Claws: STR+1D damage. Disease Transmission: Moderate stamina/STR or become wounded after 2D hours. Can eat almost anything organic and inorganic (requires EM radiation for full digestive enzyme). Chews through power cables and data wires. Herds of up to 30. Flees when confronted, fights only when cornered. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.92"
  },
  {
   "name": "Winged Xendrite",
   "type": "Repto-avian insectivore",
   "planet": "Xend",
   "habitat": "Forest, urban",
   "dex": "3D",
   "per": "4D",
   "str": "1D",
   "move": "30 (flying), 2 (crawling)",
   "size": "40 cm long, 80 cm wingspan",
   "orneriness": "",
   "special": "Eyesight: +2D PER for sight tasks. Sticky tongue catches insects with 98% accuracy. Found throughout the galaxy as pest control imports, often causing overpopulation problems. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.89"
  },
  {
   "name": "Voroos",
   "type": "Stationary predator",
   "planet": "Various",
   "habitat": "Bogs, marshland, desert, arctic",
   "dex": "4D",
   "per": "2D",
   "str": "4D/6D/8D (depending on size)",
   "move": "0",
   "size": "1-10 meters in diameter",
   "orneriness": "",
   "special": "Camouflage: mimics hills/geological features, Difficult PER to notice before initial attack. Tongue: STR+1D damage, on hit opposed brawling parry vs DEX to avoid grapple; grasped targets pulled into mouth next round (opposed STR to resist). Teeth: half STR damage, extracting from mouth causes additional 3D damage. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.87"
  },
  {
   "name": "Tymp",
   "type": "Nocturnal forager",
   "planet": "Serat",
   "habitat": "Forest, urban",
   "dex": "3D",
   "per": "1D",
   "str": "1D+1",
   "move": "10",
   "size": "0.7 meters (plus 0.7-meter tail)",
   "orneriness": "",
   "special": "Night Vision: +2D PER in low light. Tusks: STR+1D damage. Climbing: +2D climbing when using prehensile tail under extreme stress. Urban subspecies is more aggressive and can digest roti grain. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.86"
  },
  {
   "name": "Two-Headed Tortuce",
   "type": "Amphibian herbivore",
   "planet": "Canastra IV",
   "habitat": "Coastal, aquatic",
   "dex": "2D+1",
   "per": "3D+2",
   "str": "2D+2",
   "move": "4 (walking), 8 (swimming)",
   "size": "0.8-1.0 meters long",
   "orneriness": "",
   "special": "Sneak 5D+1. Brawling 3D, swimming 4D. Armor: shell +1D to resist all physical/energy. Dual Brained: two attacks at no penalty; 3rd+ actions suffer normal multi-action penalties. Heightened Smell: +1D search. Jaws: STR+1D damage. Lifespan ~100 years. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.85"
  },
  {
   "name": "Troos Armored Crebik",
   "type": "Arboreal arthropod",
   "planet": "Troos",
   "habitat": "Sub-arctic forest, arboreal",
   "dex": "2D",
   "per": "2D+2",
   "str": "3D",
   "move": "6",
   "size": "1 meter long",
   "orneriness": "",
   "special": "Falling Attack: 6D+2 damage, Very Difficult PER to avoid surprise. Pincers: STR+2D damage. Mandibles: STR+2 damage. Thick keratin armor shell. Vulnerable to bahl fly infestation through shell cracks. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.84"
  },
  {
   "name": "Tresher (Tinallis)",
   "type": "Avian cliff dweller",
   "planet": "Tinallis",
   "habitat": "Coastal cliffs",
   "dex": "3D+2",
   "per": "4D",
   "str": "5D+2",
   "move": "6 (walking), 18 (flight), 55 (diving attack)",
   "size": "1.8-2.4 meters tall, up to 3.5-meter wingspan",
   "orneriness": "",
   "special": "Dodge 7D. Search 6D. Acute Vision: +2D visual search. Bite: STR+2 damage. Talons: STR+2 damage. Tail: STR+1 damage (bony-ridged tailswipe). Diving Attack: Move 55 (160 km/h), +1D attack damage. Extremely territorial, mates for life. Protected species. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.82"
  },
  {
   "name": "Thanu",
   "type": "Carnivorous volcano dweller",
   "planet": "Kamur",
   "habitat": "Volcanic slopes",
   "dex": "3D",
   "per": "1D+2",
   "str": "2D",
   "move": "4",
   "size": "1.1 meters tall",
   "orneriness": "",
   "special": "Heat Immunity: +1D STR to resist blaster damage, can traverse any solid surface regardless of temperature. Tentacles: STR damage, on hit Moderate DEX or entangled; each round opposed STR to avoid being pulled into mouth; escape requires STR 10+ points higher than thanu. Teeth: STR+2D damage, also requires STR 10+ higher to escape mouth. Moves by rolling on spherical stone feet. Hunts in packs. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.81"
  },
  {
   "name": "Tentacle Bird of Pelemax",
   "type": "Flightless jungle avian",
   "planet": "Pelemax",
   "habitat": "Jungle",
   "dex": "3D",
   "per": "2D",
   "str": "3D",
   "move": "16 (running), 10 (climbing/tree movement)",
   "size": "1.6-2.5 meters tall, up to 3 meters long, tentacles to 3 meters",
   "orneriness": "2D (native Pelemaxians), 3D (non-native riders)",
   "special": "Running 4D. Intimidation: war cry 6D. Hide 4D. Climbing/jumping 4D, stamina 5D. Beak: STR+2 damage. Talons: STR+1D damage. Tentacles: +1D climbing/jumping/running. War Cry: intimidation succeed by 5+ = target stunned for 2 rounds. Water Storage: can go up to 1 standard month without water. Used as mounts and beasts of burden. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.80"
  },
  {
   "name": "Telkadis Hiding Tree Spider",
   "type": "Predatory arachnid",
   "planet": "Telkadis",
   "habitat": "Forest (taku trees)",
   "dex": "2D+2",
   "per": "2D+1",
   "str": "3D+1",
   "move": "12",
   "size": "60-80 centimeters in diameter",
   "orneriness": "",
   "special": "Hide 3D+2, search 4D, sneak 3D. Digging: tree burrowing 5D+1. Silent Movement: +1D+2 to all sneak. Sense: heat detection, +1D search within 20 meters. Camouflage: +3D sneak when burrowed in bark. Poison: bite does 3D damage, Very Difficult stamina or temporary paralysis (up to 1 hour). Colonies attack if host tree threatened. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.79"
  },
  {
   "name": "Tedellian Besiioth",
   "type": "High-gravity hunter",
   "planet": "Tedel",
   "habitat": "Plains",
   "dex": "1D+2",
   "per": "2D",
   "str": "4D+2",
   "move": "12 (high gravity), 18 (standard gravity)",
   "size": "Up to 2 meters long",
   "orneriness": "",
   "special": "Search 4D+2. Brawling 6D+2, climbing/jumping 5D+2. Visual Tracking: spots prey from kilometers away in open terrain, infrared vision. Intimidating Gaze: intimidation 7D; succeed by 10+ = target incapable of action for 1 round. Claws: STR+1D damage. Bite: STR+1D damage. 7 subspecies including 'stalker besiioth' with enhanced smell. Often used as guard beasts with neural inhibitor/controllers. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.78"
  },
  {
   "name": "Tantla",
   "type": "Forest scavenger",
   "planet": "Ealor",
   "habitat": "Forest, desert, arctic",
   "dex": "4D+2",
   "per": "2D+2",
   "str": "2D+2",
   "move": "12",
   "size": "1-1.3 meters tall, up to 3 meters long",
   "orneriness": "",
   "special": "Running 6D+2. Hide 4D, sneak 4D+2. Claws: STR+1 damage. Tail: STR+2 damage (barbed). Teeth: STR+1 damage. Grapple Tongue: 1m+ sucker-tipped tongue, Difficult STR to break free or -1D DEX. Extremely Protective: adults defending young get +2D STR for causing and resisting damage. Restricted Vision: -1D search vs stationary targets. Packs of 10-12. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.77"
  },
  {
   "name": "Svaper",
   "type": "Underwater voracious threat",
   "planet": "Rodia",
   "habitat": "Aquatic",
   "dex": "4D",
   "per": "2D",
   "str": "2D",
   "move": "20 (swimming)",
   "size": "2-3 meters long (captive adults up to 6 meters)",
   "orneriness": "",
   "special": "Bite: STR+3D damage. Spines: STR+3D+2 damage, remain lodged until target spends a turn removing. Tough Skin: +2D to resist damage (gills unprotected but Very Difficult to hit). Wraps around prey to secure while feeding. Illegal to own in most systems. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.76"
  },
  {
   "name": "Stalker Lizard (Dantooine)",
   "type": "Plains hunter",
   "planet": "Dantooine",
   "habitat": "Plains",
   "dex": "1D",
   "per": "2D",
   "str": "3D",
   "move": "40 (sprint), 13 (normal)",
   "size": "Up to 3.5 meters long",
   "orneriness": "",
   "special": "Search 3D+2, sneak 5D. Brawling 4D. Constriction Attack: STR+2D+2 damage. Solitary hunter, relies on surprise. Hunts nettars at dawn/dusk, covers up to 50km per night tracking prey. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.74"
  },
  {
   "name": "Stiltwalker",
   "type": "Amphibious insectivore",
   "planet": "Various (swamp worlds)",
   "habitat": "Swamp, shallow water",
   "dex": "3D",
   "per": "1D",
   "str": "1D+1",
   "move": "3",
   "size": "15 centimeters tall",
   "orneriness": "",
   "special": "Eyesight: +2D PER for all tasks involving sight. Force Sense: when a character uses the Force nearby, 2D stiltwalkers per die of control skill appear within 5 rounds and chirp loudly for up to 10 minutes. Males cannot swim, chase insects on long legs. Females are aquatic bottom-dwellers with tentacles. Males can be trained as guardian creatures (early warning). Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.75"
  },
  {
   "name": "Spor Crawler",
   "type": "Poisonous insect",
   "planet": "Nar Bo Sholla",
   "habitat": "Various (sand, dirt, urban hiding spots)",
   "dex": "0D+2",
   "per": "0D+2",
   "str": "0D+2",
   "move": "1",
   "size": "8 centimeters long",
   "orneriness": "",
   "special": "Poison: 5D damage (rolled every 5 min for 1 hour). Difficult stamina or -3D penalty to all actions for 6 hours. Moderate first aid with medpac neutralizes poison but not pain. Burrowing: digs in soft materials. Camouflage: Difficult search/PER to spot before attack; characters familiar with Nar Bo Sholla may make Easy survival to reduce search difficulty by 2 levels. Flounders on smooth surfaces. Easily killed if stepped on. Hives of 5-50. Prized by assassins.",
   "source": "WEG40080 Creatures of the Galaxy p.73"
  },
  {
   "name": "Somago (Choker)",
   "type": "Modified annelidian carnivore/scavenger",
   "planet": "Tenek",
   "habitat": "Forest, arboreal",
   "dex": "4D",
   "per": "2D",
   "str": "3D+1",
   "move": "4",
   "size": "50 centimeters long",
   "orneriness": "",
   "special": "Choking Attack: if strikes head, 3D damage per round until killed or removed. Hooks: STR+1D damage. Forms: flat sheet (catches small herbivores), saddle (long-necked prey), tapestry (flying creatures), helmet (humanoids, rarest — 35cm diameter tube). Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.71"
  },
  {
   "name": "Slork",
   "type": "Disgusting scavenger",
   "planet": "Kidron",
   "habitat": "Marshland",
   "dex": "1D",
   "per": "3D",
   "str": "4D",
   "move": "3",
   "size": "1-1.5 meters tall",
   "orneriness": "",
   "special": "Grubbers: STR+1D damage. Stench: nauseates organic beings within 10m, Difficult stamina or incapacitated; breathers reduce to Moderate. Blubber: +1D to resist damage; snout shots bypass blubber but +1 difficulty level to hit. Nearly blind, relies on smell via trunk; damaged snout = effectively blind. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.71"
  },
  {
   "name": "Slimy Nonakara",
   "type": "Amphibious omnivore",
   "planet": "Various",
   "habitat": "Lakes, swamps",
   "dex": "2D+2",
   "per": "1D+1",
   "str": "2D",
   "move": "10 (swimming), 2 (crawling)",
   "size": "20 cm to 4 meters long (varies by body of water)",
   "orneriness": "",
   "special": "Slime: Easy STR or 1D damage per round of contact. Teeth: STR+1D damage. Larval Implantation: 1 in 6 chance in nonakara-inhabited water; larva burrows into muscles, -1D STR after 4-16 hours, additional -1D STR every 10 days; STR 0 = death. Removal: Moderate medicine with surgical facilities. Recovery: 12-20 days rest in total care. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.69"
  },
  {
   "name": "Sliideptra",
   "type": "Arboreal invertebrate carnivore",
   "planet": "Tel IV",
   "habitat": "Rain forest, arboreal",
   "dex": "1D",
   "per": "3D",
   "str": "1D",
   "move": "2 (up to 7 meters from tree limb)",
   "size": "40 centimeters long",
   "orneriness": "",
   "special": "Wide Angle Vision: +2D PER for sight tasks. Poison Spray: 7D damage, once per 50 hours. Fail STR by 15+ = death. Fail by 1-14 = -1D penalty to DEX/MEC/STR/TEC (neural damage); penalty increases -1D per 5 hours untreated; when DEX penalty equals DEX, total paralysis. Medpac cures (Very Difficult first aid) but poison lingers — Easy STR roll every 20 hours for 1 week or relapse. Eliminate poison: Moderate medicine or Easy Ho'Din herbal medicines. Contact causes permanent bald scar tissue on exposed skin. Flesh is also poisonous if consumed. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.68"
  },
  {
   "name": "Slar",
   "type": "Leaping hunter",
   "planet": "Port Evokk",
   "habitat": "Jungle, rain forest",
   "dex": "2D",
   "per": "2D",
   "str": "3D",
   "move": "8 (walking), 15 (leaping horizontally), 40 (leaping vertically)",
   "size": "1.4 meters tall, 3 meters long (with tail)",
   "orneriness": "",
   "special": "Brawling parry 7D, dodge 3D. Brawling 4D, climbing/jumping 6D, lifting 5D. Leaping: up to 15m vertical or 40m horizontal (Moderate difficulty). Tail: wrap around tree limb for anchor, +1D brawling parry/brawling/lifting but cannot dodge or move more than 2m. Claws: STR+1D damage, on hit opposed brawling vs brawling parry to pin victim. Razor Quills: STR+2D damage. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.69"
  },
  {
   "name": "Skeeg",
   "type": "Predatory mollusk",
   "planet": "Vendara",
   "habitat": "Swamp, rain forest",
   "dex": "1D+2",
   "per": "1D",
   "str": "2D+1",
   "move": "0.5",
   "size": "17 centimeters long",
   "orneriness": "",
   "special": "Stingers: 1D+1 damage plus poison. Poison: Easy STR or -1D penalty on all STR actions (excluding resist damage); cured by 4 hours rest or medpac. Scent: within 1 meter, Moderate PER — those who notice become drowsy, -1D DEX while nearby and 4-14 rounds after leaving. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.66"
  },
  {
   "name": "Shredder Bat",
   "type": "Flying predator",
   "planet": "Atrisia",
   "habitat": "Mountains, plains, forest, jungle",
   "dex": "1D+2",
   "per": "5D",
   "str": "2D+2",
   "move": "18 (flying)",
   "size": "Up to 1 meter long, up to 1 meter wingspan",
   "orneriness": "",
   "special": "Search: tracking 7D, sneak 6D+2. Brawling 3D+2, flight 4D. Hearing: detects prey up to 15km, no eyes, composite hearing-based image of surroundings (reflected in PER skills). Fangs: STR+1D damage, dive-bombing attack STR+2D+2 damage. Hunts in packs of 20+, swarms of hundreds reported. Found on 1,000+ worlds. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.65"
  },
  {
   "name": "Sensor Star",
   "type": "Marsh creature",
   "planet": "F'tral",
   "habitat": "Marsh, swampland",
   "dex": "0D",
   "per": "0D",
   "str": "0D+2",
   "move": "2",
   "size": "Up to 25 centimeters across",
   "orneriness": "",
   "special": "Search 1D+2. Subsonic Communication: detectable by sensors at up to 350 meters. Sensitive Receptors: detects wide array of EM communication, useful as guard creatures. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.64"
  },
  {
   "name": "Selligore",
   "type": "Aquatic grazing mammal",
   "planet": "Corsin",
   "habitat": "Coastal, shallow aquatic",
   "dex": "1D",
   "per": "2D",
   "str": "4D",
   "move": "13 (swimming), 4 (walking)",
   "size": "Up to 20 meters long, up to 4 meters tall",
   "orneriness": "2D",
   "special": "Swimming 6D. Harmless herbivore — feeds on fish, plankton, and plants. Incredible stamina, survives hours between breaths. Can survive briefly on land but dehydrates quickly. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.63"
  },
  {
   "name": "Sanl j'ek",
   "type": "Communal forest herbivore",
   "planet": "Dancreti",
   "habitat": "Tropical swamp jungle",
   "dex": "4D",
   "per": "6D",
   "str": "1D",
   "move": "12 (walking), 40 (frenzy)",
   "size": "0.2-0.3 meters tall",
   "orneriness": "",
   "special": "Dodge 5D. Hide 7D, search 8D, sneak 7D. Climbing/jumping 4D. Claws: STR+2 damage. Birth Pouch: females carry young in rib cage pouch, +1D protection for infant vs physical/energy. Communal Loyalty: if any member attacked, entire community frenzies with +2D to all physical attacks and Move 40; use combined action bonuses at +3 levels. Enhanced Auditory Sense: detect heartbeats of pride members at 25-30km, -2D to search for own community, +2D to hide/sneak vs other creatures, +3D PER for initiative. Silent Movement: +1D sneak if making only one move per round. Creature scale.",
   "source": "WEG40080 Creatures of the Galaxy p.62"
  },
  {
   "name": "M'ven (Swooper)",
   "type": "Herbivore",
   "planet": "Goroth",
   "habitat": "All terrains except polar regions",
   "dex": "2D",
   "per": "3D+1",
   "str": "1D",
   "move": "18 (flying), 6 (walking)",
   "size": "0.25 meters long, 0.6 meter wingspan",
   "orneriness": "",
   "special": "Dodge 6D. Small avian with coarse black feathers and blood-red chest patch. Forms raucous flocks of up to 50 individuals. Highly territorial — swoops at intruders screaming loudly but never physically attacks. Both Gorothites and Imperials use m'ven rookeries as natural intrusion alarms that cannot be jammed; screams audible over 1 km. Feeds on a single common moss species. First-order heterotroph.",
   "source": "WEG40098 Goroth Slave of the Empire p.91"
  },
  {
   "name": "Highglide",
   "type": "Scavenger",
   "planet": "Goroth",
   "habitat": "Mountains, lowlands",
   "dex": "1D",
   "per": "4D",
   "str": "2D",
   "move": "16 (flying), 8 (walking)",
   "size": "0.8 meters long, 1.8 meter wingspan",
   "orneriness": "",
   "special": "Dodge 4D. Search 5D, sneak 5D. Silent Movement: +1D+2 to sneak when diving. Claws: STR+1 damage (two talon attacks when diving, normal multi-action penalties). Beak: STR+1 damage (bite only, not when stooping). Solitary high-flying avian scavenger, brilliant emerald-green with faint metallic blue highlights. Roosts in mountains, ranges far over lowlands spotting carrion with exceptional eyesight. Can drive most predators away from kills. Highly protective of nests — attacks anything approaching. Second-order heterotroph.",
   "source": "WEG40098 Goroth Slave of the Empire p.91"
  },
  {
   "name": "Likka",
   "type": "Riding Beast",
   "planet": "Masterhome",
   "habitat": "Forests, plains",
   "dex": "2D",
   "per": "2D",
   "str": "4D",
   "move": "12",
   "size": "1.5 meters tall at the shoulder, 3 meters long",
   "orneriness": "3D",
   "special": "Brawling: bite 5D. Bite: STR+1 damage. Long-legged crocodilians, greenish-brown with brilliant yellow eyes. Ill-tempered riding beasts of the Anointed People, controlled by reins attached to the likka's eyelids. Creature scale.",
   "source": "WEG40108 Classic Campaigns p.40"
  },
  {
   "name": "Staplarint",
   "type": "Omnivore (riding beast)",
   "planet": "Brosi",
   "habitat": "Forests",
   "dex": "3D+2",
   "per": "2D",
   "str": "5D",
   "move": "12",
   "size": "1.5 meters at shoulder, 2.5 meters long",
   "orneriness": "2D+2",
   "special": "Running: long distance 4D+2. Bite: STR+1D damage. Originally introduced to Brosi by Republic settlers as beasts of burden. Escaped into the wilderness centuries ago when repulsorlift transports replaced them. Not domesticated for centuries, but the Brosin Underground has begun breaking them in as mounts with surprisingly successful results. Creature scale.",
   "source": "WEG40109 Alliance Intelligence Reports p.12"
  },
  {
   "name": "Mosgoth",
   "type": "Aerial Reptilian Mount",
   "planet": "Togoria",
   "habitat": "Mountains, plains",
   "dex": "4D",
   "per": "3D",
   "str": "4D",
   "move": "7 (walking), 28 (flying)",
   "size": "12-17.5 meters long",
   "orneriness": "3D",
   "special": "Search 5D. Brawling 5D. Claws: STR+2 damage. Teeth: STR+1D damage. Tail: STR damage (weapon on ground, rudder in flight). Night Vision: organic sonar allows navigation in complete darkness with no penalty. Four-winged flying reptiles from Togoria. Togorians and mosgoths evolved a symbiotic relationship — ancient Togorians defended nests from predators, mosgoths built nests closer to camps. Used as mounts and beasts of burden. Can carry ~300 kg in flight, max 2 riders due to wing placement. Will fight alongside their Togorian companion. Speeder scale.",
   "source": "WEG40109 Alliance Intelligence Reports p.74"
  },
  {
   "name": "Methnap",
   "type": "Domesticated Cetacean",
   "planet": "Chad",
   "habitat": "Oceans",
   "dex": "2D",
   "per": "2D",
   "str": "3D+2",
   "move": "12",
   "size": "12-15 meters",
   "orneriness": "1D",
   "special": "Sea-Singing: Long-range communication allowing reasonably complex exchanges of information; with extended exposure, riders can develop understanding of basic methnap concepts. Calm cousins of the tsaelke and cy'een of Chad's oceans. Graceful barrel-bodied cetaceans with long necks and small heads, brass-colored hide with unique mottled camouflage patterns of stripes, spots, and rings. Fish-grazers. Can bear up to 125 kg or a rider and 75 kg. Used as mounts by at least one Alliance sea commando unit.",
   "source": "WEG40113 Rules of Engagement p.54"
  },
  {
   "name": "Tuggle",
   "type": "Domesticated Sledge Beast",
   "planet": "Toola",
   "habitat": "Arctic, tundra",
   "dex": "2D",
   "per": "1D",
   "str": "2D",
   "move": "12 (unhitched), 10 (hitched)",
   "size": "0.5 meters tall, 1.5 meters long",
   "orneriness": "2D",
   "special": "Lifting: pulling 4D, stamina 4D. Bite: STR+2D damage. Lightly colored, long and wiry carnivores with short powerful legs and thick fur. Six hitched in tandem can pull two riders and up to 50 kg of baggage. Wild tuggles are pack hunters with considerable stamina, used to harry, drive, and pull down prey. Domestic tuggles can draw sleds all day with only a couple of rest breaks, for days on end if fed regularly. Popular with arctic Wilderness Fighters. Used to draw Whiphid sledges.",
   "source": "WEG40113 Rules of Engagement p.55"
  },
  {
   "name": "Newoongall",
   "type": "Predator",
   "planet": "Rodia",
   "habitat": "Caves",
   "dex": "2D",
   "per": "3D",
   "str": "3D+2",
   "move": "13",
   "size": "Large (arachnid)",
   "orneriness": "",
   "special": "Sneak 6D, Brawling 4D+2, Climbing/Jumping 6D. Clawed legs: STR+2D damage. Large arachnids that cling to cavern ceilings and drop down on prey from above. Found in cave networks throughout Rodia's jungle mountains.",
   "source": "WEG40134 Shadows of the Empire Planet Guide p.29"
  },
  {
   "name": "Ganjuko",
   "type": "Predator",
   "planet": "Bothawui",
   "habitat": "Arctic wastelands",
   "dex": "1D",
   "per": "2D",
   "str": "4D",
   "move": "13",
   "size": "Up to 3.5 meters tall, up to 5 meters long (Creature scale)",
   "orneriness": "",
   "special": "Sneak 3D+1, Climbing/Jumping 4D+2. Beak: STR+1D damage, digging 5D, +1D physical/energy to resist damage to head. Temperature Sensitivity: Above 5°C for 1 hour: -1D STR, -3 Move; after 1 day: -2D STR, -10 Move; after 1 week: hibernation until temps drop. Solitary, extremely territorial (200 sq km hunting grounds). Retractable cartilage eye lids — fights by scent and sound. Takes brief naps instead of sleeping — active day and night. Lifespan up to 110 years, female lays only 4-8 eggs in lifetime. Beaks carved into expensive dagger blades popular among wealthy Bothan clan leaders. Weighs over 600 kg.",
   "source": "WEG40134 Shadows of the Empire Planet Guide p.59"
  },
  {
   "name": "Halkra (Strangle Vine)",
   "type": "Arboreal carnivore",
   "planet": "Bothawui",
   "habitat": "Forests",
   "dex": "1D",
   "per": "3D",
   "str": "2D+2",
   "move": "3",
   "size": "Body 2 meters, tentacles up to 8 meters (Creature scale)",
   "orneriness": "",
   "special": "Dodge 4D+2, Climbing 6D. Camouflage: Very Difficult search to detect body — bark-like skin with moss and seedlings growing on it, drapes tentacles from trees. Multiple Attacks: up to 4 targets at once with no penalty. Brawling = 1D per tentacle used (4 tentacles = 4D, 5 = 5D, etc.). Tentacle Crushing: ensnared target must make opposed STR roll to break free; failure = damage. No teeth — swallows prey whole, digests via stomach acid. Can digest a full-grown Bothan in 36 hours. Lures predators by shaking underbrush to simulate wounded animal; stays still for herbivores. No night vision — bright light causes pain.",
   "source": "WEG40134 Shadows of the Empire Planet Guide p.60"
  },
  {
   "name": "Krakjya",
   "type": "Intelligent predator",
   "planet": "Bothawui",
   "habitat": "Forests, mountains",
   "dex": "4D",
   "per": "3D",
   "str": "5D",
   "move": "12, 3 (jumping)",
   "size": "2 meters long, 1.3 meters tall (Creature scale), 300+ kg",
   "orneriness": "",
   "special": "Search: Tracking 6D, Sneak 5D, Brawling 7D. Claws: STR+2D damage. Climbing/Jumping: +2D climbing, +1D jumping, leaps up to 3m height. Charging: brawling roll vs every person/animal in path, stun damage = STR. Camouflage: +1D sneak in native environment. Roar (first round): 5D intimidation vs willpower/Perception — tie or +1-3 = -1D all actions 1 round, +4-8 = -1D for 3 rounds, +9 = paralyzed 3 rounds. Senses: +2D to search/Perception (negated if target perfectly still or using stinkweed). Hunt in mated pairs — male strikes first, female attacks from opposite direction. Protected species — 10,000 credit permit for capture only, no lethal weapons. Killing punishable by death. Mated pairs worth 150,000 credits to zoos. Fur varies by geography (forest green with gray stripes to muddy red with white spots). Lifespan ~50 years.",
   "source": "WEG40134 Shadows of the Empire Planet Guide p.61"
  },
  {
   "name": "Skar'kla (Glitterclaw)",
   "type": "Mountain predator",
   "planet": "Bothawui",
   "habitat": "Mountains, snow-covered peaks",
   "dex": "3D",
   "per": "2D",
   "str": "3D+1",
   "move": "14",
   "size": "1.6 meters long, 1.2 meters tall",
   "orneriness": "",
   "special": "Search: Tracking 4D, Sneak 4D+2, Brawling 5D, Climbing/Jumping 4D+1. Claws: STR+1D+1 damage, +1D to climbing. Leap Attack: +1D brawling and +1D damage on initial leaping attack. Fur: withstands freezing without shelter; +1D sneak in snowy areas. Distantly related to krakjya but lacks its strength. Prolific litters of 8-9 kittens. Lives in prides of 6-10 adults, hunts in packs of 3-4. Near-extermination of krakjyas has allowed them to spread into lowland valleys. Healthy fear of civilization but will attack lone individuals in wilderness.",
   "source": "WEG40134 Shadows of the Empire Planet Guide p.62"
  },
  {
   "name": "Nashtah",
   "type": "Predator",
   "planet": "Various",
   "habitat": "Various",
   "dex": "4D",
   "per": "3D",
   "str": "5D",
   "move": "12 (running), 15 (leaping)",
   "size": "1.3 meters",
   "orneriness": "",
   "special": "Search 4D, Brawling 6D, Climbing/Jumping 6D+2. Claws: STR+1D damage. Vicious predators sometimes kept as pets or guard animals. Klirun, a nashtah belonging to Tunroth Hunter Shotarr Kass, once savagely mauled an assailant who attempted to ambush Vigo Sprax.",
   "source": "WEG40136 Secrets of the Sisar Run p.5"
  },
  {
   "name": "Blue Desert People",
   "type": "Plains Lizard",
   "planet": "Dathomir",
   "habitat": "Desert / Grasslands",
   "dex": "4D",
   "per": "2D+2",
   "str": "4D+1",
   "move": "30",
   "size": "1.6-2.4 meters",
   "orneriness": "3D",
   "special": "Running 5D+1, Hide 3D, Sneak 4D+2. Vision: Excellent night vision, ignore all darkness penalties. Claws: STR+1D damage. Scale: Creature. Large reptilian herbivores that cross the Great Desert of Dathomir twice daily — hiding in shallow crags near the desert boundary during daytime, then racing 80 km across the desert at night to feed in fertile grasslands beyond. Light blue with lavender scales on head plates. Several rows of black spade-like teeth for rooting up plants. Deep skin-folds below mouth store water. Some xenobiologists argue they may be sentient, citing coordinated herd communication and possible verbal directions from perimeter beasts.",
   "source": "WEG40139 Cracken's Threat Dossier p.50"
  },
  {
   "name": "Dathomirian Rancor",
   "type": "Evolved Predator",
   "planet": "Dathomir",
   "habitat": "Forests / Plains",
   "dex": "4D/6D",
   "per": "1D/2D+2",
   "str": "5D/8D",
   "move": "25/40",
   "size": "8-10 meters",
   "orneriness": "",
   "special": "Attribute Dice: 14D. KNO 0D/1D, MEC 0D, TEC 0D/1D. Armor: +3D to STR to resist physical and energy damage. Claws: STR+3D damage. Teeth: STR+5D damage. Scale: Creature. Dexterity Skills — Melee weapons: clubs, trees (wield large objects as melee weapons, STR+1 Walker-scale damage). Thrown weapons: boulders, nets, trees (hurl objects 20-30/50/100 meters, 2D Walker-scale damage). Knowledge Skills — Symbolic writing (understand simple pictographic commands written by Witches of Dathomir; 4D+ means rancor can write as well). Story Factors — Evolved: have evolved over thousands of years on Dathomir; off-world rancors will likely not be identical. Domesticated: most Dathomirian rancors have been domesticated by human clans, treated as loved pets and friends. Organized into tribe-like 'herds' led by matriarchal Herd Mothers. Herds allied with Witch clans have grown strong and wise. Possess rudimentary 'racial memory' and abstract thought. Witch clans have taught rancors to make nets, spears, wear battle armor, and some to read/write simple pictographic language. Rancor 'mutants' near Witches have become increasingly intelligent.",
   "source": "WEG40139 Cracken's Threat Dossier p.52"
  },
  {
   "name": "Wuffa Worm",
   "type": "Giant Worm",
   "planet": "Dathomir",
   "habitat": "Subterranean",
   "dex": "1D",
   "per": "1D",
   "str": "4D+2/6D",
   "move": "11",
   "size": "100-250 meters",
   "orneriness": "",
   "special": "Search 2D+1. STR dependent on length. Strong Hide: hide of a dead wuffa makes incredibly strong rope (treat as STR 3D when resisting damage). Armor: +3D against physical attacks. Scale: Creature. Giant burrowing mineral-eaters living in inner recesses of planet's crust, surfacing only after severe flooding invades subterranean home. Distinct males and females (not asexual). Dathomirians hunt them by filling wuffa holes with fermented liquor smelling like a female wuffa's mating trail, luring males to the surface. Hides make excellent ropes and protective clothing; inner meat makes tasty stew. A large wuffa can feed an entire clan for two weeks.",
   "source": "WEG40139 Cracken's Threat Dossier p.53"
  }
 ,{
   "name": "Krakana",
   "type": "Deep-sea predator",
   "planet": "Mon Calamari",
   "habitat": "Deep ocean trenches",
   "dex": "2D",
   "per": "2D",
   "str": "6D",
   "move": "24 (swimming)",
   "size": "14.5-26 meters",
   "orneriness": "5D",
   "special": "Scale: Speeder. Tentacles: Each does 4D+1 damage. If more than three tentacles hit a diver, the diver is caught fast and drawn into the mouth on the following turn. Teeth: Crushing jaws do STR+2D damage. Sneak 7D. One of the most feared predators on Calamari, dwelling in deep-sea trenches like Rako's Rift.",
   "source": "WEG40153 Wretched Hives p.79"
  }
 ,{
   "name": "Ziakas (Derriphan)",
   "type": "Dark side parasite",
   "planet": "Adarlon",
   "habitat": "Parasitic — inhabits sentient hosts",
   "dex": "5D",
   "per": "7D",
   "str": "0D",
   "move": "18 (flying, natural form)",
   "size": "Variable (natural form is crackling sphere of ebony energy)",
   "orneriness": "6D",
   "special": "Force Skills: Control 8D, Sense 8D+1, Alter 7D. Innate Force Powers (no prerequisites needed): Control: Absorb/dissipate energy. Sense: Sense Force potential. Sense+Alter: Lesser force shield. Alter: Telekinesis. Control+Alter: Aura of uneasiness, Force lightning. Control+Sense+Alter: Control mind, telekinetic kill, memory wipe. | Derriphan are ancient Sith-spawned parasites — crackling spheres of ebony energy that devour the thoughts and experiences of sentient beings. Probing: Sense roll vs target's Perception/control to enter victim. Hosting: Easy control roll vs target's Perception/control at -1D (penalty increases by -1D each day). Feeding: One drain per day in preference order — Force skills, Force Points, Dark Side Points, Character Points, Knowledge/Perception/Technical skills, then attributes. Drains convert to Dark Side Points (Derriphan's lifeblood — must spend 1 DSP/day to survive). Can be sensed by Jedi (not Dark Jedi) within 10m as 'a source of great evil.' Destroyed by Jedi using transfer Force (removes control dice +1 Dark Side Points). 9 Force Points, 40 Dark Side Points, 35 Character Points. Dodge 10D, alien species 11D, cultures 10D, languages 8D+1, planetary systems 9D, streetwise 11D, bargain 8D+2, command 9D, con 12D+1, persuasion 8D, search 9D, sneak 11D. Currently hosting Kandria A'Daasha at Glow Dome.",
   "source": "WEG40153 Wretched Hives p.126"
  },
  {
   "name": "Mutriok",
   "type": "Desert scavenger and hunter",
   "planet": "Socorro",
   "habitat": "Doaba Badlands and open desert wastes",
   "dex": "1D",
   "per": "5D",
   "str": "2D+1",
   "move": "12 (walking), 16 (running)",
   "size": "0.79-0.92 meters tall at the shoulder",
   "scale": "Creature",
   "orneriness": "",
   "special": "Skills: Dodge 4D+2, running 6D, hide 5D+1, search 6D+1, sneak 5D+2, brawling 4D, climbing/jumping 5D+1, stamina 4D, swimming 4D+2. Paws do STR damage. Teeth do STR+1D damage. Mutrioks gain +1D on all search and Perception rolls in the open desert. | Slender, swift canine hunters of the open deserts. Long-legged and agile, often found running in packs in the deep wastes of the Doaba Badlands. Gray or blue eyes set at the base of a long snout, lending a friendly expression. Uncanny sense of smell and direction; roam between volcanic basements seeking water, Zsajhira berries, and chiru insects. Withstand desert heat for extended periods, making them favorite companions of desert guides and tribesmen who use them to detect Trauger gas fields, find water grottoes, and locate hidden Zsajhira berry caches. Fiercely loyal to their owners and bond with one specific person; friendly and playful in communities, particularly with young children. Cherished by native tribes but captured and sold off-world by unscrupulous traders. Adults: 200-500 credits. Puppies: 50-200 credits depending on age and birth weight.",
   "source": "WEG40154 Black Sands of Socorro p.11"
  },
  {
   "name": "Socorran Tailring",
   "type": "Desert dweller",
   "planet": "Socorro",
   "habitat": "Volcanic regions of the Doaba Badlands",
   "dex": "4D",
   "per": "4D",
   "str": "1D+1",
   "move": "16",
   "size": "0.3-1.9 meters long",
   "scale": "Creature",
   "orneriness": "",
   "special": "Skills: Brawling parry 5D, dodge 4D+2, pick pocket 5D, hide 6D, search 8D+1, sneak 5D, brawling 2D+2, stamina 5D, swimming 2D. Claws do STR+1D damage. Teeth do STR+1D damage. Sting (bite): extremely painful but rarely fatal — 4D damage rolled against victim's Strength; if a Wound results, roll 3D for hours of nausea and fever. Tail does STR damage; tailrings rattle their tails when frightened before striking. Constriction: when extremely agitated, the tailring wraps its tail around the nearest limb (often the neck) and squeezes — STR+4D damage once per minute, victim must make an opposed lifting/Strength roll and beat the tailring by 5+ points to break free. Death is certain on a neck constriction even if the tailring is killed. | Also known as Socorran sand dragons, symbols of strength to Socorro's pirate factions. Live in hordes of 7-8 with no more than two males co-existing. Surprisingly affectionate when domesticated, life-long companions once bonded. Black fur is most common; fawn and gray also seen. Molt at each stage of maturity; tail rattles indicate age — one segment per 10 years. Lifespan 50-80 years. Highly prized for rattles and supple skins, but hunting is strictly forbidden — poaching is punished by banishment to the deep desert with no provisions (certain death). Highly intelligent, perceptive, inquisitive, and easily distracted by shiny objects — cunning thieves. Few natural enemies, though they have been known to attack and kill mynocks for unknown reasons. Socorran pirates value keeping one or more aboard ship to protect crafts from mynock infestation.",
   "source": "WEG40154 Black Sands of Socorro p.12"
  },
  {
   "name": "Druyza",
   "type": "Domesticated laborer",
   "planet": "Socorro",
   "habitat": "Desert grottoes and dormant volcanic cellars in the Doaba Badlands",
   "dex": "2D",
   "per": "2D",
   "str": "7D",
   "move": "6",
   "size": "1.5 meters tall, 2.44 meters long",
   "scale": "Creature",
   "orneriness": "1D+2",
   "special": "Skills: Search 3D+2, brawling 5D, lifting 9D+1, stamina 9D, swimming 8D. Head-Butt does STR damage. Teeth do 3D damage. Kick does STR+1D damage. Druyza can hold their breath underwater for up to one hour. | A homely, ponderous beast that looks pieced together from other creatures' parts. Furless desert wanderer, compact and low to the ground, moves with the grace of a crippled dewback. Extremely docile, used by nomads for transportation and as beasts of burden — excels due to mild temperament and exceptional strength. Sure-footed and reliable even in treacherous terrain. Vary in color from tan to dark brown. Travel in small herds of 10-15 with one dominant bull, one dominant female, and several calves. Feed on Zsajhira berries and dull mosses that grow inside moist slopes of dormant volcanic cellars. Spend nearly 70% of their time underwater in shadowed grottoes; when in danger, they retreat below the water's surface. Druyza meat is a Socorran dietary staple.",
   "source": "WEG40154 Black Sands of Socorro p.12"
  },
  {
   "name": "Tra'cor",
   "type": "Amphibious creature",
   "planet": "Socorro",
   "habitat": "Underwater grottoes and volcanic cellars in the Doaba Badlands",
   "dex": "3D",
   "per": "3D",
   "str": "4D",
   "move": "5 (land), 13 (water)",
   "size": "2.7 meters tall (standing), 1.3 meters (on all fours)",
   "scale": "Creature",
   "orneriness": "",
   "special": "Skills: Brawling parry 4D+2, hide 4D, search 5D, sneak 4D+2, brawling 5D+1, climbing/jumping 5D, stamina 5D, swimming 5D+1. Claws do STR+2D damage. Teeth do STR+1D damage. Dehydration: Tra'cors can spend up to 24 hours out of water without ill effects, but afterward suffer -1D penalty to all skills for each additional 8-hour period out of water. Water Glands: Special glands along the underside of the creature's neck allow it to instinctively know the location of the nearest water source up to 10 km away — used when water cellars dry up and the tra'cor must migrate. | An underwater beast similar to the savage rancor — smaller and restricted to water, but no less nightmarish, with massive clawed talons and a gaping mouth with rows of jagged teeth. Feeds on mosses and water grasses but prefers meat, attacking stray mutrioks that wander too close to the water and the occasional weak or old druyza. Tra'cors migrate between grottoes during cool evenings using moisture channels that pool on the surface — discovered when a pair of scouts were attacked at night and only one made it back to camp. Live in groups of no more than two or three and often lie in wait at the edge of a water cellar, ambushing prey that comes to drink.",
   "source": "WEG40154 Black Sands of Socorro p.13"
  },
  {
   "name": "Ynook",
   "type": "Desert predator",
   "planet": "Socorro",
   "habitat": "Most dangerous areas of the Doaba Badlands and the Rym Mountains",
   "dex": "4D",
   "per": "3D",
   "str": "5D",
   "move": "13",
   "size": "Up to 2.3 meters tall",
   "scale": "Creature",
   "orneriness": "",
   "special": "Skills: Brawling parry 5D, dodge 5D, melee combat 7D, melee parry 6D, search 4D, sneak 5D+2, brawling 6D+2. Equipment: Club (STR+1D) or spear (STR+1D+1). | Savage and semi-sentient predators found in the most dangerous areas of the Doaba Badlands and the Rym Mountains. Swift-moving meat-eaters that survive on a diet of druyza and mutrioks. Intelligent enough to fashion spears and other simple tools, though there are no indications they have any kind of language. Generally solitary creatures. Respected by Socorran nomads, who know enough to give them a wide berth. Some superstitious smugglers consider seeing these creatures an omen of a dangerous journey — with fabulous rewards for those who survive.",
   "source": "WEG40154 Black Sands of Socorro p.13"
  },
  {
   "name": "Chiru (Sand Wasp)",
   "type": "Swarming insect",
   "planet": "Socorro",
   "habitat": "Vast underground hives in the Doaba Badlands",
   "dex": "4D+1",
   "per": "2D+2",
   "str": "1D",
   "move": "20",
   "size": "2.54 centimeters",
   "scale": "Creature",
   "orneriness": "",
   "special": "Sting: Poison does 3D damage. Characters stunned or worse by a sting suffer a -1D penalty to all actions for 1D rounds. The penalty is cumulative with multiple stings. | 'Chiru' is the Socorran word for 'deadly' and aptly describes the sinister sand wasps of Socorro. Congregated in vast underground hives of 100,000 to 150,000 members, these insects are fiercely territorial and swarm if provoked. Vibrations from landing or maneuvering jets — even simple vibrations from walking across the sands — can bring the chiru from their sandy lair and into the light for a kill. An undetermined number of hives exist across the planet, mostly in the desolate, unpopulated wastes of the Doaba Badlands. Hives within 30 km of civilization are clearly marked by Old Corellian warnings — ashoba ('hidden hive') or rha'atd ('danger'). During Tayaba'l, the winter equinox, temperatures reach an all-time low and most of the chiru population dies. But over the next 72-hour period as the sands warm again, the thousands of eggs left behind hatch, ushering in a new generation of the deadly wasps.",
   "source": "WEG40154 Black Sands of Socorro p.14"
  },
  {
   "name": "Black Bha'lir",
   "type": "Jungle predator",
   "planet": "Iyred (extinct in the wild); refuges on Duunir",
   "habitat": "Tropical and terrestrial forests",
   "dex": "5D+1",
   "per": "4D+2",
   "str": "4D",
   "move": "18",
   "size": "0.9-1.0 meters tall, 1.22 meters long",
   "scale": "Creature",
   "orneriness": "",
   "special": "Skills: Brawling parry 5D+1, dodge 4D+2, running 6D, hide 5D+1, search 5D, sneak 5D, brawling 5D, stamina 4D+1, swimming 4D+2. Claws do STR+1D damage. Teeth do STR+1D damage. Stealth: The creature gains +1D to all sneak rolls. | A native of Iyred and an opportunistic hunter of that world's tropical and terrestrial forests. They roam in small bands of one male, one female, and 6-7 cubs. Surprisingly docile when approached cautiously, but fiercely territorial and savage when threatened or defending their territories and young. Bha'lir mate for life and are notoriously loyal to their chosen mate — a primary reason biologists failed to maintain bha'lir numbers in captivity. They hunt in packs of several families: males in packs of 5-6 spearhead the hunting party while groups of 10-15 females run the perimeter until the target is exhausted, cornered, and brought down. Cubs eat first, then primary females and males, then subordinate members; afterward each family goes its own way. With the deforestation and desolation of Iyred, the Bha'lir faced starvation as food sources depleted, eventually falling prey to the whims of greedy investors. The species is officially listed as extinct, though small numbers have been raised in isolated refuges. The largest refuge, on Duunir, is a charity of the Black Bha'lir Society, who have adopted the large cats as their mascots.",
   "source": "WEG40154 Black Sands of Socorro p.39"
  },
  {
   "name": "Snow Q'lk",
   "type": "Arctic herbivore",
   "planet": "Neftali",
   "habitat": "Jhunia Snow Plains and lower fjords",
   "dex": "3D",
   "per": "3D+1",
   "str": "4D",
   "move": "17",
   "size": "1.4 meters tall at the shoulder; 1.53 meters long",
   "scale": "Creature",
   "orneriness": "",
   "special": "Skills: Brawling parry 6D+1, dodge 6D+1, running 6D+2, hide 4D, search 4D+1, brawling 5D+2, climbing/jumping 5D+1, stamina 5D. Awareness: Q'lk are extremely perceptive — in open areas with no cover, they gain +1D to all search and Perception rolls. Antlers: Head antlers do STR+1 damage. Hoofstrike: Kicking does STR+1D+2. | An elusive creature populating the snowy plains and lower fjords of Neftali. Masters of arctic adaptation, covered in a billowing shag of white fur with pale gray antlers; they excel at both camouflage and escape. Noble, peaceful animals known for their strength and speed. In rare cases they have been domesticated and used to pull sleighs. Travel in large herds of 150-300 — a tremendous defensive advantage as their stampedes can be quite effective against predators. However, they are easily panicked — there have been reports of whole herds leaping to their deaths trying to traverse a wide fjord. When first born, q'lk have solid black fur which lightens and thickens as the animal matures. At one time, black q'lk fur was a novelty on the Inner Rim worlds, but Memcha-Badawzi has outlawed hunting immature q'lk. Hunters must pay exorbitant licensing fees even for the privilege of hunting adult q'lk.",
   "source": "WEG40154 Black Sands of Socorro p.62"
  },
  {
   "name": "Ice Modrol",
   "type": "Arctic predator",
   "planet": "Neftali",
   "habitat": "Frozen plains and ice fjords",
   "dex": "3D",
   "per": "4D",
   "str": "7D",
   "move": "18",
   "size": "5 meters tall",
   "scale": "Creature",
   "orneriness": "",
   "special": "Skills: Brawling parry 5D, search 6D, sneak 5D, brawling 9D. Claws do STR+2D damage. Camouflage: Ice modrols have white skin to perfectly blend in with Neftali's terrain, receiving +1D to sneak. | Seeded on Neftali decades ago, ice modrols are ferocious predators with an incredible ability to unerringly track their prey. A modrol's claws are long sabers capable of slicing prey with one blow. They are a favored target of hunting expeditions — but all too often the hunting party falls victim to the modrol's claws.",
   "source": "WEG40154 Black Sands of Socorro p.62"
  },
  {
   "name": "D'oemir Bear",
   "type": "Arctic predator",
   "planet": "Neftali",
   "habitat": "Ice caves below the surface in desolate areas",
   "dex": "2D",
   "per": "2D+1",
   "str": "6D",
   "move": "10",
   "size": "1.3 meters tall (on all fours), 1.98 meters tall (standing)",
   "scale": "Creature",
   "orneriness": "",
   "special": "Skills: Brawling parry 4D+2, hide 3D, search 3D+2, sneak 3D+1, brawling 6D+1, lifting 6D+1, stamina 6D, swimming 6D+2. Claws do STR+1D damage. Bite does STR+2 damage. Hibernation: When food gets scarce, the d'oemir bear goes into hibernation, sometimes staying in this state for up to a year. | Before the arrival of sentient beings to Neftali, the d'oemir bear was emperor of the frozen wastes. With the arrival of settlers (including the seeding of ice modrols), this reign came to an abrupt end. Recent years saw a hunting spree that nearly drove the bears to extinction; now limited to desolate areas, the d'oemir bear is making a comeback. Big-game hunters come from across the galaxy to snare them. Because the bears live in such desolate isolation, guides are essential — expeditions can last several weeks. Licenses cost at least 10,000 credits, with full contingents of guides, carriers, and gear running upwards of 50,000 credits. Most hunters return to Cordel Cove empty-handed. A solitary animal living in ice caves, frolicking in hot springs or hibernating in the ice labyrinths beneath the mountains. Females live in groups of 2-3 and may permit one male to live with them for brief periods. They hunt in small packs and despite their cumbersome size are swift and agile, generally making 2-3 kills per hunt. Main diet is q'lk and frozen vegetation found in the lower, more sheltered ice fjords.",
   "source": "WEG40154 Black Sands of Socorro p.63"
  },
  {
   "name": "Guda Fish",
   "type": "Aquatic predator",
   "planet": "Neftali",
   "habitat": "Beneath the frozen oceans",
   "dex": "2D",
   "per": "2D+1",
   "str": "3D",
   "move": "16 (swimming)",
   "size": "3 meters long",
   "scale": "Creature",
   "orneriness": "",
   "special": "Skills: Hide 4D, search 3D, sneak 3D+1, brawling 3D+2, climbing/jumping 4D+2, stamina 5D, swimming 6D. Bite does STR+2 damage. Spur does STR+1D damage. Sonar: Guda fish receive +1D to detect objects below water or walking on the surface by homing in on sound and vibration. Escape: When attempting to escape fishing hooks, a guda fish gains +1D to brawling. | Three meters long, thin, bony, and muscular. Their cartilage is favored by native fishermen for hooks. Completely blind by millennia of evolution in watery darkness, the guda fish possesses an innate sonar that doubles as a motion detector — it can feel minute tremors in the water, such as when an animal walks over the ice. Once motion is detected, schools of 15-20 guda fish use their powerful nose spurs to hack through the ice, reducing a 3-8 meter section to slush in moments. Any animal unfortunate enough to slip through into the water will never see daylight again. The guda fish's spur is a prized trophy among fishermen and a sign of fortune and good luck to Neftali's natives. Believed to be Neftali's largest aquatic animal — accounts for more deaths per year than the q'lk and d'oemir bears combined. Hatched from eggs laid in the undersurface of the ice; the eggs and supple young fish command exorbitant prices, believed to have medicinal value. The fish was a staple for early settlers and is considered a delicacy by many Neftali residents.",
   "source": "WEG40154 Black Sands of Socorro p.66"
  },
  {
   "name": "Greel Screamer",
   "type": "Carnivorous avian",
   "planet": "Pii 4",
   "habitat": "Greel forests (nesting in greel trees)",
   "dex": "4D",
   "per": "2D",
   "str": "3D",
   "move": "14 (flying)",
   "size": "0.75 meters long, 1.5 meter wingspan",
   "scale": "Creature",
   "orneriness": "",
   "special": "Bite: STR damage. | The greel screamers reside in the greel trees harvested by the Greel Wood Logging Corporation loggers on Pii 4. These avians have reptilian bodies, toothy beaks, and flexible wings that easily fold when nesting. They often nest in greel trees in groups of four to eight. Though logging crews take measures to flush the nomadic flying creatures from a tree before cutting, on more than one occasion the screamers have been known to attack in the middle of a cut. They are extremely hostile toward those who disturb them — their name comes from the screaming cries they make when their territory is threatened.",
   "source": "WEG41006 Star Wars Adventure Journal #6 p.58 (The Greel Wood Haven)"
  },
  {
   "name": "Timber Render",
   "type": "Omnivorous forest-dwelling mammal",
   "planet": "Pii 4",
   "habitat": "Lower greel forests",
   "dex": "2D",
   "per": "2D",
   "str": "8D (brawling 10D+2)",
   "move": "9",
   "size": "3-4.5 meters long, 2 meters at the shoulder",
   "scale": "Creature",
   "orneriness": "",
   "special": "Claws: STR+2D damage. Paw Crush: STR damage. | Huge four-limbed timber renders inhabit the lower forests of Pii 4 and their territories are easily avoided — they leave their trademark slashed greel trunks for all to see. Hulking behemoths covered in tawny, ragged fur, with long snouts filled with various teeth for gathering and eating a variety of foods found in the greel forests. Their preferred attack is with sharp claws and a crushing hold from their forelimbs. Sightings are not unheard of, but on only one recorded occasion has a timber render attacked a Greel Wood Logging Corporation crew — the result was a dead Esoomian, four dead Reigat, two dead Whiphids, and a seriously-injured Wookiee. Company workers take active measures to avoid contact with timber renders.",
   "source": "WEG41006 Star Wars Adventure Journal #6 p.59 (The Greel Wood Haven)"
  },
  {
   "name": "Gornalak",
   "type": "Reptilian subterranean predator",
   "planet": "Taraloon",
   "habitat": "Subterranean caves and tunnels",
   "dex": "3D+2",
   "per": "3D+1 (search 5D)",
   "str": "6D (brawling 7D, stamina 7D+1)",
   "move": "10",
   "size": "3 meters tall, 6 meters long",
   "scale": "Creature",
   "orneriness": "",
   "special": "Teeth: STR+1D+2 damage. Claws: STR+1 damage. Subterranean Vision: Best suited for dim conditions — +1D to Perception and related skills in dark conditions, but -1D to Perception, Dexterity, and related skills in bright conditions. | Gornalaks are large reptilian creatures native to the subterranean regions of Taraloon. They were the lords of the planet until Drek Drednar's pirate band moved in and built a hidden base inside the cave network. While the pirates would make tasty meals, their weaponry and bright lights were too bothersome to be worth the effort, so the gornalaks retreated deeper into the dark caves to feed on smaller prey. Gornalaks do not attack the pirates' compound directly but sometimes pick off stray exploration groups deeper in the caves. Chillo Sanpona, the Sable III's Rodian first mate, hunts gornalaks for sport when back on Taraloon to satisfy his hunter's instincts.",
   "source": "WEG41005 Star Wars Adventure Journal #5 p.95 (Stand and Deliver)"
  },
  {
   "name": "Olai",
   "type": "Domesticated six-legged riding beast",
   "planet": "Redcap",
   "habitat": "Mountain canyons, mud plains",
   "dex": "4D (running 5D, dodge 4D+1)",
   "per": "2D",
   "str": "6D (brawling 6D+2, climbing/jumping 6D+2, lifting 8D)",
   "move": "15",
   "size": "1.5-2 meters at the shoulder",
   "scale": "Creature",
   "orneriness": "4D+1",
   "special": "Teeth: STR+1D damage. Hooves: STR+1D damage, +2 if wearing cleated shoes. Ramming: Horns do STR+2D damage when charging. | During Redcap's mining revolution, when the planet was a thriving mineral paradise, olai were brought in from a nearby moon and employed as heavy labor — moving rock, hauling debris, and powering elevator shafts in an ecosystem that would not support advanced repulsorlift technology. Even after technology became available, miners still relied heavily on the olai as backup when the systems failed. When the mines dried up and times became hard, the olai supplied a staple source of nourishment for the starving survivors. Powerfully built, sure-footed, and intelligent, they are easily domesticated for riding or hauling, with thick hollow horns used for ramming or bumping during breeding seasons. They have continued to evolve in Redcap's hostile environment for nearly a decade, multiplying and spreading across the surface, and remain the primary form of transportation in the mountain passes (where the wet clay would mire any repulsorcraft).",
   "source": "WEG41005 Star Wars Adventure Journal #5 p.141 (A Bitter Winter)"
  },
  {
   "name": "Wroonian Flycatcher",
   "type": "Sphere-bodied cave predator",
   "planet": "Wroona",
   "habitat": "Coastal sea caves",
   "dex": "2D",
   "per": "3D (search 4D, sneak 4D)",
   "str": "4D (brawling 5D, climbing/jumping 6D)",
   "move": "3 / 12 (ground / swinging by tongue)",
   "size": "Spherical body, suspended by long adhesive tongue threads",
   "scale": "Creature",
   "orneriness": "",
   "special": "Claws: STR+2D damage. May attack twice per round at no penalty with all four webbed claw hands. Adhesive Tongue: 20-meter range, sticks to most surfaces, hits a specific target on a Dexterity roll — used to swing down from cave ceilings to ambush prey. Cave Ambush Predator. | Wroonian flycatchers are large spherical predatory beasts that look like rocks attached to cave ceilings by long threads. The threads are actually their adhesive tongues — flycatchers detach from the cave wall, swing down on these tongues, and scoop up avians (and unwary intruders) with four immense webbed hands tipped with claws. They normally hunt the avians flying through Wroona's coastal sea caves, but readily assume any character passing through the cave is food. They live in groups inside the same cave system. Hidden lifeforms-bearing escape pods that drift in with the tide can become lodged in flycatcher caves, requiring intruders to fight off the resident colony to recover the pod.",
   "source": "WEG41005 Star Wars Adventure Journal #5 p.285 (Vengeance Strike)"
  },
  {
   "name": "Strider",
   "type": "Reptilian riding animal",
   "planet": "Sedesia",
   "habitat": "Steppe and tundra",
   "dex": "2D",
   "per": "2D",
   "str": "5D (brawling 6D, stamina 7D)",
   "move": "20",
   "size": "1.5-2.5 meters tall at the shoulder, 6-8 meters long",
   "scale": "Creature",
   "orneriness": "3D (females), 5D (males)",
   "special": "Tail: STR damage. Tusks: STR+1D damage. | Striders are large six-legged reptilian creatures native to Sedesia. Fast runners with excellent stamina, they have been the riding animal of choice on the planet for generations — particularly in outlying areas where roads are bad and the planet's violent storms make repulsorcraft dangerous. Striders are very cantankerous and hard to train, however. Females are merely willful (Orneriness 3D), but male striders are downright vicious (Orneriness 5D). When angered they attack with their long whiplike tails and sharp tusks.",
   "source": "WEG41006 Star Wars Adventure Journal #6 p.207 (The Cure)"
  },
  {
   "name": "Breedtash",
   "type": "Herd herbivore (livestock)",
   "planet": "Sedesia",
   "habitat": "Steppe and tundra (domesticated, ranched in vast herds)",
   "dex": "2D",
   "per": "1D",
   "str": "5D (brawling 6D)",
   "move": "18",
   "size": "Large herd animal",
   "scale": "Creature",
   "orneriness": "",
   "special": "Barbed Tail: STR+1D damage. Trample: STR damage. | Breedtash are the primary livestock of Sedesia, ranched in vast herds across the planet's steppes. They are the foundation of the Sedesian economy — breedtash meat is exported off-world and (before the FNV-23 plague disrupted shipping) made its way into Rebel mess halls in considerable tonnage. Each fall, ranchers drive their herds into Besia Osurne for sale at the city's stockyards and processing plant. When agitated by gunfire or predators, breedtash can stampede in panicked thousands — riding through a stampede is a Very Difficult task even on a strider, and trying to drive a wheelbike through a herd in motion is borderline suicidal.",
   "source": "WEG41006 Star Wars Adventure Journal #6 p.209 (The Cure)"
  },
  {
   "name": "Bildog",
   "type": "Docile Oceanic Herd-Beast",
   "planet": "Chad",
   "habitat": "Deep ocean",
   "dex": "1D",
   "per": "1D (search: listen 4D)",
   "str": "5D (swimming 6D)",
   "move": "15",
   "size": "150 meters long, 15 meters wide",
   "orneriness": "1D",
   "special": "Thrash: STR+2D damage. Gel Armor: thick sub-dermal tissue gives +2D to resist physical damage. Huge gentle sea-slugs that gather in pods of 5-50, abysmally stupid and quite harmless unless attacked — main defense is to thrash their tails and swim away, relying on their thick gel hide to absorb crippling hits. Feed on micro-organism cloud-colonies near the ocean surface. Herded along their migration routes by Chad's deep-water ranchers using foo-twitter devices that mimic the bildog herding sounds. Provide bildog meat for export.",
   "source": "WEG41010 Star Wars Adventure Journal #10 p.102 (A Free-Trader's Guide to the Planets)"
  },
  {
   "name": "Proop",
   "type": "Docile Oceanic Herd-Beast",
   "planet": "Chad",
   "habitat": "Deep ocean",
   "dex": "3D",
   "per": "1D+2 (search: listen 3D)",
   "str": "3D (brawling: tentacle 4D)",
   "move": "8",
   "size": "15 meters long, 5 meters wide",
   "orneriness": "3D",
   "special": "Tentacle Attack: STR+1D+2 damage. Multi-tentacled with small heads and gelatinous bodies, smaller and somewhat more intelligent than their bildog cousins. Herd in groups of 50-300. Feed on micro-organism cloud-colonies near the surface. Herded by Chad's ocean ranchers along the same routes as bildog and harvested for proop meat.",
   "source": "WEG41010 Star Wars Adventure Journal #10 p.102 (A Free-Trader's Guide to the Planets)"
  },
  {
   "name": "Wystoh",
   "type": "Territorial Ocean Predator",
   "planet": "Chad",
   "habitat": "Deep ocean",
   "dex": "1D",
   "per": "3D (search 5D)",
   "str": "4D (brawling 5D, swimming 5D)",
   "move": "18",
   "size": "4-7 meters long, 0.8-2 meters wide",
   "orneriness": "",
   "special": "Bite: STR+2D damage. Sinewy serrated-tooth pack hunters that locate and shred any prey that enters their hunting territory, propelled at terrifying speed by a slithering motion. Beady black eyes, large mouths. Chad's deep-water ranchers trick wystoh away from areas they need to enter by sending an automated foo-twitter through wystoh territory, mimicking prey sounds and temporarily clearing a path as the predators chase the false signal.",
   "source": "WEG41010 Star Wars Adventure Journal #10 p.103 (A Free-Trader's Guide to the Planets)"
  },
  {
   "name": "Tsaelke",
   "type": "Spirited Cetacean (Possible Sentient Precursor)",
   "planet": "Chad",
   "habitat": "Open ocean, shallow seas, and bayous",
   "dex": "3D",
   "per": "3D",
   "str": "3D (swimming 4D)",
   "move": "16",
   "size": "9-12 meters long, 3-4 meters wide",
   "orneriness": "3D+2",
   "special": "Sea-Singing: long-range communication allowing reasonably complex exchanges of information; each cetacean species has its own 'language' but some cross-communication has been noted. Rudimentary Thumb: long 'hand'-fluke with semi-opposable thumb that can roughly manipulate objects. Spirited but generally friendly mounts used by ocean ranchers, never fully domesticated — even the most reliable will occasionally play pranks on their riders. Barrel-bodied with long necks and small heads, brass-colored with mottled camouflage in unique stripe/spot/ring patterns. Carry up to 75 kg cargo or 25 kg + a rider. Fish browsers. The only Chadean cetaceans known to enter the shallow seas and bayous, and the only cetaceans known to the Chadra-Fan, who regard them as sea-spirits. May be a sentient precursor species.",
   "source": "WEG41010 Star Wars Adventure Journal #10 p.103 (A Free-Trader's Guide to the Planets)"
  },
  {
   "name": "Cy'een",
   "type": "Wild Cetacean",
   "planet": "Chad",
   "habitat": "Open ocean",
   "dex": "2D",
   "per": "3D",
   "str": "4D (swimming 5D)",
   "move": "16",
   "size": "21-26 meters long, 4-7 meters wide",
   "orneriness": "6D",
   "special": "Sea-Singing: long-range communication allowing complex exchanges of information. Wild untamable cousins of the tsaelke and methnap. Barrel-bodied with long necks and small heads, dark bronze ventral shading to black dorsal. Carnivores — fish hunters who occasionally snatch eloag (white-furred gliding birds) when the chance presents itself. Although not domesticated, young ranch hands sometimes jump on and ride them as a personal challenge; cy'een hate riders and seem to regard them as obstacles to overcome. 'Riding the cy'een' has become a Chadean metaphor for any challenging and dangerous endeavor.",
   "source": "WEG41010 Star Wars Adventure Journal #10 p.105 (A Free-Trader's Guide to the Planets)"
  },
  {
   "name": "Morrt",
   "type": "Small Tropical Parasite (Gamorrean Pet/Status Symbol)",
   "planet": "Gamorr",
   "habitat": "Forests, settlements, attached to Gamorrean hosts",
   "dex": "1D",
   "per": "+2",
   "str": "+1",
   "move": "4",
   "size": "1-5 cm",
   "orneriness": "",
   "special": "Bloodsucker: causes -1 damage per round from blood loss; Gamorreans are immune. Gamorrean physiology is adjusted to host morrts (other species lose -1 pip Strength per attached morrt). Morrts attach painlessly — their skin exudes a slime that acts as a local anesthetic, which Gamorreans use as the base for a pain-killer. They usually remain in place until found visually and can be detached by lifting them off by the tail. Wild morrts are aggressive about attaching to hosts and group in clusters of up to several dozen; a swarm could drain a Gamorrean dry. Tame morrts are docile, usually so fat from feasting on their host they can barely move. Worn as pets and status symbols on Gamorr — the more morrts you wear, the higher your status (warlords carry up to 20, matrons many more, common boars 6-10, veterans about a dozen).",
   "source": "WEG41010 Star Wars Adventure Journal #10 p.107 (A Free-Trader's Guide to the Planets) | WEG41014 Star Wars Adventure Journal #14 p.46 (Murder in Slushtime)"
  },
  {
   "name": "Algark Stalk",
   "type": "Carnivorous Fungus",
   "planet": "Gamorr",
   "habitat": "Deep forests",
   "dex": "",
   "per": "",
   "str": "+2",
   "move": "0",
   "size": "2-4 meters (stalk), 0.5-10 meter radius (spore plume)",
   "orneriness": "",
   "special": "Spore Plume: algark plants attack by forming a fungus colony on a living host. Each hour following exposure the victim makes a stamina roll against progressively increasing difficulty (first roll Moderate, +5 to difficulty each hour thereafter). Failed rolls leave the victim wounded; rolls that beat the difficulty by more than 5 wipe the colony out via the victim's immune system. Algark stalks grow tall with a wind-catching sail-bulb cap that disperses spores when agitated by strong wind or a passing animal. Spores are highly opportunistic and root in soil or organic tissue — exposed animals often become hosts for new colonies as the fungus consumes them, founding new colonies miles away from the parent at the host's death site.",
   "source": "WEG41010 Star Wars Adventure Journal #10 p.108 (A Free-Trader's Guide to the Planets)"
  },
  {
   "name": "Snoruuk",
   "type": "Mobile Mushroom (Domesticated Herd-Fungus)",
   "planet": "Gamorr",
   "habitat": "Forest floor",
   "dex": "1D",
   "per": "1D",
   "str": "1D per meter of height",
   "move": "1",
   "size": "1-5 meters tall",
   "orneriness": "1D",
   "special": "Mobility: snoruuk shuffle from feeding ground to feeding ground on a single foot. Edible slow-moving mobile mushrooms native to Gamorr. Group together in 'rings' of 5-30 and collectively creep across the forest floor — totally blind, instinctively forming circles and randomly wandering in an ongoing search for food and moisture. Can be herded with patience by tapping a single mushroom in the ring (the ring moves away from the tap). Gamorreans consider snoruuk a staple food and keep them as herd-fungi guided by herding staves. In fall, snoruuk rings take root to last out the winter; in spring new mushrooms bud in the rooted ring; as summer begins, the rings uproot and wander again.",
   "source": "WEG41010 Star Wars Adventure Journal #10 p.108 (A Free-Trader's Guide to the Planets)"
  },
  {
   "name": "Sun Beetle",
   "type": "Insect of Burden",
   "planet": "Kubindi",
   "habitat": "Plains, deserts, settlements",
   "dex": "1D",
   "per": "1D",
   "str": "5D (brawling: pincer 7D, lifting 9D+2)",
   "move": "9",
   "size": "2 meters to carapace ridge, 6 meters long",
   "orneriness": "2D",
   "special": "Pincers: STR+3D damage. Formerly common Kubaz beasts of burden, now largely replaced by machines but still used as burden carriers in remote backward areas. In developed regions used as show and racing animals, and in the past used in combat. Wild sun beetles in the dry plains duel over territory in pulling matches — the beetles lock pincers and pull until one's pincer rips out of its socket. An illegal beetle-fighting underground has developed on Kubindi in which gamblers bet on which beetle can pull its opponent to death.",
   "source": "WEG41010 Star Wars Adventure Journal #10 p.118 (A Free-Trader's Guide to the Planets)"
  },
  {
   "name": "Kuret Swarm",
   "type": "Armored Swarm Insect",
   "planet": "Kubindi",
   "habitat": "Wild forests and plains",
   "dex": "1D",
   "per": "2D",
   "str": "1D-5D (1D per 100 kurets in swarm)",
   "move": "6",
   "size": "1 cm individual",
   "orneriness": "",
   "special": "Bite: STR+2 damage. One-centimeter bronze-colored insects with a bristle of short feathery antennae and large sharp pincers. A kuret swarm acts as a group-entity, communicating via pheromone transmission with no 'royal insect' issuing orders — the swarm operates on overall instinctive consensus. Wild and never tamed by Kubaz farmers, but valued as a rare delicacy by gourmets; kuret-trappers can demand a high price for a bushel.",
   "source": "WEG41010 Star Wars Adventure Journal #10 p.118 (A Free-Trader's Guide to the Planets)"
  },
  {
   "name": "Mastmot",
   "type": "Giant Herbivore (Whiphid Prey)",
   "planet": "Toola",
   "habitat": "Tundra, ice plains",
   "dex": "1D",
   "per": "1D+2",
   "str": "8D",
   "move": "12",
   "size": "10-12 meters at shoulder, 15+ meters long",
   "orneriness": "",
   "special": "Fatty Hide: -2D against physical damage, -1D energy. Trample: stampeding in groups of 5+, mastmots inflict STR+3D damage on anyone in their way. Tusks: STR+1D damage. The preferred prey of Whiphids and snow demons — not because they're easy to kill but because they're a wealth of food and resources. A single mastmot provides a tribe with a month of meat and enough hides and furs to outfit every member with a snow cloak and tent. Rib bones are carved with Whiphid stick-letters to record lineage; tusk ivory becomes jewelry. Mastmots use their spike-tusks to slash and stab attackers and trample even the strongest hunter under their splayed feet. Other herd members defend an attacked one. Hunters typically target a calf or sick/old adult away from the herd; otherwise the herd rampages through the hunting party. Herds number 5-12 adults. Snow demons gorge on mastmot meat at kill sites until driven off, the meat rots, or all edible portions are consumed. Mastmot fur is particularly rich and well-priced.",
   "source": "WEG41010 Star Wars Adventure Journal #10 p.127 (A Free-Trader's Guide to the Planets)"
  },
  {
   "name": "Caraboose",
   "type": "Hardy Herbivore (Whiphid Prey)",
   "planet": "Toola",
   "habitat": "Tundra",
   "dex": "2D+1",
   "per": "2D",
   "str": "3D+1 (stamina 5D)",
   "move": "12",
   "size": "2 meters at shoulder",
   "orneriness": "",
   "special": "Antlers: STR+2D damage. Trample: stampeding herds inflict STR+2D+2 damage on anyone in their way. A favored Whiphid prey because it is a bit easier to take down than mastmot. Herd in groups of 10-30 adults. Main defenses are antler slashes and herd stampedes — Whiphid hunters sometimes stampede caraboose into traps or off cliffs, although the Whiphids' cleverness occasionally backfires when the herd surges the wrong way and stampedes the hunters by accident. Amazingly hardy — survives privations that would starve even the toughest of the other Toola herbivores. Trader's gamble: caraboose are big, hardy, and survive on minimal fodder; a maverick bantha rancher might work a deal to start caraboose herds elsewhere.",
   "source": "WEG41010 Star Wars Adventure Journal #10 p.128 (A Free-Trader's Guide to the Planets)"
  },
  {
   "name": "Arabore",
   "type": "Seagoing Rodent",
   "planet": "Toola",
   "habitat": "Frozen ocean shores",
   "dex": "1D",
   "per": "2D",
   "str": "4D",
   "move": "2 (walking), 8 (swimming)",
   "size": "2.3-2.8 meters long",
   "orneriness": "",
   "special": "Blubber: +2D to STR against physical damage. Bull Ram: STR+2 damage. Tusks: STR+2D damage. Fish-eating sea rodents with great flat tusks protruding from short snouts, layered in fat but conveying an impression of sleekness from their tapering aquadynamic shape. Bull arabores collect harems of 2-12 cows and acquire fishing territories by dueling with neighboring bulls. Bulls are very territorial, regard any creature stumbling across the pod as a potential threat, and respond with a charge — often punctuated by a tusk attack — while the cows dive into the safety of the sea. Whiphids hunt arabore only occasionally; a young hunter traditionally kills a bull bare-handed with tusks alone to prove himself a mighty hunter, and a new Spearmaster does the same. Otherwise the arabore are in no more danger from Whiphids than any other Toolan creature.",
   "source": "WEG41010 Star Wars Adventure Journal #10 p.126 (A Free-Trader's Guide to the Planets)"
  }
  ,{
   "name": "Zdrinbagh",
   "type": "Shelled Predator (Edible)",
   "planet": "Outer Rim worlds (various)",
   "habitat": "Mixed",
   "dex": "3D",
   "per": "2D",
   "str": "3D",
   "move": "16",
   "size": "0.5 meters tall, 0.25 meters wide",
   "orneriness": "Aggressive when hungry",
   "special": "Dodge 5D, brawling 5D. Claws and teeth do STR damage. Shells offer +2D bonus against energy attacks, +1D against physical attacks. Slavering, hungry, and very fast — four legs and four claws, razor-sharp pointed teeth, beady red eyes. Highly prized for their light tender meat — typically shipped live to keep them fresh, with chef-droid handlers releasing a stun gas canister in the kitchen (to which droids are immune) before opening the shipping crates and slaughtering the sleeping zdrinbagh. The Farrimmer Cafe on Mynock 7 once suffered a famous incident in which a bad/weak stun canister failed to put two dozen zdrinbagh to sleep — when the chef droids opened the crates, the angry creatures swarmed into the dining room and had to be beaten back by the patrons (with H'nib Statermast yelling 'Just stun them if you can — if you blast them too hard the chefs can't make stew out of them!').",
   "source": "WEG41011 Star Wars Adventure Journal #11 p.65 (The Farrimmer Cafe)"
  }
  ,{
   "name": "Snuzzleguff",
   "type": "Tiny Flying Predator",
   "planet": "Unknown",
   "habitat": "Mixed (kept as exotic pets — Snowball lives in a Gamorrean mercenary's shipboard cabin cage)",
   "dex": "5D",
   "per": "4D",
   "str": "2D",
   "move": "18",
   "size": "0.2 meters in diameter",
   "orneriness": "1D (very aggressive when hungry)",
   "special": "Dodge 7D, flight 7D, sneak 5D, brawling 4D. Natural Anti-Gravity: snuzzleguffs have a natural immunity to gravity that gives them the ability to leap and fly at incredible speeds for short distances. Poisonous Bite: bite does 1D damage and the victim must make a Difficult stamina roll or become incapacitated. A tiny creature covered with white fur — survives by cunning instincts rather than intelligence. The Gamorrean mercenary Bnugor keeps one as a pet (named Snowball) and once tracked it through the Mynock 7 ventilation system to the Farrimmer Cafe after it escaped its cage. Captures must be made by trap, cage, or Very Difficult brawling — and Bnugor will use his vibro-axe on anyone who attempts to harm Snowball.",
   "source": "WEG41011 Star Wars Adventure Journal #11 p.79 (The Farrimmer Cafe)"
  }
  ,{
   "name": "Andoan Marsh Lizard",
   "type": "Giant Carnivorous Marsh Predator",
   "planet": "Ando",
   "habitat": "Coastal swamps and quagmires",
   "dex": "2D+1",
   "per": "3D",
   "str": "3D+1",
   "move": "7 (walking), 12 (swimming)",
   "size": "2.5-4 meters long",
   "orneriness": "Aggressive predator",
   "special": "Search 4D+2, brawling: bite 4D+2. Bite: STR+1D damage. Claws: STR damage (front legs only — used as secondary weapons). Hide: +1D against physical attacks. The largest predator in Ando's coastal swamps. Dull brown scaled hide. Flat head with eyes pointing up at a 45-degree angle, allowing the creature to lie submerged and see above the water's surface. Four long legs — front pair ending in sharp claws, rear pair being oversized paddle-like flippers. Adept at swimming but somewhat awkward on land. Short flat tail acts as a third flipper for increased swimming speed and doubles as a rudder for turns. Their favorite nocturnal prey is the gray flingler bird, but they can mistake speeder bikes for the same target — exploding from a quagmire in a curtain of mud and water before realizing the mistake and concentrating on the soft morsel (the rider) instead.",
   "source": "WEG41011 Star Wars Adventure Journal #11 p.179 (The Ando Project)"
  }
  ,{
   "name": "Giant Amorphous Bantha Breakfast Biscuit",
   "type": "Mutant Breakfast Delicacy",
   "planet": "Various (Tagge Company laboratory accident)",
   "habitat": "Wherever Bantha Breakfast Biscuits are activated in autochefs",
   "dex": "2D",
   "per": "2D",
   "str": "8D",
   "move": "15",
   "size": "Varies",
   "orneriness": "Mean and hungry",
   "special": "Brawling 9D. Bite: STR damage. Horns: STR+1D damage. Trample: STR damage. Blue Sauce Vulnerability: these creatures are killed instantly (and rather explosively) when smeared with Biscuit Baron blue sauce. Not much is known about these giant amoebic ruminants — they are a new lifeform and no one has gotten close enough to study one without being consumed. They are big, mean, and will eat anything that happens to find itself placed in front of them. Several laboratory chefs working for the Tagge Company learned this the hard way from inside the gullet of what they thought was going to be their morning meal — they popped a couple of Bantha Breakfast Biscuits into the autochef and out came their worst gastronomic nightmare. Accidentally created when Biscuit Baron preservatives somehow bonded to bantha genetic material, these beasts are now being considered for use as biological weapons by the Empire. Until they can be controlled, the Tagge Company has restricted Bantha Breakfast Biscuits from being shipped to those planets they refer to as 'inadequately prepared to delight in this yummy breakfast delicacy.' Come hungry, but carry plenty of Biscuit Baron blue sauce with you.",
   "source": "WEG41011 Star Wars Adventure Journal #11 p.219 (Bungo n' Rusti Get Curry-Out)"
  },
  {
   "name": "Boetay",
   "type": "Mammal (predator)",
   "planet": "Garos IV",
   "habitat": "Forested mountains and countryside",
   "dex": "3D",
   "per": "3D+2",
   "str": "4D",
   "move": "14",
   "size": "1.5 meters at the shoulder (adult)",
   "orneriness": "2D (vicious if young are threatened, otherwise avoids humans)",
   "special": "Brawling 5D. Bite: STR+1D damage. Claws: STR+1D damage. Search 5D, sneak 5D. Wild boetays roam the countryside in small packs and are quite vicious when their young are disturbed, but they generally avoid contact with humans. Adult boetays stand about a meter and a half at the shoulder, with dark fawn hides marked at regular intervals by black stripes running the length of their necks, backs, and quarters; cubs sport the same coloration. Their mournful howls echo through the forested hillsides outside Ariana on Garos IV at night. A Force-sensitive can calm even an adult boetay through patient projective Sense work, persuading the animal that no threat is intended — Jaalib Brandl famously charmed a mother boetay and her three-week-old pup on a moonlit forest path during the New Republic's invasion of Garos.",
   "source": "WEG41012 Star Wars Adventure Journal #12 p.93 (Betrayal by Knight)"
  },
  {
   "name": "Kheilwar",
   "type": "Homunculus-Wasp Predator",
   "planet": "Af'El",
   "habitat": "Warm natural crevasses; will invade live biological hosts",
   "dex": "4D",
   "per": "4D",
   "str": "4D",
   "move": "20 (flight)",
   "size": "0.01 to 2 meters",
   "orneriness": "4D",
   "special": "Brawling parry 6D, search 5D, sneak 5D, brawling 6D, stamina 7D. RAZOR-FINS: cause STR+2D damage. SUPER-THINNESS: the kheilwar can flatten and fold itself out to an extraordinary degree, slipping into very small cracks, crannies, and niches to hide. IMPERSONATE: the kheilwar can momentarily take on the image and sound of another being or creature it has seen — apparently a kind of telepathic projection (it can communicate simple concepts and they are understood in the language of the observer). It does not seem to consciously select its decoy, instead picking recent impressions. LIGHT BLIND: unable to 'see' in strong light (and possibly other radiation), and unable to attack while blinded; its visual sense organs are often destroyed by direct sunlight. The dark and dangerous homunculus-wasp of Af'El is deadly to nearly all forms of life. In its larval stage it is near-microscopic and only slightly larger when first matured. It seeks warm crevasses to feed and grow, often invading a live biological host; well-fed specimens grow quickly. A whirling storm of sharp fins, mandibles, and wings — fast, deadly, strong, and mean. Stats above reflect a roughly 10-kilo specimen; subtract 1D-2D from attributes and skills for small specimens and add 1D-2D for larger ones.",
   "source": "WEG41014 Star Wars Adventure Journal #14 p.43 (Murder in Slushtime)"
  },
  {
   "name": "Pittin",
   "type": "Domesticated Pet Pest Predator",
   "planet": "Unknown (galaxy-wide where droid pest-units are impractical)",
   "habitat": "Homes, shops, and businesses as a pet vermin-hunter",
   "dex": "4D",
   "per": "3D",
   "str": "2D",
   "move": "12",
   "size": "0.5 meters",
   "orneriness": "3D",
   "special": "Brawling parry 5D, search 4D, brawling 5D. TEETH: STR+2D damage. CLAWS: STR+2D damage. Pittins are pesters — small carnivores kept as pets to clear vermin from homes and shops. Their world of origin is unknown; they have been domesticated for millennia and are most commonly found on worlds where droid pest-units are impractical or non-existent (such as Gamorr). They make good and loyal pets, although their owners are occasionally frustrated with their natural independence.",
   "source": "WEG41014 Star Wars Adventure Journal #14 p.46 (Murder in Slushtime)"
  },
  {
   "name": "Dwoob",
   "type": "Semi-Domesticated Draft Animal",
   "planet": "Gamorr",
   "habitat": "Cleared tracks, snowy slushtime trails",
   "dex": "2D",
   "per": "2D",
   "str": "3D",
   "move": "10",
   "size": "1 meter long",
   "orneriness": "4D",
   "special": "Lifting 4D, stamina 5D. Dwoobs are small, semi-domesticated draft animals used on Gamorr — generally hitched to pull sleds in winter and early slushtime. They are not a triumph of Gamorrean domestication and are usually only handled by skilled sled drivers; most Gamorreans prefer wheelbarrows for hauling goods.",
   "source": "WEG41014 Star Wars Adventure Journal #14 p.46 (Murder in Slushtime)"
  },
  {
   "name": "Glarsaur",
   "type": "Sentient Carnivore (Gelgelar Native)",
   "planet": "Gelgelar",
   "habitat": "Swamps and wetlands",
   "dex": "2D/4D",
   "kno": "1D/3D",
   "mec": "1D/2D",
   "per": "2D/4D+2",
   "str": "2D/4D",
   "tec": "1D/2D",
   "move": "10",
   "size": "1-1.5 meters tall",
   "orneriness": "Highly aggressive",
   "special": "Attribute Dice: 12D. BITE: STR+1D+2 damage. CLAWS: STR+1D damage. The only native sentient creatures on the swamp planet Gelgelar — brutish, warlike, highly aggressive, perpetually hungry carnivores who get along with no one. They stand a meter to a meter and a half tall and are covered with small green scales that resemble leaves; their coloration helps them blend into the Gelgelar foliage, rendering them practically invisible until they strike. Cold-blooded and muscular, they are adept at using their claws or fashioning crude weapons such as spears and clubs. Though dull-witted they are not without the ability to make plans and set ambushes, and have been known from time to time to work with elements of Gelgelar's criminal underworld — stopping vohis mold shipments from reaching the Free Port in exchange for particularly tasty hunks of meat and not-so-crude weapons. Some Gelgelar settlers (primarily humans and Sullustans) have tried to communicate with them and establish a tenuous relationship; while Glarsaurs are capable of reasoning, they have shown no desire to be reasoned with and have on more than one occasion attacked the individuals trying to befriend them. TYPICAL GLARSAUR WARRIOR: Dexterity 2D+2, dodge 4D+2, Perception 2D, hide 5D, search 4D, Strength 2D+2, brawling 4D+2; bite STR+1D+2, claws STR+1D, Move 10.",
   "source": "WEG41014 Star Wars Adventure Journal #14 p.182 (The Breath of Gelgelar)"
  },
  {
   "name": "Spidercat",
   "type": "Temple Guardian (Domesticated Predator)",
   "planet": "Tasariq",
   "habitat": "Ancient Tasari temples and the areas surrounding them",
   "dex": "4D",
   "per": "3D",
   "str": "3D",
   "move": "12",
   "size": "2 meters long",
   "orneriness": "Territorial / Aggressive",
   "special": "CLAWS: spidercats strike victims with barbed claws on their legs for STR+2 damage. POISONOUS BITE: the bite attack does 1D damage; additionally, the victim must roll Moderate or better against Strength or be paralyzed for 1-6 hours. Fur-covered insects that look like a cross between a spider and a cat. In the dark times after the destruction of Tasari society, the Tasari bred them for use as guardians in the temples devoted to the tasar crystals. Even now, more ferocious spidercats (Dexterity and Strength 5D each) are found in the areas surrounding the forgotten temples.",
   "source": "WEG41015 Star Wars Adventure Journal #15 p.184 (Tasariq: The Crystal Planet)"
  },
  {
   "name": "Sting-Snake",
   "type": "Electrical Ground Snake",
   "planet": "Tasariq",
   "habitat": "Habitable craters (greater concentrations around ancient temples)",
   "dex": "3D",
   "per": "3D",
   "str": "2D",
   "move": "9",
   "size": "1 meter long",
   "orneriness": "Docile unless startled or cornered",
   "special": "ELECTRICAL DISCHARGE: Sting-Snakes attack by discharging electricity from the stinger in their tail. The sting inflicts 3D damage; after an attack it takes an hour for the snake to build up enough charge for another attack. Found in all the habitable craters of Tasariq; rarely attack humanoids but can be dangerous if startled or cornered. Found in greater concentrations around the ancient tasar crystal temples because trained sting-snakes were used alongside spidercats as temple guardians. Off-worlders have discovered sting-snakes can be trained as attack beasts — an increasing number of miners use them to guard their tents and vehicles.",
   "source": "WEG41015 Star Wars Adventure Journal #15 p.184 (Tasariq: The Crystal Planet)"
  },
  {
   "name": "Tunnel Worm",
   "type": "Earth-Burrowing Carnivore",
   "planet": "Tasariq",
   "habitat": "Underground tunnels near habitable craters",
   "dex": "3D",
   "per": "2D",
   "str": "4D",
   "move": "8",
   "size": "4 meters long",
   "orneriness": "Aggressive predator",
   "special": "BITE ATTACK: STR+2D damage. Large segmented worms that live in tunnels they burrow through the earth. They can hunt on the surface for hours, living off stored oxygen. They tend to cluster around habitable craters where the most food is available, but concerted efforts are made to keep their numbers down near inhabited craters.",
   "source": "WEG41015 Star Wars Adventure Journal #15 p.185 (Tasariq: The Crystal Planet)"
  },
  {
   "name": "Krizar (pupal stage)",
   "type": "Developing predator",
   "planet": "Torpris",
   "habitat": "Subterranean nutrient slime caverns",
   "dex": "1D",
   "per": "1D",
   "str": "1D",
   "move": "3",
   "size": "4-8 centimeters long",
   "orneriness": "Developing predator",
   "special": "BITE: 1D damage. Locking teeth have a Strength of 5D+2 to hold in place — must typically be removed surgically. SHELL: Hard ellipsoid shell provides +1D+2 defense against physical damage. Sometimes referred to as 'clampjaws' even in pupal stage. The pupae are sustained by viscous gray sludge that flows through subterranean caverns. When ready to hatch, mouth palps emerge first — if they sink into skin, a trip to the med facility is required. Krizar are now bred and slave-harvested in large numbers by unscrupulous beings, Chay Praysh among them.",
   "source": "WEG41017 Star Wars Adventure Journal #17 p.56-57 (Jade Solitaire)"
  },
  {
   "name": "Krizar (adult)",
   "type": "Predator",
   "planet": "Torpris",
   "habitat": "Mountain, Urban (guard creature)",
   "dex": "3D",
   "per": "3D",
   "str": "1D",
   "move": "14",
   "size": "0.75-1.2 meters at the shoulder",
   "orneriness": "Aggressive predator",
   "special": "BITE: 4D+1 damage first round, plus 1D additional damage every round the creature stays clamped. Clamping jaw has a Strength of 7D — killing the creature will NOT release the jaw (system floods with a locking chemical upon death). CLAWS: STR+1 physical damage. DEW CLAWS: Sheathed claws on all four hind legs can sink nearly half a meter into the ground, anchoring the creature and its victim. Provides +2D Strength bonus to resist being moved — killing the creature will NOT weaken the dew claws' hold. Skills: tracking 5D, brawling 5D+2. Sometimes called 'clampjaws,' krizar are fierce guard animals indigenous to Torpris, used by many estates and compounds. Some analogous species inhabit Vodran and other marsh-like worlds, but krizar are the most vicious and are exported to other systems for a hefty price.",
   "source": "WEG41017 Star Wars Adventure Journal #17 p.56-57 (Jade Solitaire)"
  },
  {
   "name": "Binarian Sabercat",
   "type": "Jungle predator",
   "planet": "Binaros",
   "habitat": "Tropical forests and undergrowth",
   "dex": "3D",
   "per": "3D+1",
   "str": "3D+2",
   "move": "12",
   "size": "1m at shoulder, 2-3m long",
   "orneriness": "Aggressive predator",
   "special": "BRAWLING PARRY: 4D. SEARCH: 4D+1, SNEAK: 5D. BRAWLING: 4D. CLAWS: Can attack with two claw attacks per round without penalty (third paw -1D, fourth -2D, etc.). Claws do STR+1D damage. BITE: Huge teeth do STR+2D damage. A lie-in-wait ambush predator with eight powerful clawed paws and natural brown-tan camouflage fur speckled with green splotches. Has a 2m-long tail ending in a bony bulbous protrusion (used in mating rituals, not combat). Four paws can be brought to bear on an opponent including a rear raking attack.",
   "source": "WEG40209 The DarkStryder Campaign: Kathol Outback p.27 (Plant Food)"
  },
  {
   "name": "Binarian Carnivorous Plant",
   "type": "Ambush predator plant",
   "planet": "Binaros",
   "habitat": "Jungle undergrowth",
   "dex": "1D",
   "per": "1D",
   "str": "4D",
   "move": "0 (mouth moves in 2m radius)",
   "size": "3 meters tall",
   "orneriness": "Ambush predator",
   "special": "MISSILE WEAPONS: tendril 3D+1. HIDE: 4D. BRAWLING: 5D. TENDRILS: Six 3m-long tendrils entangle victims — no damage, but -1D to all Dex rolls and +1D to plant's next bite attack. Entangled characters make opposed Strength rolls or are drawn closer. BITE: 4D+1 damage from digestive juices every other round (via brawling). On incapacitated result, victim is swallowed. Only one victim can be entrapped at a time. Lies still until prey comes within reach, then snakes out tendrils to draw prey into its bulbous oval maw (three modified leaves atop a half-meter stalk adorned with alternating orange and red leaves). Can go months between feedings.",
   "source": "WEG40209 The DarkStryder Campaign: Kathol Outback p.28 (Plant Food)"
  },
  {
   "name": "Binaros Feeder",
   "type": "Mobile hunter plant (spore-spawned)",
   "planet": "Binaros",
   "habitat": "Mother plant territory; spreads to any warm, dark, metal-rich environment",
   "dex": "2D+2",
   "per": "1D",
   "str": "1D+1",
   "move": "15",
   "size": "0.5 meters tall",
   "orneriness": "Mindless hunter — swarms prey",
   "special": "BRAWLING PARRY: 3D+2, RUNNING: 4D+1. SEARCH: 2D+1, SNEAK: 4D, HIDE: 4D. BRAWLING: 2D. PROBOSCIS: Needle-like proboscis stabs for STR+1D damage. If attached for more than one turn, begins sucking blood for +1D damage per turn. BRAWLING SWARM: Each additional feeder beyond the first attacking an opponent grants +1D to all attackers' brawling rolls (up to 5 feeders on a human-sized target). Spawned by a mother plant to gather water, minerals, and nitrogen (from blood). Four fleshy vine-like legs, lichen-encrusted underbelly with water-storage sacs, spiny thorned carapace, and protruding bony proboscis. Non-intelligent — responds to environmental cues triggering hunt, flee, and feed behaviors. Prefers darkened corridors and shadowed spaces.",
   "source": "WEG40209 The DarkStryder Campaign: Kathol Outback p.35 (Plant Food)"
  },
  {
   "name": "Binaros Mother Plant",
   "type": "Huge spawning plant (Speeder scale)",
   "planet": "Binaros",
   "habitat": "Warm, dark, metal-rich environments",
   "dex": "—",
   "per": "—",
   "str": "5D+1",
   "move": "0",
   "size": "10 meters in diameter",
   "orneriness": "Non-sentient",
   "special": "SCALE: Speeder. MAW: STR+1D damage but feeders must place victim inside to attack. SIZE: +10 to hit rolls to strike a vital part (otherwise attacks do not harm it). WEAKNESS: Energy weapons do +1D damage. In natural habitat on Binaros, the adult plant is dormant during daylight and folds in on itself, unfurling only at night. Life cycle: grows from spore, generates feeders to bring nutrients (water, minerals, metals for growth, then blood for nitrogen when preparing to spawn new spores). Completes cycle in 1-2 weeks. Round body with a huge thorny-toothed maw hidden by leafy stalks, six short taproots and dense secondary root mat, green coloration with brown roots. Emits a mild sweet-honey and earthy scent. The FarStar's hold is actually a better environment than Binaros — warm, metal-rich, and the feeders solve the water problem by raiding hydroponics.",
   "source": "WEG40209 The DarkStryder Campaign: Kathol Outback p.37 (Plant Food)"
  },
  {
   "name": "Ghest",
   "type": "Cartilaginous reptile predator",
   "planet": "Rodia (transplanted to Danoor's irrigation canals)",
   "habitat": "Dark, flooded underground tunnels",
   "dex": "1D",
   "per": "2D",
   "str": "7D",
   "move": "15 (swimming), 8 (walking)",
   "size": "6 meters long",
   "orneriness": "Ambush predator",
   "special": "TEETH: STR+2D damage. CLAWS: STR+1D damage. A 6-meter reptilian predator with slick skin and crooked razor-sharp teeth. Native to Rodia but somehow transplanted to Danoor's ancient abandoned irrigation canals beneath Eror Zeen, where it ambushes travelers from the murky water. Leaps up from the water with wicked claws and crooked teeth, striking before victims can fully react.",
   "source": "WEG40209 The DarkStryder Campaign: Kathol Outback p.93 (Galaxy's Edge)"
  },
  {
   "name": "Tunnel Worm",
   "type": "Mutated bio-experiment (aquatic predator)",
   "planet": "Alien Construct (Kathol Rift Waystation)",
   "habitat": "Flooded lower-level corridors of the ancient alien construct",
   "dex": "3D",
   "per": "2D",
   "str": "3D",
   "move": "15 (swimming)",
   "size": "3 meters long",
   "orneriness": "Stealth predator",
   "special": "BRAWLING PARRY: 5D. SEARCH: 6D, SNEAK: 5D. BRAWLING: 5D. INTERNAL LUMINESCENCE: Generates an internal phosphorescent glow — normally uses this to blind underwater lifeforms; appears dim to human eyes. ABSORPTION: Absorbs prey through its permeable outer membrane. Internal digestive acids have 4D Strength for damage. PERMEABLE BODY: Attempts to puncture the translucent creature will fail — it can adjust its outer membrane to let solids pass through. However, energy weapons inflict major damage: all blaster hits receive +1D bonus to damage. Result of an ancient alien experiment — the original designers intended 1cm versions to be introduced into the water supply to ingest bacteria. After centuries of evolution and the Rift's mutagenic effects, the transparent viscous-looking worm has evolved into its present predatory form.",
   "source": "WEG40121 The Kathol Rift p.65 (Waystation)"
  },
  {
   "name": "Owr'apa",
   "type": "Amphibious feline predator",
   "planet": "Yvara",
   "habitat": "Fungal plains watering holes and creeks",
   "dex": "4D",
   "per": "3D",
   "str": "3D+2",
   "move": "10 (running), 12 (jumping), 6 (swimming)",
   "size": "1.5-2 meters long, 0.9m at shoulder",
   "orneriness": "Territorial predator",
   "special": "BRAWLING: claws 5D+1, swimming 4D+2. CLAWS: STR+1D slash damage. LEAP: Can leap up to 12 meters with a Moderate jumping roll. AMPHIBIOUS: Pair of extra nostrils on a snorkel-horn allows indefinite underwater breathing. A large feline with a thick dark pelt. Submerges itself in watering holes found on the long dry treks of zeebuck herds, concealing its breathing snorkel among the reeds. Uses claws and incredible leaps to ambush prey. Territorial and will attack any creatures straying too close to its watering holes.",
   "source": "WEG40121 The Kathol Rift p.76 (Home)"
  },
  {
   "name": "Zeebuck",
   "type": "Grazing ruminant",
   "planet": "Yvara",
   "habitat": "Fungal plains",
   "dex": "2D",
   "per": "4D",
   "str": "4D",
   "move": "11",
   "size": "1.5 meters at shoulder",
   "orneriness": "3D",
   "special": "RUNNING: 4D. Knobby-boned three-eyed four-legged ruminant that feeds off the fungal plains of Yvara. Tan to cinnamon-brown fur. Always on the move. Gorges on the fungus when predators aren't around; proteins and essential nutrients the zeebuck can't immediately process are diverted to tubular sacks that grow down the length of its neck. These sacks are eaten by young when food is scarce, or re-assimilated after a period. Yvarema do not hunt, but occasionally tame zeebucks and harvest the protein sacks.",
   "source": "WEG40121 The Kathol Rift p.76 (Home)"
  },
  {
   "name": "DarkStryder Fire Creature",
   "type": "DarkStryder-conjured fiery apparition",
   "planet": "Alien Construct (Kathol Rift Waystation)",
   "habitat": "Wherever summoned by its DarkStryder control artifact",
   "dex": "3D",
   "per": "1D",
   "str": "2D",
   "move": "15",
   "size": "20 meters (scale: Speeder)",
   "orneriness": "Controlled apparition",
   "special": "SEARCH: 4D. FIRE: Inflicts 6D damage. A glowing deep-red cone-shaped fiery apparition that reaches almost 20 meters high and 20 meters wide, with a discernable eerie monstrous face, glowing ember-eyes, and huge ravenous jaws. Mist deploys this creature via a silvery-white wire-like DarkStryder artifact coiled around an arm — the wire glows red as it activates. Duration: 1D rounds, after which the fire dissipates. Blaster fire has no effect. The controller can direct the creature to herd or attack specific targets.",
   "source": "WEG40121 The Kathol Rift p.68 (Waystation)"
  }
  ,
  // === WEG40215 Lords of the Expanse — Creatures ===
  { name: "Rodian Karstag", type: "Swamp-dwelling predator", planet: "Vilhon (imported from Rodia)", habitat: "Swampy/damp forests", dex: "2D+1", per: "3D+1 (hide 4D+1, sneak 4D)", str: "3D+2 (brawling: tail 4D+2)", move: "9 (8 swimming)", size: "1.5m shoulder, 4-5m long", orneriness: "3D+2", special: "Thick Hide: +1D resist physical and energy. Bite: STR damage. Antlered Tail Club: damage varies by age (0-1yr STR, 1-3yr STR+1D, 3-5yr STR+2D, 5-10yr STR+3D, 10+yr STR+4D). Number of tail spikes increases with age. Four powerful webbed feet grant good mobility in water and swamps.", source: "WEG40215 Lords of the Expanse: Campaign Guide p.58" },
  { name: "Mrlssti Mynkin", type: "Ambush predator", planet: "Mrlsst", habitat: "Greenstalk forests, caves", dex: "2D", per: "3D+1 (hide 5D+1, sneak 4D)", str: "3D", move: "7", size: "2+ meters tall", orneriness: "3D", special: "Claws: STR+1D damage. Bite: STR+2D damage. Stone Hide Camouflage: +2D to hide against stone surfaces. Stone Hide Armor: +1D to Strength to resist damage. Humanoid shape with gangly arms, stumpy legs, huge tooth-filled maw, mottled plate scales that can be manipulated to blend with rocky surfaces.", source: "WEG40215 Lords of the Expanse: Campaign Guide p.27" }
  ,
  { name: "Crimson Slug", type: "Space-dwelling parasite", planet: "Tapani Sector (nebula)", habitat: "Nebulae, derelict ships", dex: "2D", per: "1D", str: "5D", move: "8 (4 in gravity)", size: "6-8 meters", orneriness: "2D", special: "Vacuum: Native to space, needs no assistance to survive in vacuum. Teeth: STR+1D damage. Heat-starved — attracted to engine heat and power sources. In standard gravity, Dexterity drops to 1D and Move to 4. Heavy gravity (2x normal) inflicts one Wound level per minute.", source: "WEG40147 Tapani Sector Instant Adventures p.36 (Lost Destiny)" }
  ,
  { name: "Large Nightsinger", type: "Canyon-dwelling flying predator", planet: "Nightsinger's Orb (Pella system)", habitat: "Canyon crags", dex: "3D (dodge 4D+2, melee combat: claws 6D+2, melee combat: beak 6D)", per: "4D", str: "4D+2 (brawling 5D)", move: "8", size: "Large (bat-winged)", orneriness: "3D+1", special: "Claws: STR+2D damage. Beak: STR+3D damage. Echolocation: Uses eerie high-pitched screams for communication and sonar to navigate windswept canyons and locate prey. Bat-winged with elongated heads used as rudders, sharp talons. Considered a great delicacy. Only one is ever hunted per family gathering in tribute to fallen kin.", source: "WEG40147 Tapani Sector Instant Adventures p.49 (Blood Inheritance)" }
// --- END OF CREATURES_DATA ---
];
