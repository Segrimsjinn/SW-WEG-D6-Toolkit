...
- **Version counter**: The header badge shows `v1.0.{commit count}` below the commanders count. After every commit+push, update the version number in `index.html` (search for `v1.0.`) to match the new total commit count (`git rev-list --count HEAD`). Tell the user the new version number so they can verify the deploy landed after hard refresh.
