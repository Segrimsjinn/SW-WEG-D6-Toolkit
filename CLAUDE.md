...
- **Sisar Run** (WEG40136) — 21 systems, pre-computed from route map, multi-grid corridor. Gateways: Sriluur (default), Reibrin (from R-7/R-8/S-7), Novor (from R-7/S-7), Nwarcol Point (from S-7/T-7/T-8), Sespe (from S-9/T-9/T-10), Ques (from T-8/T-9).
- **Kathol Sector** (WEG40209) — 17 systems, pre-computed from route map (Trition Trade Route + lesser-known routes), grid M-21. Gateway: Kal'Shebbol (default).
- **Kathol Outback** (WEG40209) — 18 systems, pre-computed from route map (all lesser-known routes across Marcol Void), grid M-21. Gateways: Galtea (default), Sebiris (default). Shared systems with Kathol Sector: Galtea, Sebiris.
- **Tapani Sector** (WEG40215) — 89 systems, pre-computed from route map (tree-structured network with hyperspace buoys), grid L-13. Gateways: Achillea (default), Mrlsst (from L-12/K-13), Tallaan (from L-14/M-13/M-14). Largest gazetteer — 3,916 pairs.
...
- **Planet detail → Notable NPC cross-link**: `linkifyNpcNames(text, system)` in `index.html` scans `NOTABLE_NPCS` base names against `p.locations` and `p.ofNote` text at render time. Any match becomes a cyan clickable span that calls `lookupNotableNpcByName(name)` — switching to the NPC tab, switching to the Lookups sub-tab, populating the Notable NPC search with the base name, and auto-opening the stat block when there's a unique match (otherwise showing the filtered list so multi-era entries can be picked).
...
- **Version counter**: The header badge shows the version below the commanders count. Format: `v1.XX.YYY` where `XX` = floor(commitCount / 1000) zero-padded to 2 digits, `YYY` = commitCount % 1000 zero-padded to 3 digits. Example: commit 1000 = `v1.01.000`, commit 1042 = `v1.01.042`, commit 2000 = `v1.02.000`. After every commit+push, compute the version from `git rev-list --count HEAD` and update in `index.html` (search for the current version string). Tell the user the new version number so they can verify the deploy landed after hard refresh.
